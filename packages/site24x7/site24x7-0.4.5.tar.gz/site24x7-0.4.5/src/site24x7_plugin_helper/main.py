...
parser.add_argument('--version',
    ...
    required=False,
    type=int)
parser.add_argument('--server', help='IPv4', type=str)
...
# load the environment variables from file location
...
if __name__ == "__main__":
    this_plugin_name = parser.parse_args().plugin
    ...
    driver_location = os.environ.get("DATABASE_DRIVER_PATH")
    if (server is None
        or database is None
        or username is None
        or password is None):
...
CLIENT = CacheClient(
    ...
    ignored_list=["database", "query", "this_plugin_name"]
)
