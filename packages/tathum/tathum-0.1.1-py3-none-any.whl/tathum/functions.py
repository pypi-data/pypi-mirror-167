...
def fill_missing_data(x: np.ndarray, y: np.ndarray, z: np.ndarray, time: np.ndarray,
...
def compute_transformation(surface_points_x: np.ndarray,
    ...
    :param axis_rotation: the axis around which the original surface was rotated relative to horizontal
    :param axis_angle: the axis based on which the original surface's orientation angle will be computed
    :param full_output: whether to return full output, which includes the objects for the plane and corners
    :return:
    """
    if axis_rotation == 'x':
        ax_rot = vg.basis.x
    elif axis_rotation == 'y':
        ax_rot = vg.basis.y
    elif axis_rotation == 'z':
        ax_rot = vg.basis.z
    else:
    ...
    screen_center = np.mean(surface_points, axis=0)
    surface_points = surface_points - screen_center

    # find the best fitting plane
    screen_plane = Plane.best_fit(surface_points)

    # get the screen's angle
    transform = Vector(screen_plane.cartesian())
    plane_vec = Vector(transform[:3] / np.linalg.norm(transform[:3]))

    # create an axis-angle representation of the surface
    axis = vg.perpendicular(ax_rot, plane_vec)  # rotate around x
    angle = vg.angle(ax_ang, plane_vec, units='rad')  # but angle is relative to y
    rotmat = matrix_from_axis_angle(np.hstack((axis, (angle,))))
    rotation = Rotation.from_matrix(rotmat)

    # counter rotate the surface points to transform them back to horizontal
    screen_corners_rot = Points(rotation.apply(surface_points))
...
def find_movement_bounds(velocity: np.ndarray,
                         ...
                         allow_multiple_segments=True) -> (int, int):
    """
    ...
    :return: the movement initiation and termination indices
    ...
    if len(vel_threshold_ind) == 0:
        ...
        move_end_ind = vel_threshold_ind[-1] + 1 if vel_threshold_ind[-1] < 99 else len(vel_eval) - 1
...
def b_spline_fit_1d(time_vec, coord, n_fit, smooth=0.):
    """
