...
class RectangularBlock(Block):
    ...
    def homogenize(self):
        ...
        radius_0, phi_0 = self.areas[0].get_line(0).p1.get_r(), self.areas[0].get_line(0).p1.get_phi()
        radius_1, phi_1 = self.areas[0].get_line(0).p2.get_r(), self.areas[0].get_line(0).p2.get_phi()
        radius_2, phi_2 = self.areas[-1].get_line(2).p1.get_r(), self.areas[-1].get_line(2).p1.get_phi()
        radius_3, phi_3 = self.areas[-1].get_line(2).p2.get_r(), self.areas[-1].get_line(2).p2.get_phi()

        param_dct = dict(no=self.block_def.no, type=2, nco=self.block_def.nco, x=self.block_def.x, y=self.block_def.y,
                         radius_0=radius_0, radius_1=radius_1, radius_2=radius_2, radius_3=radius_3,
...
class HomogenizedRectangularBlock(RectangularBlock):
    ...
    def build_block(self):
        ...
        # |         |
        # 0 ------- 1
        p0 = Point.of_polar(self.block_def.radius_0, self.block_def.phi_0)
        p1 = Point.of_polar(self.block_def.radius_1, self.block_def.phi_1)
        p2 = Point.of_polar(self.block_def.radius_2, self.block_def.phi_2)
        p3 = Point.of_polar(self.block_def.radius_3, self.block_def.phi_3)
