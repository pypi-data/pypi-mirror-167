          ...
          if "Format" in parsedMsg:
            if parsedMsg["Format"] == "SoundHoundVoiceSearchParialTranscript" or parsedMsg["Format"] == "HoundVoiceQueryPartialTranscript":
              try:
                ...
                pass
              try:
                if len(parsedMsg["EntityDetection"]) > 0:
                  listener.onEntityIdentification(parsedMsg["EntityDetection"], parsedMsg["FinalPartialTranscriptTokens"])
              except:
                pass
              try:
                if len(parsedMsg["TopicIdentification"]) > 0:
                  listener.onTopicIdentification(parsedMsg["TopicIdentification"], parsedMsg["FinalPartialTranscriptTokens"])
              except:
                pass
              try:
