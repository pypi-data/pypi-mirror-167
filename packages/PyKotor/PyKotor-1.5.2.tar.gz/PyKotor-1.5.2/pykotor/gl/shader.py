...
void main()
{
    ...
    dist = (10 - distance(vec2(position), vec2(cursor)))/10;
...
void main()
{
    //float distance = max(0, 1-distance(vec2(gl_FragCoord), vec2(cursor))/100);
    //float dist = (1000 - distance(vec2(gl_FragCoord), vec2(cursor)))/1000;
    
    vec4 diffuseColor = texture(diffuse, diffuse_uv);
    FragColor = diffuseColor * vec4(dist, dist, dist, 0);
}
"""
