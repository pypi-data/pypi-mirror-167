...
def excecuteElement(config,records,ps_delete_query):
    ...
    try:
        ...
        res = -1
        writeLogTxt(name=getNameFileOutput()+'_Log.txt',data="Error excecuteElement : " + str(error),root_directory_name="rep",dir_name = '_LOG_Postgres')
    finally:
...
def selectElement(config,where,sql_select_query,one=False):
    ...
    try:
        ...
        writeLogTxt(name=getNameFileOutput()+'_Log.txt',data="Error selectElement : "+ str(error) ,root_directory_name="rep",dir_name = '_LOG_Postgres')
...
def excecuteSQL(config,sql_query):
    ...
    try:
        ...
        print("Error while connecting to PostgreSQL :", error)
        writeLogTxt(name=getNameFileOutput()+'_Log.txt',data="Error excecuteSQL : "+str(error),root_directory_name="rep",dir_name = '_LOG_Postgres')
    finally:
        # closing database connection.
        if connection:
            cursor.close()
