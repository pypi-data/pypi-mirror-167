...
def process_report(client:Qoa_Client, interval:int, pid:int = None, metrics: dict = None, category='resource_monitor'):
    try:
        print("start add metric")
        client.add_metric(metrics['proc_cpu'],category=category)
        print("add cpu metric")
        client.add_metric(metrics['proc_mem'],category=category)
        print("add mem metric")
    except Exception as e:
