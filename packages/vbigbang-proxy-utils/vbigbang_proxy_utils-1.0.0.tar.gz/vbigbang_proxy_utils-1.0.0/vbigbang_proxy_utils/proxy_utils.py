...
@retry(retry=(retry_if_exception_type()),
       stop=(stop_after_delay(9 * 60) | stop_after_attempt(999)),
       wait=wait_random(min=5, max=10))
def get_proxy_list(kafka_topic: str, bootstrap_servers: List[str], kafka_username: str, kafka_password: str,
                   redis_conn: redis.Redis, city_name='北京市', check_pass_type: int = 1, http_req: bool = True,
                   reserve_city: str = '南宁市,防城港市', get_proxy_type: int = GetProxyType.QUEUE, province='北京市',
                   p_key: str = None, check_qqwry: bool = False, count: int = 1,
                   check_level: CheckBelongType = CheckBelongType.CHECK_CITY, remain_time: int = 5 * 60 * 1000,
                   use_lock: bool = False, proxy_type: int = HttpProxyType.XX):
    """
    获取野比大熊代理IP
