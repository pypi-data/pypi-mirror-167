...
class QVTKRenderWindowInteractor(QVTKRWIBaseClass):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self.grabGesture(Qt.PinchGesture)    # QGesture Class --》QPanGesture, QPinchGesture, QSwipeGesture, QTapAndHoldGesture, and QTapGesture
    ...
    def event(self, event):
        if event.type() == QEvent.Gesture: 
            # print('event.activeGestures():', event.activeGestures())
            self.gestureEvent(event)
        return super().event(event)
    

    def gestureEvent(self, event):
        """ 手势事件。Pan/Pinch/TapAndHold
        
        Pan/Pinch 参考GUISupport/Qt/QVTKInteractorAdapter.cxx
        TapAndHold 参考本文件中keyPressEvent(self, ev)和keyReleaseEvent(self, ev)，模拟一个键盘'f'事件
        """
        self._Iren.LeftButtonReleaseEvent()  # 释放touch的leftButton
        eventPosition = self._Iren.GetEventPosition()
        lastEventPosition = self._Iren.GetLastEventPosition()
        # print('eventPosition:', eventPosition)
        # scale = self._getPixelRatio()
        # print('scale:', scale)


        # if event.gesture(Qt.SwipeGesture) :
        #     print('i am in SwipeGesture. ' * 10)
            

        if event.gesture(Qt.PanGesture):  # and 0 
            event.accept(Qt.PanGesture)
            pan = event.gesture(Qt.PanGesture)
            delta = pan.delta()
            translation = [delta.x(), -delta.y()]
            self._Iren.SetTranslation(translation)
            pan_state = pan.state()
            if(
                pan_state == Qt.GestureFinished
                or pan_state == Qt.GestureCanceled):
                self._isPanning = False
                self._Iren.EndPanEvent()
            elif(pan_state == Qt.GestureStarted):
                self._isPanning = True
                self._Iren.StartPanEvent()
                self._Iren.PanEvent()
            else:
                self._Iren.PanEvent()


        if event.gesture(Qt.PinchGesture):  #   and 0
            event.accept(Qt.PinchGesture)
            pinch = event.gesture(Qt.PinchGesture)
            # position = pinch.centerPoint().toPoint()
            position = self.mapFromGlobal(pinch.centerPoint().toPoint())    # ???
            # print('pinch.centerPoint().toPoint():', position)
            self._Iren.SetEventInformationFlipY(position.x(),position.y())
            self._Iren.SetScale(1.0)
            self._Iren.SetScale((pinch.scaleFactor()-1)/10 + 1)
            # self._Iren.SetScale(pinch.scaleFactor())     #  totalScaleFactor
            # print('pinch.scaleFactor()', pinch.scaleFactor())
        
            case_state = pinch.state()
            if(
                case_state == Qt.GestureFinished
                or case_state == Qt.GestureCanceled):
                self._Iren.EndPinchEvent()
                # print("end pinch")
            elif(case_state == Qt.GestureStarted):
                self._Iren.StartPinchEvent()
                self._Iren.PinchEvent()
                # print("start pinch")
            else:
                self._Iren.PinchEvent()
                # print("else: pinch")

            # self._Iren.SetRotation(-pinch.lastRotationAngle())
            # self._Iren.SetRotation(-pinch.rotationAngle())
            # # print('pinch.lastRotationAngle():', pinch.lastRotationAngle())
            # # print('pinch.rotationAngle():', pinch.rotationAngle())

            # # case_state=pinch.state()
            # if(
            #     case_state==Qt.GestureFinished
            #     or case_state==Qt.GestureCanceled):
            #     self._Iren.EndRotateEvent()
            #     # print("end rotate")
            # elif(case_state==Qt.GestureStarted):
            #     self._Iren.StartRotateEvent()
            #     self._Iren.RotateEvent()
            #     # print("start rotate")
            # else:
            #     self._Iren.RotateEvent()
            #     # print("else: rotate")
        ...
        if event.gesture(Qt.TapAndHoldGesture):
            # print('i am in TapAndHoldGesture'* 10)
            tapAndHold = event.gesture(Qt.TapAndHoldGesture)
            position = tapAndHold.position().toPoint()
            position = self.mapFromGlobal(position) 
            # Press
        ...
        self._Iren.SetEventPosition(eventPosition)
        self._Iren.SetLastEventPosition(lastEventPosition)
        # self.update()
        return True
        ################### ~zhl #########################
...
def QVTKRenderWidgetConeExample():
    ...
    style = vtk.vtkInteractorStyleMultiTouchCamera()  #   MyEvent( )  #  vtk.vtkInteractorStyleTrackballCamera()  
    # help(style)
