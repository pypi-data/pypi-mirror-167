...
cdef np.ndarray[np.float64_t, ndim=3] _pairwise_distances(np.ndarray[np.float64_t, ndim=3] a, np.ndarray[
...
cdef np.ndarray[np.float64_t, ndim=1] _cdef_minimum_cross_distances(np.ndarray[np.float64_t, ndim=3] a0, np.ndarray[
    ...
    cdef np.ndarray[np.float64_t, ndim=1] min_r, r
    cdef np.ndarray[np.uint8_t, ndim=1, cast=True] is_closer
    ...
    for dx in np.arange(-1, 2):
        for dy in np.arange(-1, 2):
            for dz in np.arange(-1, 2):
                ...
                # Update min_r?
                r = np.zeros(a0.shape[0])
                for i in np.arange(a0.shape[0]):
                    r[i] = np.min(cdist(a0[i, :, :], am[i, :, :]))
