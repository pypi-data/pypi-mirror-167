...
def fetchData(contract, marketplaces, start, end):
    ...
    if 'OpenSea' in marketplaces:
        overall_prices = overall_prices + requestOpenSea(contract, start, end)

    if 'LooksRare' in marketplaces:
        overall_prices = overall_prices + requestLooksRare(contract, start, end)

    if 'X2Y2' in marketplaces:
        overall_prices = overall_prices + requestX2Y2(contract, start, end)
    
    overall_prices = list(map(int, overall_prices))
