...
from pyguppy_client_lib.helper_functions import get_barcode_kits, package_read, pull_read, get_server_stats,\
    get_server_information, get_return_code_message
...
class TestPyGuppyClient(unittest.TestCase):
    ...
    def test_04_pass_reads_and_get_completed(self):
        ...
        client.set_params({"client_name": "guppy_client_test_04_pass_reads_and_get_completed"})
        client.connect()
        self.assertEqual(client.get_status(), PyGuppyClient.connected)
    ...
    def test_06_pass_read_connection_error(self):
        ...
        client.set_params({"client_name": "guppy_client_test_06_pass_read_connection_error"})
    ...
    def test_07_get_completed_reads_raises(self):
        ...
        client.set_params({"client_name": "guppy_client_test_07_get_completed_reads_raises"})
    ...
    def test_09_set_params_already_connected(self):
        ...
        client.set_params({"client_name": "guppy_client_test_09_set_params_already_connected"})
    ...
    def test_14_return_code_messages(self):
        ...
        for name, enum_value in GuppyClient.result.__members__.items():
            ...
            self.assertIsNotNone(message, "Return code {} not supported by get_return_code_message method.".format(enum_value))
    ...
    def test_15_pass_reads_override_and_get_completed(self):
        ...
        client.set_params({"client_name": "guppy_client_test_15_pass_reads_override_and_get_completed"})
        client.connect()
        self.assertEqual(client.get_status(), PyGuppyClient.connected)
        ...
        while recv < sent:
            ...
            for split_read in reads:
                for read in split_read:
                    recv_ids.add(read["metadata"]["read_id"])
                    sequences1[read["metadata"]["read_id"]] = read["datasets"]["sequence"]
            time.sleep(0.1)
        ...
        while recv < sent:
            ...
            for split_read in reads:
                for read in split_read:
                    recv_ids.add(read["metadata"]["read_id"])
                    self.assertAlmostEqual(read["metadata"]["daq_scaling"] * 2, read["metadata"]["scaling_median"], 4)
                    self.assertAlmostEqual(read["metadata"]["daq_offset"] * 2, read["metadata"]["scaling_med_abs_dev"], 4)
                    sequences2[read["metadata"]["read_id"]] = read["datasets"]["sequence"]
            time.sleep(0.1)
