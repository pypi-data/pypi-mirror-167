...
# Each element is a tuple of three elements of the form (AT, TIMEDELTA, FUNC)
# where either AT can be None or TIMEDELTA can be None.
# See the functions defined below as `schedule_often`, `schedule_daily`.
SCHEDULES = []

def unused_schedule_often(every=10):
    def decorator(func):
        if settings.SITE.use_linod or settings.SITE.is_installed('linod'):
            td = timezone.timedelta(seconds=every)
            sched = (None, td, sync_to_async(func))
            if sched not in SCHEDULES:
                SCHEDULES.append(sched)
        return func
    return decorator

def unused_schedule_daily(at="20:00"):
    def decorator(func):
        if settings.SITE.use_linod or settings.SITE.is_installed('linod'):
            try:
                at = time.strptime(at, "%H:%M")
            except:
                at = time.strptime("20:00", "%H:%M")
            sched = (at, None, sync_to_async(func))
            if sched not in SCHEDULES:
                SCHEDULES.append(sched)
        return func
    return decorator
