...
def bubbleColumnTutorial():
    ...
    if foamVersionNumber()>=(3,):
        if path.exists(path.join(prefix, "twoPhaseEulerFoam")):
            return path.join(prefix,"twoPhaseEulerFoam","laminar","bubbleColumn")
        else:
            return path.join(prefix,"multiphaseEulerFoam","laminar","bubbleColumn")
    else:
...
class FoamFileGeneratorRoundtrip4(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            copyfile(path.join(buoyHotRoomTutorial(),"0.orig","U"),self.theFile)
    ...
    def setUp(self):
        ...
        try:
            ...
            try:
                ...
                copyfile(path.join(bubbleColumnTutorial(),"0",alphaName + ".orig"),self.theFile)
...
class IncludeFilesRoundTrip(unittest.TestCase):
    def setUp(self):
        ...
        self.theFile=path.join(usedDir,"U" + suffix)
