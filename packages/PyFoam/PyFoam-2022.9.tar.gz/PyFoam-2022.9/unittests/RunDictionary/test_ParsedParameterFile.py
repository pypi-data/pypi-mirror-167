...
def buoyHotRoomTutorial():
    ...
    if foamVersionNumber()>=(3,):
        if path.exists(path.join(prefix, "buoyantSimpleFoam")):
            return path.join(prefix,"buoyantSimpleFoam","hotRadiationRoom")
        else:
            return path.join(prefix,"buoyantFoam","hotRadiationRoom")
    else:
...
def XiFoamMoriyoshiTutorial():
    prefix=foamTutorials()
    if (foamFork()=="openfoamplus" and foamVersionNumber()>=(1612,)) or (foamFork()=="openfoam" and foamVersionNumber()>=(10,)):
        ras="RAS"
    else:
        ras="ras"
...
nix
    ...
    def test_false_reaction_openfoam_10(self):
        p1 = FoamStringParser("""functions
{
...
class ParsedBoundaryDictTest(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            copyfile(path.join(foamTutorials(),"incompressible","simpleFoam","airFoil2D","constant","polyMesh.orig","boundary.gz"),self.theFile + ".gz")
...
class ParsedParameterFileTest(unittest.TestCase):
    def setUp(self):
        self.theFile=mktemp()
        self.momentumTransport = False
        turb=path.join(simplePitzTutorial(),"constant")
        if oldApp():
            turb=path.join(turb,"turbulenceProperties")
        elif foamVersionNumber()>=(3,):
            if path.exists(path.join(turb, "momentumTransport")):
                turb=path.join(turb,"momentumTransport")
                self.momentumTransport = True
            else:
                turb=path.join(turb,"turbulenceProperties")
        else:
    ...
    def setUp(self):
        ...
        try:
            ...
            try:
                ...
                copyfile(path.join(buoyHotRoomTutorial(),"0.orig","T"),self.theFile)
...
class ParsedParameterFileIncludeTest(unittest.TestCase):
    def setUp(self):
        if oldTutorialStructure():
            ...
            null="0.org"
        suffix = ""
        if not path.exists(path.join(simpleBikeTutorial(),null)):
            null = "0"
            suffix = ".orig"
        self.fileName=path.join(simpleBikeTutorial(),null,"U" + suffix)
...
class ParsedParameterFileIncludeFuncTest(unittest.TestCase):
    ...
    def testBasicIncludeFunc(self):
        if foamFork() == "openfoam" and foamVersionNumber() >= (10, ):
            with pytest.raises(PyFoamParserError, match="Parameters for #includeFunc not implemented"):
                _ = ParsedParameterFile(self.fileName,doMacroExpansion=True)
        else:
            test=ParsedParameterFile(self.fileName,doMacroExpansion=True)
...
class ParsedParameterFileIncludeFuncQuotedTest(unittest.TestCase):
    ...
    @pytest.mark.skipif(foamTutorials()=='',reason="$FOAM_TUTORIALS is not defined")
    @pytest.mark.skipif(foamFork()=="openfoam" and foamVersionNumber()>=(10,),reason="backwardFacing tutorial no longer exists for this version")
    def testBasicIncludeFunc(self):
        test=ParsedParameterFile(self.fileName,doMacroExpansion=True)
...
class ReadIncludeAndMacroExpansionTest(unittest.TestCase):
    def setUp(self):
        ...
        src = path.join(foamTutorials(),"incompressible","simpleFoam","motorBike",null)
        if not path.exists(src):
            src = path.join(foamTutorials(),"incompressible","simpleFoam","motorBike", "0")

        copytree(src, self.dest)
...
class SemicolonParserProblems(unittest.TestCase):
    def test_double_semicolon(self):
        p1=FoamStringParser("test 1.23e-4; ;")
        self.assertAlmostEqual(p1["test"],1.234e-4,6)
        assert len(p1.data) == 1
        self.assertEqual(str(p1),"test 0.000123;\n")
