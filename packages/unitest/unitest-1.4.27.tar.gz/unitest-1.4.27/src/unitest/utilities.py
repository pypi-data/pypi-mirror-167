...
class Utilities(object):
    ...
    def check_status_code_with_custom_header(self,
        ...
        try:
            assert response.status_code == expectedStatusCode, \
                f"Expected Status Code {expectedStatusCode} and Actual status {response.status_code} code did not match with failure reason"
            return response
        except:
            # logger.error(response.headers)
            pytest.fail(f"Expected Status Code {expectedStatusCode} and Actual status {response.status_code} code did not match")
