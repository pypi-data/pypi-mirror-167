...
class LineTransformer():
    ...
    @staticmethod
    def transform_script(lines : List[str], init_offset=7, dev=False) -> List[str]:
        '''
        ...
        for i, line in enumerate(lines):
            ...
            if token_chain.is_valid() and not in_block_quote:
                ...
                if token_chain.get_command() == '@linkedcode':
                    ...
                    # Store MD
                    transformed_lines.append(f"{token_chain.get_indentation()}app.create_output({i - init_offset + 1}, {i - init_offset + 1}, '', f'{markdown_text}', 'markdown', '')")
                    continue
                ...
                if '@output' == token_chain.get_command() and has_data_arg:
                    ...
                    # append output
                    if data is None:
                        transformed_lines.append(f"{token_chain.get_indentation()}app.create_output({line_start}, {line_end}, 'element_{i}', {None}, '{component_type}', '', {named_arg_str})")
                    else:
                ...
                if '@delay' == token_chain.get_command() and len(token_chain.get_named_args()) > 0 and token_chain.get_named_args()[0][0] == 'time':
                    ...
                    try:
                        ...
                        delay_time = 1
                    transformed_lines.append(f'{token_chain.get_indentation()}app.delay({delay_time})')
                    continue
                ...
                if '@card' == token_chain.get_command():
                    ...
                    transformed_lines.append(f"{token_chain.get_indentation()}app.create_output({i - init_offset + 1}, {i - init_offset + 1}, 'card', {named_args}, 'card', '')")
                    continue
