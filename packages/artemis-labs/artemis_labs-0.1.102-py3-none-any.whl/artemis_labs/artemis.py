...
class Artemis:
    ...
    @staticmethod
    def init(runner_path="", launch_command = "", code_path="", launch=True, dev=False):
        '''
        ...
        # Initialize IO
        Artemis._stdout = None
        Artemis._stderr = None
        Artemis._r = None
        Artemis._w = None
        Artemis._thread = None
        Artemis._on_readline_cb = None
    ...
    @staticmethod
    def _on_readline(callback):
        '''
        Set callback to receive readline events
        '''
        Artemis._on_readline_cb = callback
    ...
    def _iohandler():
        ...
        Handle IO events
        '''
        while not Artemis._w.closed:
            try:
                while True:
                    line = Artemis._r.readline()
                    if line.strip() == 'KeyboardInterrupt':
                        Artemis._unhook_io()
                        break
                    print(line, file=Artemis._stdout, end='')
                    if len(line) == 0:
                        break
                    if Artemis._on_readline_cb:
                        Artemis._on_readline_cb(line)
            except Exception as exception:
    ...
    def _hook_io():
        ...
        '''
        Artemis._stdout = sys.stdout
        Artemis._stderr = sys.stderr
        io_r, io_w = os.pipe()
        io_r, io_w = os.fdopen(io_r, 'r'), os.fdopen(io_w, 'w', 1)
        Artemis._r = io_r
        Artemis._w = io_w
        sys.stdout = Artemis._w
        sys.stderr = Artemis._w
        Artemis._thread = threading.Thread(target=Artemis._iohandler)
        Artemis._thread.start()
    ...
    def _unhook_io():
        ...
        Artemis._w.close()
        if Artemis._thread:
            Artemis._thread.join()
        Artemis._r.close()
        sys.stdout = Artemis._stdout
        sys.stderr = Artemis._stderr
    ...
    def callback_handler(message : str) -> None:
        ...
        if message['type'] != 'ping':
            ...
            if message['type'] == 'query':
                ...
                subprocess.Popen(['artemis_labs', Artemis.code_path, Artemis.launch_command , dev_arg, 'nolaunch'], creationflags=subprocess.CREATE_NO_WINDOW)
    ...
    def create_input(line_start : int, line_end : int, name : str, comment : str) -> None:
        ...
        :return: None
        '''
        Artemis.artemis_socket.send(json.dumps({"type": "create", "element": "input", 'line_start' : line_start, 'line_end': line_end, 'name' : name, 'comment': comment}))
    ...
    def run(launch=True) -> None:
        ...
        if Artemis.mode == "code":
            with open(os.path.join(Artemis.cur_dir, Artemis.code_path), 'r', encoding='utf-8') as file:
                init_packet = { 'type' : 'init', 'state' : file.read(), 'settings' : json.dumps(Artemis.settings) }
                out_str = json.dumps(init_packet)
                out_str  = out_str.replace("<", "&lt;").replace(">", "&gt;")
                Artemis.artemis_socket.send(out_str)
        else:
            init_packet = { 'type' : 'init', 'state' : json.dumps(Artemis.app) }
            Artemis.artemis_socket.send(json.dumps(init_packet))
