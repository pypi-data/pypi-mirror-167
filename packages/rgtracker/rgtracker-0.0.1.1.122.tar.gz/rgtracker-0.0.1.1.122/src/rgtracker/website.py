...
class Website:
    ...
    def create_new_metrics(self):
        try:
            execute('CMS.INITBYDIM', self.metric_pageviews_key, self.cms_width, self.cms_depth)
            tracker_log(f'CMS.INITBYDIM {self.metric_pageviews_key}', f'Website - create_new_metrics - ')
        ...
        try:
            ...
            tracker_log(f'PFADD {self.metric_unique_device_key}', f'Website - create_new_metrics - ')
        except Exception as e:
            # tracker_log(f'{e} - {self.metric_pageviews_key}', 'Website - ')
            pass
    ...
    def create_metrics(self):
        ...
        #
        # for id in ids:
        #     ud_key = create_key_name(Type.HLL.value, '', Dimension.WEBSITE.value, id, parsed_key.get('ts'),
        #                              Metric.UNIQUE_DEVICES.value)
        #     try:
        #         execute('PFADD', ud_key)
        #         tracker_log(f'PFADD {ud_key}', f'Website - create_metrics - ')
        #     except Exception as e:
        #         # tracker_log(f'{e} - {self.metric_pageviews_key}', 'Website - ')
        #         pass

    def incr_new_metrics(self, page_id, device_id):
        execute('CMS.INCRBY', self.metric_pageviews_key, self.id, 1)
        tracker_log(f'CMS.INCRBY {self.metric_pageviews_key} {self.id} 1', f'Website - incr_new_metrics - ')
        execute('PFADD', self.metric_unique_device_key, device_id)
        tracker_log(f'PFADD {self.metric_unique_device_key} {device_id}', f'Website - incr_new_metrics - ')
    ...
    #
    #     for id in ids:
    #         execute('CMS.INCRBY', self.metric_pageviews_key, id, 1)
    #         tracker_log(f'CMS.INCRBY {self.metric_pageviews_key} {id} 1', f'Website - incr_metrics - ')
    #
    #         ud_key = create_key_name(Type.HLL.value, '', Dimension.WEBSITE.value, id, parsed_key.get('ts'),
    #                                  Metric.UNIQUE_DEVICES.value)
    #         execute('PFADD', ud_key, device_id)
    #         tracker_log(f'PFADD {ud_key} {device_id} 1', f'Website - incr_metrics - ')


    def incr_new_minute_metrics(self, page_id, device_id):
        execute('CMS.INCRBY', self.metric_pageviews_key, self.id, 1)
        tracker_log(f'CMS.INCRBY {self.metric_pageviews_key} {self.id} 1', f'Website - incr_new_minute_metrics - ')
        execute('PFADD', self.metric_unique_device_key, device_id)
        tracker_log(f'PFADD {self.metric_unique_device_key} {device_id}', f'Website - incr_new_minute_metrics - ')

    def incr_new_record_metrics(self, page_id, device_id):
        execute('CMS.INCRBY', self.metric_pageviews_key, self.id, 1)
        tracker_log(f'CMS.INCRBY {self.metric_pageviews_key} {self.id} 1', f'Website - incr_new_record_metrics - ')
        execute('PFADD', self.metric_unique_device_key, device_id)
        tracker_log(f'PFADD {self.metric_unique_device_key} {device_id}', f'Website - incr_new_record_metrics - ')

    def expire_new_metrics(self):
        # Check if CMS and HLL keys have already expired time fixed
        ...
        if ttl_pageviews == -1:
            ...
            tracker_log(f'Expire key {self.metric_pageviews_key} = {expire_pg}', f'Website - expire_new - ')
        ...
        if ttl_unique_devices == -1:
            ...
            tracker_log(f'Expire key {self.metric_unique_device_key} = {expire_ud}', f'Website - expire_new - ')

    # Todo: put expire time in method params
    def expire_metrics(self):
        expire_pg = execute('EXPIRE', self.metric_pageviews_key, 3600)
        tracker_log(f'Expire key {self.metric_pageviews_key} = {expire_pg}', f'Website - expire - ')

        expire_ud = execute('EXPIRE', self.metric_unique_device_key, 3600)
        tracker_log(f'Expire key {self.metric_unique_device_key} = {expire_ud}', f'Website - expire - ')

        # parsed_key = parse_key_name(self.dimension_ts_key)
        # # tracker_log(f'{parsed_key}', 'Website - 0 - ')
        # #
        # current_ts = int(parsed_key.get("ts"))
        # prev_unix_ts = int((datetime.fromtimestamp(current_ts / 1000) - timedelta(minutes=1)).timestamp() * 1000)
        # # # Todo: create index_name with method instead hard coding it
        # # # FixMe: IDS maybe not present in website json/referential when it's new website and when we try to expire it?
        # ids_raw = execute('FT.SEARCH', 'I::W:::', f'@last_visited:[{prev_unix_ts} {current_ts}]', 'RETURN', '1', 'id')
        # ids = []
        # for d in ids_raw:
        #     if type(d) is list:
        #         ids.append(d[-1])
        # # tracker_log(f'{prev_unix_ts} {current_ts} - {ids}', 'Website - 1 - ')
        #
        # for id in ids:
        #     ud_key = create_key_name(Type.HLL.value, '', Dimension.WEBSITE.value, id, parsed_key.get('ts'),
        #                              Metric.UNIQUE_DEVICES.value)
        #     expire_ud = execute('EXPIRE', ud_key, 3600)
        #     tracker_log(f'Expire key({execute("EXISTS", ud_key)}) {ud_key} = {expire_ud}', 'Website - expire - ')
        #     # if expire_ud != 1:
        #     #     tracker_log(f'Expire key {ud_key} = {expire_ud}', f'Website - Warning - ', 'warning')

    def check_if_need_to_expire(self):
        # Check if CMS and HLL keys have already expired time fixed
        ttl_pageviews = execute('TTL', self.metric_pageviews_key)
        # tracker_log(f'TTL {self.metric_pageviews_key} {ttl_pageviews}', 'Website - expire new - ')
        if ttl_pageviews == -1:
            expire_pg = execute('EXPIRE', self.metric_pageviews_key, 3600)
            tracker_log(f'Expire key {self.metric_pageviews_key} = {expire_pg}', f'Website - check_if_need_to_expire - ')

        ttl_unique_devices = execute('TTL', self.metric_unique_device_key)
        # tracker_log(f'TTL {self.metric_unique_device_key} {ttl_unique_devices}', 'Website - expire new ')
        if ttl_unique_devices == -1:
            expire_ud = execute('EXPIRE', self.metric_unique_device_key, 3600)
            tracker_log(f'Expire key {self.metric_unique_device_key} = {expire_ud}', f'Website - check_if_need_to_expire - ')
    ...
    def notification_of_active_websites(self, stream_names):
        ...
        tracker_log(f'{prev_unix_ts} {current_ts} active websites {ids}', f'Website - notification_of_active_websites - ')
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*',
                'ts', parsed_key.get("ts"),
                'status', 'active',
                'ids', json.dumps(ids))
    ...
    def log_new_website(self, device_id):
        tracker_log(f'NEW WEBSITE! {self.id}_{self.last_visited}_{device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_minute(self, device_id):
        tracker_log(f'CLAP! {self.id}_{self.last_visited}_{device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_records(self, device_id):
        tracker_log(f'NEW RECORD! {self.id}_{self.last_visited}_{device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')
...
def load_website(website, section, page, device, output_stream_names):
    if website.is_exists() != 1:
        ...
        website.create()
        website.create_new_metrics()
        website.incr_new_metrics(page.id, device.id)
        ...
        if website.has_metrics() < 1:
            website.log_new_minute(device.id)
            website.create_metrics()
            website.incr_new_minute_metrics(page.id, device.id)
            ...
            website.incr_new_record_metrics(page.id, device.id)
            website.check_if_need_to_expire()
            website.update_last_visited()
