    ...
    def is_weird(self):
        ...
        if delta > 80:
            ...
            output_log = {
                'id': self.id,
                'now_ts': ts,
                'record_ts': datetime.fromtimestamp(int(self.last_visited) / 1000),
                'delta': delta,
                'bucket_ts': datetime.fromtimestamp(int(parse_key_name(self.dimension_ts_key).get("ts")) / 1000)
            }
            tracker_log(f'WEIRDO!\t{json.dumps(output_log, indent=2)}', f'WEbsite - ')
            return True
        else:
            return False
