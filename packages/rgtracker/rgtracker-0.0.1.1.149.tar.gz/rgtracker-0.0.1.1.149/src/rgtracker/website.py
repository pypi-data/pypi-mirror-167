...
class Website:
    ...
    def update_last_visited_minute(self):
        execute('JSON.SET', self.dimension_key, '.last_visited', self.last_visited)
        tracker_log(f'JSON.SET {self.dimension_key} .last_visited {self.last_visited}', 'Website - update_last_visited_minute - ')

    def update_last_visited_record(self):
        execute('JSON.SET', self.dimension_key, '.last_visited', self.last_visited)
        # tracker_log(f'JSON.SET {self.dimension_key} .last_visited {self.last_visited}', 'Website - update_last_visited_record - ')
    ...
    def notification_of_active_websites(self, stream_names):
        ...
        ids_raw = execute('FT.SEARCH', 'I::W:::', f'@last_visited:[{prev_unix_ts} 9999999999999]', 'RETURN', '1', 'id')
        ...
        tracker_log(f'[{prev_unix_ts} {self.last_visited} {current_ts}] active websites {ids}', f'Website - notification_of_active_websites - ')
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*',
                'ts', parsed_key.get("ts"),
                'status', 'active',
                'ids', json.dumps(ids))
    ...
    def log_raw_record(self, device_id):
        tracker_log(f'RAW!\t\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_website(self, device_id):
        tracker_log(f'NEW WEBSITE!\t\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_minute(self, device_id):
        tracker_log(f'CLAP!\t\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_records(self, device_id):
        tracker_log(f'NEW RECORD!\t\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')
...
def load_website(website, section, page, device, output_stream_names):
    ...
    if website.is_exists() != 1:
        ...
        if website.has_metrics() < 1:
            ...
            website.update_last_visited_minute()
            ...
            website.update_last_visited_record()
