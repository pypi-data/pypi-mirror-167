...
class Website:
    ...
    def log_raw_record(self, device_id):
        tracker_log(f'RAW!\t\t{self.id} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_website(self, device_id):
        tracker_log(f'NEW WEBSITE!\t\t{self.id} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_minute(self, device_id):
        tracker_log(f'CLAP!\t\t{self.id} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')

    def log_new_records(self, device_id):
        tracker_log(f'NEW RECORD!\t\t{self.id} {self.last_visited} {device_id} at {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')
