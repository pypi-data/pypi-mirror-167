...
def transform(records, number_of_rotated_keys, dimension):
    ...
    tracker_log(f'IDS: {df["ids"].values.tolist()}', 'RotatePG-1to5-W - ')
    ...
    tracker_log(f'TS {grouped_df["ts"].values.tolist()}', 'RotatePG-1to5-W - ')
    ...
    for x in range(chunks):
        ...
        if df_sliced.shape[0] >= number_of_rotated_keys:
            ...
            tracker_log(f'UNIQUE_IDS: {unique_ids}', 'RotatePG-1to5-W - ')
