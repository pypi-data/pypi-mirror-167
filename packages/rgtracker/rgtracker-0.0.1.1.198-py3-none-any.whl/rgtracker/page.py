...
class Page:
    ...
    def notification_of_new_page(self, stream_names):
        ...
        # tracker_log(f'{parsed_key.get("ts")} new page {self.id}', f'Page - notification_of_new_page - ')
    ...
    def notification_of_active_pages(self, stream_names):
        ...
        # FixMe: HOT FIX
        if len(ids) > 0:
            # tracker_log(f'{prev_unix_ts} {current_ts} active pages {ids}', f'Page - notification_of_active_pages - ')
            execute('XADD', stream_names.get('page_pageviews'), 'MAXLEN', '86400000', '*',
                    'ts', parsed_key.get("ts"),
                    'status', 'active',
                    'ids', json.dumps(ids))
            execute('XADD', stream_names.get('page_unique_devices'), 'MAXLEN', '86400000', '*',
                    ...
                    'ids', json.dumps(ids))
        # else:
            # tracker_log(f'[{prev_unix_ts} {current_ts}] active Page {ids}', f'Page - notification_of_active_websites - warning - ')
    ...
    def is_weird(self):
        ...
        if delta > 80:
            # tracker_log(f'WEIRDO!\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} VS {ts} => {delta} seconds at {parse_key_name(self.dimension_ts_key).get("ts")} {datetime.fromtimestamp(int(parse_key_name(self.dimension_ts_key).get("ts")) / 1000)}', f'Page - ')
            return True
        else:
            return False
