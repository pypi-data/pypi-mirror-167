...
def transform(records, number_of_rotated_keys, dimension):
    ...
    for x in range(chunks):
        ...
        if df_sliced.shape[0] >= number_of_rotated_keys:
            ...
            # tracker_log(f'chunk_{x}_ts - {df_sliced["ts"].values.tolist()}', f'W-PG-1to5 - transform - ')
            # tracker_log(f'chunk_{x}_ids - {df_sliced["ids"].values.tolist()}', f'W-PG-1to5 - transform - ')
            ...
            # tracker_log(f'chunk_{x}_unique_ids - {unique_ids}', f'W-PG-1to5 - transform - ')
    ...
    # tracker_log(f'final_ids - {results.get("ids")}', f'W-PG-1to5 - transform - ')
    return results
