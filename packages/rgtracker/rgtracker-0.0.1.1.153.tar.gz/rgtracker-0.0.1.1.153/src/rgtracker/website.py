...
class Website:
    ...
    def is_weird(self):
        ...
        if delta > 80:
            tracker_log(f'WEIRDO!\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} VS {ts} => {delta} seconds at {parse_key_name(self.dimension_ts_key).get("ts")} {datetime.fromtimestamp(int(parse_key_name(self.dimension_ts_key).get("ts")) / 1000)}', f'Website - ')
            return True
        else:
            return False
...
def load_website(website, section, page, device, output_stream_names):
    ...
    if website.is_exists() != 1:
        ...
        # maybe the 'new' minute have already metrics, and we don't notify ..
