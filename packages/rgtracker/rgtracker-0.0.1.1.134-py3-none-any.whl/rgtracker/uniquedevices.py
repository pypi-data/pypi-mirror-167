...
def transform(records, number_of_rotated_keys, dimension):
    ...
    for x in range(chunks):
        ...
        if df_sliced.shape[0] >= number_of_rotated_keys:
            ...
            tracker_log(f'chunk_{x}_ts - {df_sliced["ts"].values.tolist()}', f'W-UD-1to5 - transform - ')
            tracker_log(f'chunk_{x}_ids - {df_sliced["ids"].values.tolist()}', f'W-UD-1to5 - transform - ')

            ids = [id for id in df_sliced['ids']]
            unique_ids = {x for l in ids for x in l}
            for unique_ids in unique_ids:
                results.get('ids').append(unique_ids)
            tracker_log(f'chunk_{x}_unique_ids - {unique_ids}', f'W-UD-1to5 - transform - ')
    ...
    # Deduplicate ids
    tracker_log(f'final_ids - {results.get("ids")}', f'W-UD-1to5 - transform - ')
    ...
    for hll_merge in records.get("merge"):
        if execute('EXISTS', hll_merge.get('name')) != 1:
            ...
            for id in records.get('ids'):
                k = create_key_name(Type.HLL.value, '', Dimension.WEBSITE.value, id, parsed_key.get('ts'), Metric.UNIQUE_DEVICES.value)
                keys = [create_key_name(Type.HLL.value, '', Dimension.WEBSITE.value, id, parse_key_name(k).get('ts'), Metric.UNIQUE_DEVICES.value) for k in hll_merge.get('keys')]
            ...
            # tracker_log(f'PFMERGE {hll_merge.get("name")} {hll_merge.get("keys")}', f'{job_name} - load - ')
            #
            ...
            #         tracker_log(f'PFCOUNT {hll_merge.get("keys")}', f'{job_name} - load - ')
            #         index_name = create_key_name(
            #             type=Type.INDEX.value,
            #             name='',
            #             dimension=dimension,
            ...
            #             # tracker_log(f'Write {unique_devices} devices to {timeseries_key_name} at {get_ts(parsed_key_name.get("ts"))}', f'{job_name} - ')
            #             tracker_log(
            #                 f'Write {unique_devices} devices to {timeseries_key_name} at {get_ts(parsed_key_name.get("ts"))}',
            #                 f'{job_name} - load - ')
            ...
            # tracker_log(
            #     f'XADD {output_stream_name} * ts {parsed_key_name.get("ts")} ids {json.dumps(hll_merge.get("ids"))}',
            #     f'{job_name} - load - ')
            #
