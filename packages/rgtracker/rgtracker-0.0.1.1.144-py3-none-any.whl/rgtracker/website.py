...
class Website:
    ...
    def notification_of_active_websites(self, stream_names):
        ...
        tracker_log(f'FT.SEARCH I::W:: @last_visited:[{prev_unix_ts} {self.last_visited} {current_ts}] RETURN 1 id\nids_raw {ids_raw}', 'Website - notification_of_active_websites - ')
        ...
        tracker_log(f'{prev_unix_ts} 9999999999999 active websites {ids}', f'Website - notification_of_active_websites - ')
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*',
                'ts', parsed_key.get("ts"),
                'status', 'active',
                'ids', json.dumps(ids))
