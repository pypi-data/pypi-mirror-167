...
class Website:
    ...
    def expire_metrics(self):
        expire_pg = execute('EXPIRE', self.metric_pageviews_key, 3600)
        tracker_log(f'Expire key {self.metric_pageviews_key} = {expire_pg}', f'Website - expire_metrics - ')

        expire_ud = execute('EXPIRE', self.metric_unique_device_key, 3600)
        tracker_log(f'Expire key {self.metric_unique_device_key} = {expire_ud}', f'Website - expire_metrics - ')
