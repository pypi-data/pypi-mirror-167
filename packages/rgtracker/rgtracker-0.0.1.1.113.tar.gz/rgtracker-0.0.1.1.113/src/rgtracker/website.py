...
class Website:
    ...
    def expire_metrics(self):
        ...
        tracker_log(f'{parsed_key}', 'Website - T - ')
        ...
        for d in ids_raw:
            if type(d) is list:
                ids.append(d[-1])
        tracker_log(f'{prev_unix_ts} {current_ts} - {ids}', 'Website - T - ')

        for id in ids:
            pg_key = create_key_name(Type.CMS.value, '', Dimension.WEBSITE.value, id, parsed_key.get('ts'), Metric.PAGEVIEWS.value)
            tracker_log(f'Expire {pg_key}', 'Website - T - ')
            expire_pg = execute('EXPIRE', pg_key, 3600)
            if expire_pg != 1:
                tracker_log(f'Expire key {pg_key} = {expire_pg}', f'Website - ', 'warning')
            ...
            if expire_ud != 1:
                tracker_log(f'Expire key {ud_key} = {expire_ud}', f'Website - ', 'warning')
