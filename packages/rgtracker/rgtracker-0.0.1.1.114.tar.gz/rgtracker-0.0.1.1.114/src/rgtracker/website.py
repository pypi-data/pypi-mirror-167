...
class Website:
    ...
    def create_metrics(self):
        ...
        try:
            ...
            # tracker_log(f'PFADD {self.metric_unique_device_key}', 'Website - ')
        except Exception as e:
            # tracker_log(f'{e} - {self.metric_pageviews_key}', 'Website - ')
            pass
    ...
    def expire_metrics(self):
        ...
        ttl_pageviews = execute('TLL', self.metric_pageviews_key)
        tracker_log(f'TLL {self.metric_pageviews_key} {ttl_pageviews}', 'Website - ')

        ttl_unique_devices = execute('TLL', self.metric_unique_device_key)
        tracker_log(f'TLL {self.metric_unique_device_key} {ttl_unique_devices}', 'Website - ')
        ...
        #
        #
        # pg_key = create_key_name(Type.CMS.value, '', Dimension.WEBSITE.value, '', parsed_key.get('ts'), Metric.PAGEVIEWS.value)
        #
        # expire_pg = execute('EXPIRE', pg_key, 3600)
        # tracker_log(f'Expire {pg_key} = {expire_pg}', 'Website - 2 - ')
        # # if expire_pg != 1:
        # #     tracker_log(f'Expire key {pg_key} = {expire_pg}', f'Website - Warning - ', 'warning')
        #
        # for id in ids:
        #     ud_key = create_key_name(Type.HLL.value, '', Dimension.WEBSITE.value, id, parsed_key.get('ts'), Metric.UNIQUE_DEVICES.value)
        #     expire_ud = execute('EXPIRE', ud_key, 3600)
        #     tracker_log(f'Expire {ud_key} = {expire_ud}', 'Website - 3 - ')
        #     # if expire_ud != 1:
