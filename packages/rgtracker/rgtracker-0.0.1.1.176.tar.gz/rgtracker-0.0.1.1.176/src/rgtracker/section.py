...
class Section:
    ...
    def notification_of_new_section(self, stream_names):
        ...
        tracker_log(f'{parsed_key.get("ts")} new section {self.id}', f'Section - notification_of_new_section - ')
    ...
    def notification_of_active_sections(self, stream_names):
        ...
        ids_raw = execute('FT.SEARCH', 'I::S:::', f'@last_visited:[{prev_unix_ts} {current_ts}]', 'RETURN', '1', 'id')
        ids = []
        for d in ids_raw:
            if type(d) is list:
                ids.append(d[-1])
        ...
        # FixMe: HOT FIX
        if len(ids) > 0:
            tracker_log(f'{prev_unix_ts} {current_ts} active sections {ids}', f'Section - notification_of_active_sections - ')
            execute('XADD', stream_names.get('section_pageviews'), 'MAXLEN', '864000', '*',
                    'ts', parsed_key.get("ts"),
                    'status', 'active',
                    'ids', json.dumps(ids))
            ...
            tracker_log(f'[{prev_unix_ts} {current_ts}] active sections {ids}', f'Section - notification_of_active_websites - warning - ')

    def is_weird(self):
        ts = datetime.now()
        current_ts = datetime.fromtimestamp(int(self.last_visited) / 1000)
        delta = (current_ts - ts).seconds
        if delta > 80:
            # tracker_log(f'WEIRDO!\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} VS {ts} => {delta} seconds at {parse_key_name(self.dimension_ts_key).get("ts")} {datetime.fromtimestamp(int(parse_key_name(self.dimension_ts_key).get("ts")) / 1000)}', f'Section - ')
            return True
        else:
            return False
    ...
    def log_new_weirdo(self, device_id):
        # tracker_log(f'WEIRDO!\t{self.id} {datetime.fromtimestamp(int(self.last_visited) / 1000)} {self.last_visited} at {parse_key_name(self.dimension_ts_key).get("ts")} {datetime.fromtimestamp(int(parse_key_name(self.dimension_ts_key).get("ts")) / 1000)}', f'Section - ')
        x = True
...
def load_section(website, section, page, device, output_stream_names):
    if section.is_exists() != 1:
        ...
        if section.is_weird():
            section.log_new_weirdo(device.id)
