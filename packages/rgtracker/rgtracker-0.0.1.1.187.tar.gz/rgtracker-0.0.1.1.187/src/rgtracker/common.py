...
def is_weird(last_visited_rounded):
    now = round_time()
    now_inf = now - timedelta(minutes=1)
    now_sup = now + timedelta(minutes=1)
    record_dt = datetime.fromtimestamp(last_visited_rounded / 1000)

    if record_dt >= now_sup or record_dt <= now_inf:
        tracker_log(f'{record_dt} between [{now_inf} {now} {now_sup}] ')
        return True
    else:
        tracker_log(f'{record_dt} is not between [{now_inf} {now} {now_sup}]')
        return False
