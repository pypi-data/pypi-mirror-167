...
class VayaDrive(object):
    ...
    def get_func(self, section, option):
        def getf(self):
            ...
            if 'SensorEnable' in option and self.configuration['Pipe-0']['SENSORS_BACKWARDS_Bool'].value == 'false':
    ...
    def _set_vaya_config(self):
        ...
        for sec in config.sections():
            ...
            for option in config[sec]:
                ...
                self.configuration[sec] = section
        if not 'SENSORS_BACKWARDS_Bool' in self.configuration['Pipe-0'].keys():
            self.set_explicit_param(section='Pipe-0', option='SENSORS_BACKWARDS_Bool', value='true')
    ...
    def enabled_cameras(self):
        if self.configuration['Pipe-0']['SENSORS_BACKWARDS_Bool'].value == 'true':
    ...
    def turn_off_all_algos(self, pipe: int):
        for key, val in self.configuration[f'Pipe-{pipe}'].items():
            if key.endswith('Bool') and not any(map(key.__contains__, ['Settings', 'SensorEnable', 'SENSORS__Table', 'SENSORS_BACKWARDS_Bool'])):
                val.value = 'false'
...
class VayaException(Exception):
    def __init__(self, *args):
        if args:
            ...
            self.msg = 'VayaException has been raised'
