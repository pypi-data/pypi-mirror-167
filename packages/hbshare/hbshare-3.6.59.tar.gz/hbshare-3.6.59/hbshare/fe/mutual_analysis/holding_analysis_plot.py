...
def fund_style_plot(date, pic_path):
    ...
    # for i, style in enumerate(fund_style2_latest.columns):
    #     ax.plot(fund_style2_latest.index, fund_style2_latest[style], color_list[i], label=style)
    # ax.legend(loc=2)
    # ax.set_ylim([0.0, 0.5])
    # ax.set_xticklabels(labels=fund_style2_latest.index, rotation=90)
    # ax.yaxis.set_major_formatter(FuncFormatter(to_100percent))
    # plt.title('细分风格统计')
    # plt.tight_layout()
    # plt.savefig('{0}{1}'.format(pic_path, '细分风格统计'))
