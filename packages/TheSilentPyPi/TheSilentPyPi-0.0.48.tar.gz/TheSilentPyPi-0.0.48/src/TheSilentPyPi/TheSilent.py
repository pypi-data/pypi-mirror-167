...
def packet_sniffer():
    ...
    while True:
        ...
        #protocol
        protocol = struct.unpack("! 1s", packet[0][13:14])
        super_protocol = binascii.hexlify(protocol[0])
        ...
        super_ip_header = struct.unpack("!12s4s4s", ip_header)

        #formatting
        destination = socket.inet_ntoa(super_ip_header[2])
        source = socket.inet_ntoa(super_ip_header[1])
