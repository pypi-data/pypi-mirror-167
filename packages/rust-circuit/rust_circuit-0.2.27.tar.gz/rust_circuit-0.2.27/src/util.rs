...
pub fn filter_out_idx<T: Clone>(col: &Vec<T>, idxs: &HashSet<usize>) -> Vec<T> {
    col.iter()
        .enumerate()
...
pub fn dedup_with_order<T: Hash + Eq + Clone>(col: &[T]) -> Vec<T> {
...
pub fn apply_fn_until_same<T: Eq + Hash + Clone, F>(x: &T, f: &mut F) -> T
where
    F: Fn(&T) -> T,
{
    let mut result = x.clone();
