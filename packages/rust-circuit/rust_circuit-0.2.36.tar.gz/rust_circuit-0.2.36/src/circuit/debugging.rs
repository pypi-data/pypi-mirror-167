...
pub fn randn_ablate_great_grandchildren(circ: CircuitRc) -> CircuitRc {
    ...
    for great_grandchild in great_grandchildren {
        let ablated = Circuit::ArrayConstant(ArrayConstant::randn_named_seeded(
            great_grandchild.info().shape.clone(),
            great_grandchild.name_cloned(),
            great_grandchild.info().hash_usize(),
        ))
        .rc();
        replacements.insert(great_grandchild.info().hash.clone(), ablated);
    }
...
pub fn create_circuit_test(
    ...
    let a_str = rust_expression_notation_circuit(a.clone());
    let b_str = rust_expression_notation_circuit(b.clone());
    let test = format!(
        "#[test]\
        \nfn {}() {{\
