...
use super::{
    apply_fn_cache, deep_map_unwrap, visit_circuit, ArrayConstant, Circuit, CircuitNode, CircuitRc,
    HashBytes, Index,
};
...
pub fn cast_circuit(circ: CircuitRc, device_dtype: &TorchDeviceDtype) -> CircuitRc {
    deep_map_unwrap(&circ, |circ| match circ {
        Circuit::ArrayConstant(node) => ArrayConstant::nrc(
            device_dtype.cast_tensor(node.value.clone()),
            node.name_cloned(),
        ),
        Circuit::Index(node) => Index::nrc(
            node.node.clone(),
            TensorIndex(
                node.index
                    .0
...
pub fn replace_all_randn_seeded(circuit: CircuitRc) -> CircuitRc {
    deep_map_unwrap(&circuit, |node| match node {
        Circuit::ArrayConstant(ac) => ArrayConstant::randn_named_seeded(
            ac.info().shape.clone(),
            ac.name_cloned(),
