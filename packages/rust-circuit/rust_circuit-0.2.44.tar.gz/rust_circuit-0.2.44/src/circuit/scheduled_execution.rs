...
use crate::{
    cached_lambda,
    circuit::circuit_optimizer::OptimizationContext,
...
use super::{
    ...
    visit_circuit, ArrayConstant, HashBytes, PyHashBytes, ScalarConstant, Symbol,
};
...
impl Dag {
    ...
    pub fn chains(&self) -> Vec<usize> {
        ...
        let node_to_top: Vec<usize> = (0..self.node_costs.len())
            .map(|node| top_of_chain(node))
            .collect();
        node_to_top
            .into_iter()
            .enumerate()
