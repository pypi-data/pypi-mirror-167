...
use circuit::algebraic_rewrite::{
    ...
    concat_merge_uniform, concat_pull_removable_axes, deep_optimize_einsums, distribute,
    distribute_all, einsum_elim_identity, einsum_elim_zero, einsum_flatten_once,
    einsum_merge_scalars, einsum_nest_optimize, einsum_nest_path, einsum_of_permute_merge,
    einsum_pull_removable_axes, generalfunction_pull_removable_axes, index_elim_identity,
    index_fuse, index_merge_scalar, index_split_axes, permute_of_einsum_merge, push_down_index,
...
use circuit::circuit_manipulation::{
    deep_map_preorder_py, deep_map_py, filter_nodes_py, path_get, replace_nodes, update_nodes_py,
    update_path_py,
};
use circuit::circuit_optimizer::{
    optimize_and_evaluate, optimize_and_evaluate_many, optimize_circuit,
};
use circuit::circuit_utils::{
    cast_circuit, count_nodes, toposort_circuit, total_arrayconstant_size, total_flops,
};
use circuit::compiler_heuristics::{deep_maybe_distribute_py, maybe_distribute};
...
use circuit::deep_rewrite::{
    compiler_simp_py, compiler_simp_step_py, compiler_simp_until_same, deep_heuristic_nest_adds,
    deep_pull_concat, deep_pull_concat_messy, deep_push_down_index,
};
...
fn rust_circuit(py: Python<'_>, m: &PyModule) -> PyResult<()> {
    use circuit::algebraic_rewrite::concat_repeat_to_rearrange;
    ...
    m.add_function(wrap_pyfunction!(maybe_distribute, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(replace_nodes, m)?)?;
