...
use super::{
    ...
    CachedCircuitInfo, Circuit, CircuitConstructionError, CircuitNode, CircuitNodeAutoName,
    CircuitRc, PyCircuitBase, ScalarConstant, Shape,
};
use crate::{
...
impl CircuitNode for Einsum {
        ...
        F: FnMut(&'a Circuit) -> Result<CircuitRc, E>,
    {
        Self::try_new(
            self.args
                .iter()
                .map(move |(circ, axes)| -> Result<_, CircuitConstructionError> {
                    Ok((f(circ)?, axes.clone()))
                })
                .collect::<Result<_, _>>()?,
            self.out_axes.clone(),
            self.name.clone(),
    ...
    fn eval_tensors(
        ...
        Python::with_gil(|py| {
            ...
            let out_axes_deduped = dedup_with_order(&self.out_axes);
...
impl Einsum {
    ...
    pub fn scalar_mul(node: &CircuitRc, scalar: f64, name: Option<String>) -> Self {
        ...
        let axes: Vec<u8> = (0..node.info().rank()).map(|x| x as u8).collect();
        Einsum::try_new(
            vec![
                (node.clone(), axes.clone()),
                (ScalarConstant::new(scalar, vec![], None).rc(), vec![]),
            ],
            axes,
            Some(name_definite),
        )
...
impl Einsum {
    ...
    #[staticmethod]
    pub fn new_diag(node: CircuitRc, ints: Vec<EinInt>, name: Option<String>) -> Self {
        let deduped = dedup_with_order(&ints);
        Einsum::try_new(vec![(node, deduped)], ints, name).unwrap()
    }

    #[staticmethod]
    pub fn new_trace(node: CircuitRc, ints: Vec<EinInt>, name: Option<String>) -> Self {
        let deduped = dedup_with_order(&ints);
    ...
    fn out_axes(&self) -> PyEinsumAxes {
        PyEinsumAxes(self.out_axes.to_vec())
    }
...
impl CircuitNode for Rearrange {
    ...
    where
        ...
        F: FnMut(&'a Circuit) -> Result<CircuitRc, E>,
    {
        Self::try_new(f(&self.node)?, self.spec.clone(), self.name.clone())
    }
...
impl CircuitNode for Index {
    ...
    where
        ...
        F: FnMut(&'a Circuit) -> Result<CircuitRc, E>,
    {
        Self::try_new(f(&self.node)?, self.index.clone(), self.name.clone())
    }
    ...
    // commented this out bc cast_circuit is broken with this bc deep_map assumes you can construct
    // original with new children
    //
