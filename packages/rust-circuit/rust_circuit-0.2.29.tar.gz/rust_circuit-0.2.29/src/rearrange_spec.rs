...
pub struct RearrangeSpec {
    #[pyo3(get)]
    pub input_ints: Vec<Vec<EinInt>>,
    #[pyo3(get)]
    pub output_ints: Vec<Vec<EinInt>>,
    #[pyo3(get)]
...
impl RearrangeSpec {
    #[new]
    pub fn new(
        input_ints: Vec<Vec<EinInt>>,
        output_ints: Vec<Vec<EinInt>>,
        int_sizes: OpShape,
    ) -> Self {
        // TODO: validation
        RearrangeSpec {
    ...
    pub fn flatten(rank: usize) -> Self {
        ...
        RearrangeSpec {
            input_ints: (0..rank).map(|i| vec![i as u8]).collect(),
            output_ints: vec![(0..rank as u8).collect()],
    ...
    pub fn unflatten(shape: Shape) -> Self {
        ...
        assert!(rank <= u8::MAX as usize + 1);
        RearrangeSpec {
            input_ints: vec![(0..rank as u8).collect()],
            output_ints: (0..rank).map(|i| vec![i as u8]).collect(),
    ...
    pub fn ints_in_inp(&self) -> HashSet<EinInt> {
        self.input_ints.iter().flatten().copied().collect()
    }
    pub fn ints_in_out(&self) -> HashSet<EinInt> {
        self.output_ints.iter().flatten().copied().collect()
    }
    ...
    pub fn is_valid(&self) -> bool {
        ...
        let result = is_unique(&self.input_ints.iter().flatten().collect::<Vec<&EinInt>>()[..])
            && is_unique(&self.input_ints.iter().flatten().collect::<Vec<&EinInt>>()[..]);
    ...
    pub fn shapes(&self) -> Result<(Vec<usize>, Vec<usize>), TensorInvariantError> {
    ...
    pub fn canonicalize(&self, special_case_ones: bool) -> Self {
        ...
        if special_case_ones {
            let drop_einints = |ints_list: Vec<Vec<u8>>| {
                ints_list
                    .iter()
                    .map(|ints| {
                        ints.iter()
        ...
        let mut merge_tuples_and_renumber_fn = |ints: Vec<Vec<EinInt>>| {
            ints.iter()
                .map(|single_dim| {
        ...
        let input_ints: Vec<Vec<EinInt>> = merge_tuples_and_renumber_fn(input_ints);
        let output_ints: Vec<Vec<EinInt>> = merge_tuples_and_renumber_fn(output_ints);
    ...
    pub fn fill_empty_ints(
        ...
        let mut output_ints: Vec<Vec<u8>> = self
            .output_ints
            .iter()
            .map(|ints| {
                if ints.is_empty() {
...
impl RearrangeSpec {
    ...
    pub fn unremove_axes(removed_axes: &HashSet<usize>, output_shape: &Shape) -> RearrangeSpec {
        RearrangeSpec {
            input_ints: (0..output_shape.len())
                ...
                .map(|i| vec![i as u8])
                .collect(),
            output_ints: (0..output_shape.len()).map(|i| vec![i as u8]).collect(),
    ...
    pub fn expand(
        ...
        RearrangeSpec {
            input_ints: input_ints.iter().map(|i| vec![*i]).collect(),
            output_ints: output_ints.iter().map(|i| vec![*i]).collect(),
            int_sizes: shape_to_op_shape(&dict_to_list(&int_sizes_usize, None)),
        }
    }
    ...
    ///     vec![vec![0], vec![1]],
    ///     vec![vec![0, 1]],
    ///     vec![OpSize(2), OpSize(3)],
    /// );
    /// let outer = RearrangeSpec::new(
    ///     vec![vec![0]],
    ///     vec![vec![0], vec![1]],
    ///     vec![OpSize(6), OpSize(4)],
    /// );
    /// let fused = RearrangeSpec::new(
    ///     vec![vec![0], vec![1]],
    ///     vec![vec![0, 1], vec![2]],
    ///     vec![OpSize(2), OpSize(3), OpSize(4)],
    /// );
    ...
    pub fn fuse(inner: &Self, outer: &Self) -> Result<Self, RewriteError> {
        ...
        let input_ints: Vec<Vec<EinInt>> = expand_dims(&inner.input_ints, &mut inner_replaces);
        let output_ints: Vec<Vec<EinInt>> = expand_dims(&outer.output_ints, &mut outer_replaces);
        ...
        let input_ints: Vec<Vec<EinInt>> = input_ints
            .iter()
            .map(|dim_ints| {
                dim_ints
                    .iter()
