...
use crate::{pyo3_prelude::*, util::BitMask64};

use super::{
    computational_node::EinsumArgs, deep_map_unwrap, evaluate_fn, Add, Circuit,
    CircuitConstructionError, CircuitNode, CircuitRc, Concat, Einsum, GeneralFunction, HashBytes,
    Index, Rearrange, ScalarConstant, Scatter, Symbol,
};
use crate::{
    filter_by_variant,
        ...
        let outer_indices = {
            if stack.is_empty() {
                ...
                let inner_indices_set: HashSet<EinInt> = args_here
                    ...
                    .copied()
                    .collect();
                let mut outer_indices =
                    Vec::from_iter(inner_indices_set.intersection(&outer_indices_set).copied());
                outer_indices.sort();
                outer_indices
            }
        };
...
pub fn expand_maybe_diag(
    ...
    let out_int_appearances = unique_to_appearance(out_ints);
    let ints_to_add_before_einsum: Vec<u8> = out_int_appearances
        .iter()
    ...
    // make diag
    let einsum_out_ints: Vec<u8> = out_ints
        .iter()
        .filter(|i| cur_ints.contains(i))
        .cloned()
        .collect();
    result = Einsum::try_new(vec![(result, cur_ints)], einsum_out_ints.clone(), None)
        .unwrap()
        .rc();
    ...
    let input_ints = node
        ...
        .map(|(i, x)| {
            if *x == 1 {
                ...
                vec![(i + rank_dif) as u8]
            }
        })
        .collect();
    let output_ints: Vec<Vec<u8>> = (0..(out_shape.len())).map(|i| vec![i as u8]).collect();
...
pub fn einsum_elim_identity(einsum: &Einsum) -> Option<CircuitRc> {
        ...
        && is_unique(&einsum.out_axes)
    {
        ...
        Some(ScalarConstant::new(1.0, vec![], None).rc())
    } else {
        None
    }
}
...
pub fn einsum_pull_removable_axes(einsum: &Einsum) -> Option<CircuitRc> {
        ...
        .map(|(node, ints)| {
            ...
            if !removable_axes.is_empty() {
                ...
                (
                    ...
                    filter_out_idx(ints, &removable_axes),
                )
            } else {
    ...
    let scalar_mul: f64 = reduced_axes
        ...
        .product();
    if scalar_mul != 1.0 {
        new_args.push((ScalarConstant::new(scalar_mul, vec![], None).rc(), vec![]));
    }

    let new_out_ints: Vec<u8> = einsum
        .out_axes
        .iter()
        .filter(|i| ints_in_new_args.contains(i))
        .copied()
        .collect();
    let new_einsum = Einsum::try_new(new_args, new_out_ints.clone(), einsum.name_cloned())
        .unwrap()
        .rc();
...
pub fn add_pull_removable_axes(add: &Add, remove_non_common_axes: bool) -> Option<CircuitRc> {
    ...
    let post_rearrange_spec = RearrangeSpec::new_canon(
        (0..add.info().rank())
            ...
            .map(|x| vec![x as u8])
            .collect(),
        (0..add.info().rank()).map(|x| vec![x as u8]).collect(),
    ...
    let new_operands = zip(&add.nodes, removable_axes)
        .map(|(node, removable_here_base)| {
            ...
            if !remove_non_common_axes {
                ...
                Rearrange::try_new(raw_removed, broadcast_rspec, None)
                    .unwrap()
                    .rc()
            }
        })
        .collect();
    let add_circuit = Add::try_new(new_operands, add.name_cloned()).unwrap().rc();
    Some(
        Rearrange::try_new(add_circuit, post_rearrange_spec, None)
            .unwrap()
            .rc(),
    )
}
    ...
    einsum.args[operand_idx].0.as_add().and_then(|add_node| {
        ...
        let summands = add
            ...
            .map(|node| {
                ...
                Einsum::try_new(new_args, einsum.out_axes.clone(), None)
                    .unwrap()
                    .rc()
            })
            .collect();
...
pub fn einsum_merge_scalars(einsum: &Einsum) -> Option<Einsum> {
    ...
    let mut new_args: Vec<(CircuitRc, Vec<u8>)> = einsum
        .args
        .iter()
    ...
    match &**node.node {
            ...
            let new_operands = inner
                ...
                .map(|(operand, rank_difference)| {
                    let index_here = (0..operand.info().rank())
                        ...
                        .collect();
                    Index::try_new(operand.clone(), TensorIndex(index_here), None)
                        .unwrap()
                        .rc()
                })
                .collect();
            Some(
                Add::try_new(new_operands, inner.name_cloned())
                    .unwrap()
                    .rc(),
            )
        }
        Circuit::Concat(inner) => {
            ...
            let new_concat = Concat::try_new(
                inner
                    ...
                    .iter()
                    .map(|operand| {
                        Index::try_new(operand.clone(), index_non_axis.clone(), None)
                            .unwrap()
                            .rc()
                    })
                    .collect(),
            ...
            Some(Index::try_new(new_concat, final_index, None).unwrap().rc())
        }
        Circuit::GeneralFunction(inner) => {
                ...
                || inner.spec.num_non_batchable_output_dims == inner.info().rank()
            {
                ...
                let new_operands = zip(&inner.nodes, &inner.spec.input_batchability)
                    .map(|(child, batchable)| {
                        if *batchable {
                            ...
                            Index::try_new(child.clone(), index_passed, None)
                                .unwrap()
                                .rc()
                        } else {
                            child.clone()
                    ...
                    .collect();
                let new_gf =
                    GeneralFunction::try_new(new_operands, inner.spec.clone(), inner.name_cloned())
                        .unwrap()
                        .rc();
                ...
                Some(Index::try_new(new_gf, index_top, None).unwrap().rc())
            }
        }
        Circuit::Einsum(inner) => {
            ...
            let ints_filter_index = |ints: &Vec<EinInt>| {
                ints.iter()
                    .filter_map(|i| {
                        match int_indices
                            .get(i)
            ...
            let new_operands: Vec<(CircuitRc, Vec<EinInt>)> = inner
                .args
                .iter()
                .map(|(node, ints)| {
                    ...
                    (
                        Index::try_new(node.clone(), index_here, None).unwrap().rc(),
                        ints_filter_index(ints),
                    )
                })
                .collect();
            ...
            let new_einsum =
                Einsum::try_new(new_operands, ints_filter_index(&inner.out_axes), None)
                    .unwrap()
                    .rc();
            ...
            Some(Index::try_new(new_einsum, out_index, None).unwrap().rc())
        }
        Circuit::Rearrange(inner) => {
            ...
            python_result.map(|(index, spec)| {
                let new_index = Index::try_new(inner.node.clone(), index, node.name_cloned())
                    .unwrap()
                    .rc();
                Rearrange::try_new(new_index, spec, inner.name_cloned())
                    .unwrap()
                    .rc()
            })
        }
        Circuit::Scatter(inner) => {
            ...
            let inner_node = Index::try_new(inner.node.clone(), TensorIndex(passed_index), None)
                .unwrap()
                .rc();
            let result = Some(
                Index::try_new(
            ...
            if same_seen > 1 {
                ...
                new_nodes.push(
                    Rearrange::try_new(
                        prev_node.clone(),
                        RearrangeSpec {
                            input_ints: (0..prev_node.info().rank())
                                .map(|x| vec![x as u8])
                                .collect(),
                            output_ints: (0..prev_node.info().rank())
                                .map(|x| {
                                    if x == concat.axis {
                                        ...
                                        // count * axis_size()
                                        vec![prev_node.info().rank() as u8, x as u8]
                                    } else {
                                        vec![x as u8]
                                    }
                                })
...
fn basic_check_same_concat_shape() {
    let a = Symbol::new_with_random_uuid(vec![2, 3, 4, 5, 6], None).rc();
...
#[inline]
pub fn is_not_outer(a: BitMask64, b: BitMask64) -> bool {
    let union = a & b;
    union == a || union == b
}
...
pub fn bitmask_outer_product_sets(bitmasks: &Vec<BitMask64>) -> Vec<BitMask64> {
    let mut result: Vec<BitMask64> = vec![];
    let mut result_axes: Vec<BitMask64> = vec![];
...
pub fn add_outer_product_broadcasts_on_top(add: &Add) -> Option<Add> {
    let axis_sets: Vec<BitMask64> = add
        .nodes_and_rank_differences()
        .iter()
        .map(|(child, rank_dif)| {
            let mut bitmask: BitMask64 = 0;
    ...
    let new_operands = outer_product_sets
        ...
        .collect();
    return Some(Add::try_new(new_operands, None).unwrap());
}
