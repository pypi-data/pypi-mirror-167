...
class AugmentQueries(object):
    ...
    __AUGMENT_TO_CACHE_TABLE = {
        PER_CYCLE_TABLE: 'neware_cache',
        PER_PULSE_TABLE: 'neware_pulses_cache',
        PER_ANODE_TABLE: 'neware_cache_anode',
    }

    __REQUIRED_COLUMNS_PER_TABLE = {
        PER_CYCLE_TABLE: ['ip', 'dev_uid', 'unit_id', 'chl_id', 'test_id', 'cycle'],
        PER_PULSE_TABLE: ['ip', 'dev_uid', 'unit_id', 'chl_id', 'test_id', 'cycle'],
        PER_ANODE_TABLE: ['ip', 'dev_uid', 'unit_id', 'chl_id', 'test_id', 'cycle'],
    ...
    def set_features(self, table: str, features: pd.DataFrame, columns: list, validate=True): # TABLE, CELL, TEST_ID, CYCLE 
        ...
        _columns = ['cell_table', 'cell_id', 'test_id', 'cycle'] + columns
        ...
        try:
            ...
            if table != AugmentQueries.PER_CELL_TABLE: # Tests/Cycles/Pulses/Anode
                ...
                if table != AugmentQueries.PER_TEST_TABLE: # Cycles/Pulses/Anode
    ...
    def __create_and_get_rows_per_cycle_pulse_or_anode(self, data: pd.DataFrame, table: str):
        ...
        def __create_augm_per_pk(group):
            ...
            cache_table = AugmentQueries.__AUGMENT_TO_CACHE_TABLE[table]
            cache_columns = f'{cache_table}.ip', f'{cache_table}.dev_uid', f'{cache_table}.unit_id', f'{cache_table}.chl_id', f'{cache_table}.test_id', f'{cache_table}.cycle'
            ...
            try:
                data = Queries.table_query(
                    ...
                    cache_table, columns=AugmentQueries.__REQUIRED_COLUMNS_PER_TABLE[table]
                )
                
                joined_group = group.merge(data, how='inner', left_on=['augm_per_test_id', 'cycle'], right_on=[f'{test_table}.id', f'{cache_table}.cycle'])
                joined_group.drop(['ip', 'dev_uid', 'unit_id', 'chl_id', 'test_id', 'cycle'], inplace=True, axis=1, errors='ignore')
            ...
            values = [
                str(tpl) for tpl in joined_group[
                    ['augm_per_test', *cache_columns]
            ...
            return joined_group.merge(
                _data, how='inner', 
                left_on=cache_columns,
                right_on=AugmentQueries.__REQUIRED_COLUMNS_PER_TABLE[table]
            )
