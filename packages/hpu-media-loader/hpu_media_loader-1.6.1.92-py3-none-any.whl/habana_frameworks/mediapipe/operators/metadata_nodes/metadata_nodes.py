...
class ssd_metadata_processor(CPPNode):
    ...
    def gen_output_info(self):
        ...
        """
        #Crop windows
        windows = opnode_tensor_info(dt.FLOAT32, np.array([4, self.batch_size], dtype=np.uint32), "")

        #Image ids
        ids = opnode_tensor_info(dt.UINT32, np.array([self.batch_size], dtype=np.uint32), "")

        #Image sizes
        sizes = opnode_tensor_info(dt.UINT32, np.array([2, self.batch_size], dtype=np.uint32), "")

        #Image bounding boxes
        bboxes = opnode_tensor_info(dt.FLOAT32, np.array([ 4, 8732, self.batch_size], dtype=np.uint32), "")

        #Image labels
        labels = opnode_tensor_info(dt.UINT32, np.array([8732, self.batch_size], dtype=np.uint32), "")

        #Lengths
        lengths = opnode_tensor_info(dt.UINT32, np.array([self.batch_size], dtype=np.uint32), "")
    ...
    def __call__(self, *argv):
        ...
        windows = np.empty([self.batch_size, 4], dtype = np.float32)
