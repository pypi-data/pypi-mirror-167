...
def client_app_data(
    out_file_path: Optional[Path] = None, dry_run: bool = False
) -> dict:
    """
    ...
    # ======================================================================================================================
    global_columns = orjson.loads(
        pkgutil.get_data(__name__, "assets/global_columns.json")
    )

    for model in neuro_data_models:
        ...
        for name, column in model._columns.items():
            ...
            if name not in column_to_type:
                column_to_type[name] = CASSANDRA_TYPES_TO_SYNDB_TYPE_NAME[
                    column.db_type
                ]
            if column.db_type in COLLECTION_COLUMN_TYPES_TO_DART:
                collection_column_typings[name] = COLLECTION_COLUMN_TYPES_TO_DART[
                    column.db_type
                ]
            ...
            if not (
                name in global_columns["all"]
                or model_name in global_columns
                and name in global_columns[model_name]
            ):
    ...
    for (
        ...
        for second_partition_field in second_partition_fields:
            ...
            model_name_to_partition_keys_to_mv_name[model_name][
                ...
                model_name, composite_key
            )

    model_name_to_partition_keys_to_mv_name["Mesh"] = {
        "object_id": materialized_view_name_from_table_name_and_partition_key(
            Mesh.__table_name__, "object_id"
        )
    }
    ...
    brain_tree = orjson.loads(
        pkgutil.get_data(__name__, "assets/neurometa/human_brain_tree.json")
    )
    model_organisms = orjson.loads(
        pkgutil.get_data(__name__, "assets/neurometa/model_organism.json")
    )
    neurotransmitters = orjson.loads(
        pkgutil.get_data(__name__, "assets/neurometa/neurotransmitters.json")
    )
    ...
    result = {
        ...
        "genericColumnToChoices": {
            **neuron_types,
            "first_parent_table_name": structure_model_names,
            "brain_structure": brain_tree["neuronal_structure_flat"],
            "brain_structure_lower": [
                s.lower() for s in brain_tree["neuronal_structure_flat"]
            ],
        },
