...
class DataVault:
    ...
    def link(data_df, config, conn):
        ...
        with conn.cursor() as cur:
            ...
            if src_nk:
                l_nk, r_nk = src_nk[0], src_nk[1]
                data_df[l_fk] = data_df[l_nk].apply(hash_col)
                data_df[r_fk] = data_df[r_nk].apply(hash_col)

            data_df['link_pk'] = data_df[src_fk].apply(hash_cols, axis=1)
            args_str = ','.join("(%s, " % row['link_pk'] + cur.mogrify(
                "%s, %s, %s, now())", (row[l_fk], row[r_fk], row['record_source'])
            ).decode('utf-8')
                                for i, row in data_df.iterrows())
            query = "insert into %s (%s) VALUES " % (
                destination, ','.join([src_pk, l_fk, r_fk, 'record_source', 'load_date'])) + args_str
            cur.execute(query)
