...
def check_config_file(kwargs: dict) -> str:
    ...
    if "config_file_path" not in kwargs or (
        ...
        config_file_path = os.path.join(data_dir, "WLP_NPK.conf")
    else:
...
class Wofost:
    ...
    def init_wofost(latitude: float = 51.97, longitude: float = 5.67, **kwargs):
        ...
        data_zip = dict(zip(cabo_files, data_))
        ...
        params = ParameterProvider(data_zip["crop"], data_zip["soil"], data_zip["site"])
