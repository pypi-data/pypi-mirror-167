...
def check_cabo_file(param_name : str, kwargs : dict) -> CABOFileReader:
    ...
    if f"{param_name}_file_path" not in kwargs or (f"{param_name}_file_path" in kwargs and not check_file_path(kwargs[f"{param_name}_file_path"])):
...
def check_config_file(kwargs : dict) -> str:
    ...
    if "config_file_path" not in kwargs or (f"config_file_path" in kwargs and not check_file_path(kwargs["config_file_path"])):
...
class Wofost:
    ...
    def init_wofost(latitude: float = 51.97, longitude: float = 5.67, **kwargs):
        ...
        for file in cabo_files:
            data_[file] = check_cabo_file(file, kwargs)
        ...
        params = ParameterProvider(data_zip["crop"], data_zip["soil"], data_zip["site"], config)
