...
class LoadData:
    ...
    def write_data(self, data_list, cache=True, base_time=0):
        ...
        df = df[(df['timestamp'] >= self.unix_start-base_time) & (df['timestamp'] <= self.unix_end-base_time)]
        ...
        for _ in range(10000):
            ...
            try:
                ...
                result = [{
                    'symbol': trade['symbol'],
                    'id': trade['id'].replace('\n', ''),
                    'timestamp': int(trade['timestamp'])-self.unix_start,
                    'side': trade['side'][0],
                    'price': trade['price'],
                    'amount': trade['amount'],
                } for trade in trades]

                self.write_data(result, base_time=self.unix_start)
                #time.sleep(int(self.exchange.rateLimit / 1000))
