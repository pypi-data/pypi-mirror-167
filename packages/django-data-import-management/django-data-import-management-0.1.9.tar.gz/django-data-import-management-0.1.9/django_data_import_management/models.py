...
class DjangoSimpleExportedDataImportWorkflow(DjangoDataImportWorkflow):
    ...
    def check_manytomanyfield_change(self, obj, data, field_name, field_settings):
        related_keyfield= field_settings["keyfield"]
        data_m2m_keys = sorted(self.get_manytomanyfield_values(data[field_name], field_name, field_settings))
        obj_m2m_keys = sorted([str(getattr(x, related_keyfield)) for x in getattr(obj, field_name).all()])
    ...
    def update_manytomanyfield(self, obj, data, field_name, field_settings):
        ...
        data_relates = {}
        for keyword in self.get_manytomanyfield_values(data[field_name], field_name, field_settings):
        ...
        if created_keys:
            getattr(obj, field_name).add(*[related_obj_mapping[x] for x in created_keys])

        if deleted_keys:
            getattr(obj, field_name).remove(*[related_obj_mapping[x] for x in deleted_keys])
    ...
    def do_parse(self):
        load_rows = "{0}-".format(self.start_row)
        try:
            if self.copy_datafile or (self.datafile is None):
                temp_datafile = fsutils.TemporaryFile(content=self.datafile_field.read(), filename_suffix=".xlsx")
            ...
            raise RuntimeError(_("Load data from excel file failed: {message}").format(message=str(error)))
        ...
        for row in rows:
            ...
            try:
                ...
                if keyword in obj_mapping:
                    ...
                    changed, changed_fields = dictutils.changes(obj, data, keys=compare_field_names, return_changed_keys=True, do_update=False, ignore_empty_value=True)

                    for field_name, field_settings in self.foreignkey_settings.items():
                        foreignkey_changed = self.check_foreignkey_change(obj, data, field_name, field_settings)
                        if foreignkey_changed:
                            changed = True
                            changed_fields.append(field_name)

                    for field_name, field_settings in self.manytomanyfield_settings.items():
                        manytomanyfield_changed = self.check_manytomanyfield_change(obj, data, field_name, field_settings)
                        if manytomanyfield_changed:
                            changed = True
                            changed_fields.append(field_name)

                    if changed:
                        info = _("Parse line of {verbose_name} success, keyword={keyword},  changed_fields={changed_fields}".format(verbose_name=self.model._meta.verbose_name, keyword=keyword, changed_fields=changed_fields))
                    else:
                        info = _("Parse line of {verbose_name} success without any change, keyword={keyword}".format(verbose_name=self.model._meta.verbose_name, keyword=keyword))
                    item.mark_success(info, data)
                    items.append(item)
                    continue
                else:
                    info = _("Parse line of {verbose_name} success, create a new object with keyword={keyword}".format(verbose_name=self.model._meta.verbose_name, keyword=keyword))
                    item.mark_success(info, data)
                    items.append(item)
                    continue
            except Exception as error:
                info = _("Parse line of {verbose_name} failed: error_message={error_message}".format(verbose_name=self.model._meta.verbose_name, error_message=str(error)))
    ...
    def do_import(self, imported_items):
        ...
        for item in imported_items:
            try:
                ...
                changed, changed_fields = dictutils.changes(obj, item.data, keys=compare_field_names, return_changed_keys=True, do_update=True, ignore_empty_value=True)

                if changed or create_flag:
                    obj.save()
                
                for field_name, field_settings in self.manytomanyfield_settings.items():
                    manytomanyfield_changed = self.update_manytomanyfield(obj, item.data, field_name, field_settings)
                    if manytomanyfield_changed:
                        changed = True
                        changed_fields.append(field_name)

                if changed:
                    info = _("Import {verbose_name} item success, keyword={keyword}, changed_fields={changed_fields}".format(verbose_name=self.model._meta.verbose_name, keyword=keyword, changed_fields=changed_fields))
                else:
                    info = _("Import {verbose_name} item success without any change, keyword={keyword}".format(verbose_name=self.model._meta.verbose_name, keyword=keyword))
                item.mark_success(info)
                continue
            except Exception as error:
                info = _("import {verbose_name} item failed, error_message={error_message}").format(verbose_name=self.model._meta.verbose_name, error_message=str(error))
                logger.exception(info)
...
def get_django_data_import_workflow_choices():
    ...
    for code, info in DJANGO_DATA_IMPORT_WORKFLOWS.items():
