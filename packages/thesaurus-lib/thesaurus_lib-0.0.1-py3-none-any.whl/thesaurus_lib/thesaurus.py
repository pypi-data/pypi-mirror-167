...
class Thesaurus:
    ...
    def get_stopwords(path_):
        stopwords_file = pkg_resources.read_text("thesaurus.data.stopwords", path_)
        stopwords = []
    ...
    def set_som(self, local_som_file=None):
        if local_som_file is None:
            som = pkg_resources.read_binary("thesaurus.data.pretrained_models", self.som_file)
    ...
    def plot_bokeh(self, background_embeds, background_words, foreground_names, preprocessed_foregrounds,
        ...
        if os.path.isfile(index_files[self.lang]) or self.external_background is False:
            if self.external_background is False:
                index = pkg_resources.read_binary("thesaurus.data.index_files", index_files[self.lang])
    ...
    def import_background(self, b_tokens=None, b_embeds=None):
        ...
        if b_tokens is None and b_embeds is None:
            background_words = pkg_resources.read_binary("thesaurus.data.back_tokens", back_tokens[self.lang])
            background_embeds = pkg_resources.read_binary("thesaurus.data.back_embeds", back_embeds[self.lang])
