...
from .tclab_cae import TCLab_CAE, TCLabModel
...
class SimpleInteraction(NotebookInteraction):
    ...
    def __init__(self):
        ...
        self.T3widget, T3box = labelledvalue('T3:', 0, '°C')

        temperatures = VBox([T1box, T2box, T3box])

        # Current display
        self.I1widget, I1box = labelledvalue('I1:', 0, 'mA')
        self.I2widget, I2box = labelledvalue('I2:', 0, 'mA')

        currents = VBox([I1box, I2box])

        self.ui = HBox([heaters, temperatures, currents])

    def update(self, t):
        self.T1widget.value = '{:2.1f}'.format(self.lab.T1)
        self.T2widget.value = '{:2.1f}'.format(self.lab.T2)
        self.T3widget.value = '{:2.1f}'.format(self.lab.T3)
        self.I1widget.value = '{:2.4f}'.format(self.lab.I1)
        self.I2widget.value = '{:2.4f}'.format(self.lab.I2)
