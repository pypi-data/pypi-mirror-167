...
class TCLab_CAE(object):
    ...
    @property
    def T3(self):
        """Return a float denoting TCLab temperature T3 in degrees C."""
        return self.send_and_receive('T3', float)

    @property
    def I1(self):
        """Return a float denoting TCLab CURRENT I1 in mA."""
        return self.send_and_receive('I1', float)

    @property
    def I2(self):
        """Return a float denoting TCLab CURRENT I2 in mA."""
        return self.send_and_receive('I2', float)
    ...
    def scan(self):
        ...
        T3 = self.T3  # float(self.receive())
        I1 = self.I1  # float(self.receive())
        I2 = self.I2  # float(self.receive())
        Q1 = self.Q1()  # float(self.receive())
        Q2 = self.Q2()  # float(self.receive())
        return T1, T2, T3, I1, I2, Q1, Q2
...
class TCLabModel(object):
    def __init__(self, port='', debug=False, synced=True):
        ...
        self._T3 = self.Ta            # temperature thermister 3
        self._I1 = self.Ta            # current heater 1
        self._I2 = self.Ta            # current heater 2
    ...
    def T3(self):
        """Return a float denoting TCLab temperature T2 in degrees C."""
        self.update()
        return self.measurement(self._T3)

    @property
    def I1(self):
        """Return a float denoting TCLab current I1 in mA."""
        self.update()
        return self.measurement(self._I1)

    @property
    def I2(self):
        """Return a float denoting TCLab current I2 in mA."""
        self.update()
        return self.measurement(self._I2)
    ...
    def scan(self):
        ...
        return (self.measurement(self._T1),
                ...
                self.measurement(self._T3),
                self.measurement(self._I1),
                self.measurement(self._I2),
                self._Q1,
                self._Q2)
...
def diagnose(port=''):
    ...
    with TCLab_CAE(port=port, debug=True) as lab:
        print('Reading temperature')
        print(lab.T1)

    heading('Testing TCLab functions')

    with TCLab_CAE(port=port) as lab:
        ...
        T2 = lab.T2
        T3 = lab.T3
        print('T1 = {} °C, T2 = {} °C, T3 = {} °C'.format(T1, T2, T3))
