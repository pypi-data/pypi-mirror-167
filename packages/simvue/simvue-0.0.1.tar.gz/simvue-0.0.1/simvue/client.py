...
class Simvue(object):
    ...
    def _send_metrics(self):
        if self._data:
            self._executor.submit(post, '%s/api/metrics' % self._url, self._headers, self._data.copy())
            self._data = []

    def _send_events(self):
        if self._events:
            self._executor.submit(post, '%s/api/events' % self._url, self._headers, self._events.copy())
            self._events = []
