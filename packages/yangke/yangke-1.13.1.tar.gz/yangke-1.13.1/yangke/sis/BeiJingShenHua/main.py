...
def optimize():
    ...
    _debug1 = pressure_funcs[state_max](xi)
    delta[state_max]["背压"] = _debug1
    ...
    last_state.update({"凝汽器热负荷": Q_c, "t0": t0, "humid": humid})
    logger.debug(result)
    logger.debug({"理论计算推荐背压": _debug1, "当前背压": pres_act, "当前运行方式": cur_state})
