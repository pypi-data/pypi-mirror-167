...
import shutil
from yangke.objDetect.models import Darknet
import cv2
import time
import numpy as np
...
def xyxy2xywh(x):
    """
    将 xyxy 形式的矩形框表达式转换为 xywh 形式，不进行归一化处理

    :param x: 形状为(:,4)的数组，4列分别对应矩形框的 左上角x坐标，左上角y坐标，右下角x坐标，右下角y坐标
    :return: 形状为(:,4)的数组，4列分别对应矩形框的 x_center，y_center, 宽, 高
    """
...
def detect(save_img=False):
    ...
    # Initialize model
    model: Darknet = Darknet(opt.cfg, img_size)
    ...
    if os.path.exists(weights):
        ...
        print("模型文件不存在，请检查！ {}".format(os.path.abspath(weights)))
        exit(0)

    # Second-stage classifier
    classify = False
    if classify:
        modelc = load_classifier(name='resnet34', n=2)  # initialize
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='D:\\Users\\pycharm\\face\\yolov3-tiny.pt', help='weights path')
    parser.add_argument('--source', type=str, default=r'D:\Users\pycharm\face\image',
