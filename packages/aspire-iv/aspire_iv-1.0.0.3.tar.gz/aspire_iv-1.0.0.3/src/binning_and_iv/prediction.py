...
def DT_binning(dataframe, max_depth_=3,min_samples_split_=40, min_samples_leaf_=30,max_leaf_nodes_ = 6, num_feature= [],cat_feature=[]):
    ...
    if num_feature != []:
        df_binning, df_bins, feature, feature_bin = binning(df_binning,df_bins, num_feature, max_depth_,min_samples_split_, min_samples_leaf_,max_leaf_nodes_)
        cal_bad_num = num_bucket(df_bins, feature, feature_bin)
        cal_bad_num = cal_bad_num.set_index(['feature','bin_val'])
    else:
        cal_bad_num = np.nan
    if cat_feature != []:
        cal_bad_cat = cat_bucket(df, cat_feature)
    else:
        cal_bad_cat = np.nan
    ...
    for i in range (1,len(df_binning.columns)):
        k=calculate_woe_iv(df_binning,df_binning.iloc[:,i].name,'target')
