...
def open_given_file(
    ...
    # If no prompt_cmd given exit
    if not prompt_cmd:
        return 1
    # Search and prompt user
    if z.zathura_search(z.pids):
        choice = zathura_prompt(z.pid_dict, prompt_cmd=prompt_cmd)
...
def main():
    ...
    """
    args = get_opts()
    user = UserSettings(program=__program__, cmd_line_args=args)
    args = user.cmd_args
