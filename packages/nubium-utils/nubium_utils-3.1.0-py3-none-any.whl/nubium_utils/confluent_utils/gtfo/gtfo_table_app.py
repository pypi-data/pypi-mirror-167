...
class TableTransaction(Transaction):
    ...
    def _update_pending_table_writes(self, value):
        ...
        if self._is_not_changelog_message:
            ...
            self._pending_table_offset_increase[self.partition()] += self.offset() - self._table_offset(partition=self.partition())
    ...
    def _table_write(self, recovery_multiplier=None):
        for p, msgs in self._pending_table_writes.items():
            if msgs:
                ...
                LOGGER.debug(f'Table {table.table_name} offset before write: {table.offset}, expected after write: {self._table_offset(partition=p) + int(self._is_not_changelog_message)} (should increase by {len(msgs)} + {int(self._is_not_changelog_message)})')
