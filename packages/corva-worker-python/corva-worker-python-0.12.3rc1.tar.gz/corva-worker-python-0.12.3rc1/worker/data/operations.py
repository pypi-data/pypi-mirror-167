...
def point_main_envs(env: Literal['qa', 'staging', 'production', 'local']):
    ...
    env_all_caps = env.upper()

    api_url = os.getenv(f"API_ROOT_URL_{env}") or os.getenv(f"API_ROOT_URL_{env_all_caps}")
    api_key = os.getenv(f"API_KEY_{env}") or os.getenv(f"API_KEY_{env_all_caps}")
    cache_url = os.getenv(f"CACHE_URL_{env}") or os.getenv(f"CACHE_URL_{env_all_caps}")
    print(f"{env} environment variables: {api_url=}, {api_key=}, {cache_url=}")
...
def setup_redis_worker(env: Literal['qa', 'staging', 'production', 'local']):
    ...
    cache_url = os.getenv(f"CACHE_URL_{env}") or os.getenv(f"CACHE_URL_{env.upper()}")
