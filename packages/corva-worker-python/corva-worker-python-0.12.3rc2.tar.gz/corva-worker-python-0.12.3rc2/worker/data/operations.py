...
def point_main_envs(env: Literal['qa', 'staging', 'production', 'local']):
    ...
    api_url = os.getenv(f"API_ROOT_URL_{env}") or os.getenv(f"API_ROOT_URL")
    api_key = os.getenv(f"API_KEY_{env}") or os.getenv(f"API_KEY")
    cache_url = os.getenv(f"CACHE_URL_{env}") or os.getenv(f"CACHE_URL")
