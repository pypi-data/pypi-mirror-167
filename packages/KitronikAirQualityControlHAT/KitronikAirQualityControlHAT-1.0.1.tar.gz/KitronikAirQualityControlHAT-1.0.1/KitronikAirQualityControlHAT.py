...
class KitronikADC:
    def __init__(self, adcNumber):
        ...
        self.request = "adc" + str(adcNumber) + "r\n"
        self.request = self.request.encode()
        self.air_board_path = "/dev/serial0"
        self.ser = serial.Serial(self.air_board_path, baudrate=115200,
...
class KitronikGPIO:
    def __init__(self, gpioNumber, isPWM = False):
        if gpioNumber > 24: gpioNumber = 24
        if gpioNumber < 22: gpioNumber = 22
...
class KitronikHighPowerOut:
    def __init__(self, poNumber):
        poPin = 13
        if poNumber == 2: poPin = 19
        GPIO.setmode(GPIO.BCM)
        GPIO.setwarnings(False)
        GPIO.setup(poPin, GPIO.OUT)
        self.po = GPIO.PWM(poPin, 100)
        self.dutyCycle = 0

    def changeDutyCycle(self, dutyCycle):
        if dutyCycle > 100: dutyCycle = 100
        if dutyCycle < 0: dutyCycle = 0
        if dutyCycle < 15: self.po.ChangeFrequency(20)
        elif dutyCycle < 20: self.po.ChangeFrequency(50)
        else: self.po.ChangeFrequency(100)
        self.dutyCycle = dutyCycle
        self.po.ChangeDutyCycle(dutyCycle)

    def start(self):
        self.po.start(self.dutyCycle)
...
class KitronikRTC:
    ...
    def set(self, year, month, day, dayOfWeek, hour, minute, second):
        request = "rtc0s"
        for i in range(len(str(year)), 4): request += "0"
        request += str(year)
        for i in range(len(str(month)), 2): request += "0"
        request += str(month)
        for i in range(len(str(day)), 2): request += "0"
        request += str(day)
        for i in range(len(str(dayOfWeek)), 1): request += "0"
        request += str(dayOfWeek)
        for i in range(len(str(hour)), 2): request += "0"
        request += str(hour)
        for i in range(len(str(minute)), 2): request += "0"
        request += str(minute)
        for i in range(len(str(second)), 2): request += "0"
        request += str(second)
        request += "\n"
...
class KitronikZIPLEDs:
    def __init__(self, numberOfLEDs, brightness = 25, autoShow = True):
        self.numberOfLEDs = numberOfLEDs
        self.brightness = brightness
        self.autoShow = autoShow
        self.pixels = PixelStrip(numberOfLEDs, 12, brightness = brightness)
        self.pixels.begin()
        self.fill((0, 0, 0))
        self.show()
        '''
        run("sudo python KitronikZIPLEDs.py -affirm " + 
            str(numberOfLEDs) + " " +
            str(brightness) + " " +
            "fill 0 0 0", shell = True)'''

    def show(self):
        self.pixels.show()

    def setPixel(self, pixelNumber, colour):
        self.pixels.setPixelColorRGB(pixelNumber, colour[0], colour[1], colour[2])
        if self.autoShow: self.show()
        '''
        run("sudo python KitronikZIPLEDs.py -affirm " + 
            str(self.numberOfLEDs) + " " +
            str(self.brightness) + " " +
            "setPixel" + " " +
            str(pixelNumber) + " " +
            str(colour[0]) + " " +
            str(colour[1]) + " " +
            str(colour[2]), shell = True)'''

    def fill(self, colour):
        for i in range(self.numberOfLEDs): self.pixels.setPixelColorRGB(i, colour[0], colour[1], colour[2])
        if self.autoShow: self.show()
        '''
        run("sudo python KitronikZIPLEDs.py -affirm " + 
            str(self.numberOfLEDs) + " " +
