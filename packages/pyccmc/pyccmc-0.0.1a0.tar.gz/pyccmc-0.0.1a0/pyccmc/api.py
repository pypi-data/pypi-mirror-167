...
def metaclient(workdir=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dist')):
...
def download_file(ccmfile, tf, td, config):
    # download and extract files
    # todo: extract tar, rar, ...
    try:
        with tf.open('wb') as o:
            m = requests.Session().get(ccmfile['download_url'], allow_redirects=True, stream=True,
            ...
            if not config['local']['dryrun']:
                ...
                if not str(hashlib.sha1(str(d[k]).encode('utf-8')).hexdigest()) in index:
                    try:
                        ...
                        if download_file(d[k], ftf, ftd, config):
