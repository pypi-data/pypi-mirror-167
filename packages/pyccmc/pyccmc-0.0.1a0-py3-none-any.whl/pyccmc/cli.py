...
#######################################################################
# ccmclient
# creates a local copy of audio files from given artist name or query
# and stores the downloads info 
# api documentation: http://ccmixter.org/query-api
# client documentation:  https://dev.azure.com/kreaterra/_git/ktccmclient
#######################################################################
"""
print(info)

# todo: get a list of all users and / or uploads (with usernames, which would
#  give us all relevant (aka with content) user names)
#  user count. http://ccmixter.org/api/query?limit=0&search_type=any&search=%&dataview=user_basic&format=count
#  user search: limit=0 too high for ccmixter api, find a way to handle this (wrong query=)
#  http://ccmixter.org/api/query?limit=&search_type=any&search=%&dataview=user_basic&format=json&sort=date
#  upload search:

# script info
__author__ = 'tigabeatz'
__contact__ = 'https://dev.azure.com/kreaterra/_git/ktccmclient'
__license__ = 'gpl-3.0'
__version__ = '0.1a'
...
#
# @Gooey(requires_shell=True)
def add_args():
    # command line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument("--test", help="check if this tool is working", default=False)
    parser.add_argument("--artist", help="an artists name")
    parser.add_argument("--limit", help="maximum allowed results", default=10)
    parser.add_argument("--dryrun", default=False)
    parser.add_argument("--itq", default=False)
    parser.add_argument("--mdm", default=False)
    ...
    parser.add_argument("--logfile", help="C:/ccmrepo/ccmclient.log",
                        default=os.path.dirname(os.path.abspath(__file__)) + '/ccmclient.log')
    parser.add_argument("--index", help="e.g. C:/ccmrepo/index.json; used to keep track of already "
                        ...
                        default=os.path.dirname(os.path.abspath(__file__)) + '/index.json')
...
def main():
    ...
    # logging
    logging.basicConfig(filename=Path(args.logfile), filemode='w', level=logging.DEBUG,
    ...
    try:
        if args.graph:
            ...
            cleave = True
            with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'user_graph.json'), 'w') as o:
                o.write(gd)
            print('written to user_graph.json')
        # metadata manager
        elif args.mdm:
            cleave = True
            with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'metacat.json'), 'w') as o:
                o.write(json.dumps(metaclient(os.path.dirname(os.path.abspath(__file__))),
                                   indent=4))
    ...
    - type graph to vizualize the network of uploads and 
      remixes centering on the useers uploads
    - or just type a username 
      (secret mixter mode)
    ##########################################\n
    your selection :>
                        """
    ...
    if not cleave:
        ...
        try:
            ...
            logging.debug('the return is: ' + str(r))
            ...
            with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'downloads.json'), 'w+') as o:
