...
from fastapi_crawler_scheduler.dbRedisHelper import redis_db, standard_time
...
class NewsScheduler(object):
        ...
        for key_list in delete_task_key_list:
            for delete_key, delete_value in redis_db.get_tasks(keys=key_list).items():
                ...
                if os.getpid() == process_id:
                    ...
                    redis_db.delete_key(lock_name=f"scheduler_job:{os.getpid()}:{apscheduler_id}")
                else:
    ...
    def sync_scheduler_job_to_redis(self) -> None:
        ...
        for job in job_list:
            key = f"scheduler_job:{os.getpid()}:{job.id}"
            lock_value = standard_time()
            redis_db.process_acquire(lock_name=key, lock_value=lock_value, expire_time=300)
