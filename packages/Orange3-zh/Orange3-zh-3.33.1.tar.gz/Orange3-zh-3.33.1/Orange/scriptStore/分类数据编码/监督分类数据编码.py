...
def encode(df_X, df_y, cat_encoder='TargetEncoder', percent=0.5):
    ...
    enc = Encoder(cols=df_X[[cat_name]])
...
df, encoder = encode(in_data.X_df, in_data.Y_df, cat_encoder=in_params['cat_encoder'], percent=in_params['percent'])

y_df = in_data.Y_df.reset_index(drop=True)
