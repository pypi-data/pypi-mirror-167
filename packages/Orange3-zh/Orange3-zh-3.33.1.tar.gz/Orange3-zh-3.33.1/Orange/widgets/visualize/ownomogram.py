...
class OWNomogram(OWWidget):
    ...
    def __init__(self):
        ...
        radio_all = gui.appendRadioButton(radio_group, "全部", addToLayout=False)
        radio_best = gui.appendRadioButton(radio_group, "最佳排名:", addToLayout=False)
        ...
        names = list(
            chain.from_iterable(
                [
                    _get_labels(
                        a,
                        lr and lr[i] and lr[i][0] and lr[i][cls_index],
                        OWNomogram.get_ruler_values(
    ...
    def get_points_from_coeffs(current_value, coefficients, possible_values):
        ...
        k = (
            0
            if diff < 1e-6
            else (sorted_values[i] - current_value)
            / (sorted_values[i] - sorted_values[i - 1])
        )
        return sorted_coefficients[i - 1] * sorted_values[
            i - 1
        ] * k + sorted_coefficients[i] * sorted_values[i] * (1 - k)
