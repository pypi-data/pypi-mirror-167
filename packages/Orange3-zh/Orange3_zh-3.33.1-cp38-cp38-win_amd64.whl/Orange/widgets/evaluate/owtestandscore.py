...
class OWTestAndScore(OWWidget):
    ...
    priority = 100
    keywords = ['Cross Validation', 'CV', 'ceshi']
    ...
    class Inputs:
        ...
        learner = MultiInput(
            "学习器(Learner)", Learner, filter_none=True, replaces=['Learner']
        )
        preprocessor = Input("预处理器(Preprocessor)",
                             Preprocess, replaces=['Preprocessor'])

    class Outputs:
        predictions = Output("预测(Predictions)", Table,
                             replaces=['Predictions'])
        evaluations_results = Output(
    ...
    def __init__(self):
        ...
        self.class_selection_combo = gui.comboBox(
            ...
            label="评估目标", orientation=Qt.Horizontal,
