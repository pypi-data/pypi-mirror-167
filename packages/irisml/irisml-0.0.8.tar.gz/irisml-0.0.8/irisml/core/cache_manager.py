...
class AzureBlobStorageManager(StorageManager):
    ...
    def put_contents(self, paths, contents, hash_value):
        ...
        try:
            self._container_client.upload_blob('/'.join(paths), contents, content_settings=content_settings, metadata={self.HASH_METADATA_NAME: hash_value}, max_concurrency=8, timeout=300)
