...
from steam_sdk.parsers.ParserYAML import dict_to_yaml, yaml_to_data_class
...
class AnalysisSTEAM:
    ...
    def __init__(self, file_name_analysis: str = None,
        ...
        self.data_analysis = yaml_to_data_class(self.file_name_analysis, DataAnalysis)
    ...
    def write_analysis_file(self, path_output_file: str):
        ...
        ** Write the analysis data in the target file **
        ...
        all_data_dict = {**self.data_analysis.dict()}
        dict_to_yaml(all_data_dict, path_output_file, list_exceptions=['AnalysisStepSequence', 'variables_to_change'])
    ...
    def step_make_model(self, step, verbose: bool = False):
        ...
        if step.simulation_number:
            if 'FiQuS' in step.software:
                self.setup_sim_FiQuS(magnet_type=BM.model_data.GeneralParameters.magnet_type, simulation_name=step.simulation_name,
                                     simulation_type=step.type, sim_number=step.simulation_number)
        ...
        for v, value in enumerate(step.variable_value):
            ...
            if len_simulation_numbers > 0:
                ...
                if 'FiQuS' in step.software:
                    BM.buildFiQuS()
                    self.setup_sim_FiQuS(magnet_type=BM.model_data.GeneralParameters.magnet_type, simulation_name=step.simulation_name,
                                         model_name=step.model_name, simulation_type=step.type, sim_number=simulation_number)
                if 'LEDET' in step.software:
    ...
    def setup_sim_FiQuS(self, simulation_name, simulation_type, sim_number, model_name=None, magnet_type=None):
        """
        ...
        """
        def _find_original_model(name):
            src = None
            for k, s in self.data_analysis.AnalysisStepDefinition.items():
                if s.type == 'MakeModel':
                    if s.model_name == name:
                        src = os.path.join(os.path.dirname(local_model_folder), str(s.simulation_number), simulation_name)
            return src
        ...
        local_model_folder = os.path.join(local_FiQuS_folder, 'Analysis_' + simulation_name, str(sim_number))
        ...
        if magnet_type == 'multipole':
            if simulation_type == 'MakeModel':
                ...
                source = _find_original_model(model_name)
                while not source:
                    for key, step in self.data_analysis.AnalysisStepDefinition.items():
                        if step.type in ['ModifyModel', 'ModifyModelMultipleVariables']:
                            if model_name in step.new_model_name:
                                new_name = step.model_name
                                source = _find_original_model(new_name)
                os.symlink(source + '.geom', file_name_local + '.geom')
                os.symlink(source + '.set', file_name_local + '.set')
    ...
    def run_sim(self, software: str, simulation_name: str, sim_number: int, simFileType: str = None, verbose: bool = False):
        ...
        """
        if software == 'FiQuS':
            local_analysis_folder = 'Analysis_' + simulation_name
            local_model_folder_input = os.path.join(self.settings.local_FiQuS_folder, local_analysis_folder, str(sim_number))
            ...
            dFiQuS.run_FiQuS(sim_file_name=simulation_name,
                             output_directory=os.path.join(local_analysis_folder, simulation_name))
        elif software == 'LEDET':
