        ...
        if self.circuit_data.InitialConditions.initial_conditions:
            ...
            for name, value in self.circuit_data.InitialConditions.initial_conditions.items():
                if name[0].casefold()=='i':
                    print('Initial condition for currents not supported in XYCE. Will be ignored.')
                    continue
...
def translate_library_PSPICE_2_XYCE(in_file_library: str, out_file_library: str, verbose: bool = False):
    ...
    with open(in_file_library) as f:
        ...
        for i in range(len(lines)):
            if lines[i].startswith('.subckt') and not flag_inSubckt and not flag_inModel:
                flag_inSubckt = 1
                idx_start.append(i)
            elif flag_inSubckt and lines[i].startswith('.ends'):
                flag_inSubckt = 0
                idx_end.append(i)
            if lines[i].startswith('.model') and not flag_inModel and not flag_inSubckt:
                flag_inModel = 1
                idx_start.append(i)
        ...
        for i in range(len(idx_start)):
            ...
            new_model = translate_model_PSPICE_2_XYCE(old_model, output_path)
            new_model = new_model.split('\n')
            new_model = [x+'\n' for x in new_model]
            lines[idx_start[i]:idx_end[i]+1] = new_model[:-1]
