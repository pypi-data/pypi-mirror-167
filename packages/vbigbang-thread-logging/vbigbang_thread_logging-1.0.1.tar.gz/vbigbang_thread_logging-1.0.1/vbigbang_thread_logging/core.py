...
class LoggerT(threading.Thread):
    ...
    def __init__(self, threadID, name, module, logLevel, dont_thread: bool = False):
        ...
        if not hasattr(LoggerV, "_first_init"):
            LoggerV._first_init = True
    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(LoggerV, "_instance"):
            LoggerV._instance = object.__new__(cls)
        return LoggerV._instance
    ...
    def run(self):
        ...
        while True:
            ...
            if not LoggerV.AQueue.empty():
                # 从队列获取日志消息
                data = LoggerV.AQueue.get()
                # 解析日志消息，格式：日志级别，内容
...
def warning(msg):
    if s_dont_thread:
        ...
        LoggerV.AQueue.put({'WARNING': msg})
...
def init(module, level, dont_thread: bool = False):
    ...
    # 创建新线程
    __thread1 = LoggerV(1, "Thread-log", module, level, dont_thread)
    # 开启新线程
    __thread1.start()
    return __thread1
