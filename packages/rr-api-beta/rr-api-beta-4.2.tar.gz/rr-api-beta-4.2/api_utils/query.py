...
class Players:
    ...
    def rank_similar_players(object_id: int, position: str, player_rank, seasons=None):
        ...
        return "select rpr.player_id as rankset_id, rpr.player_name as name, rpm.age, rpm.place_of_birth, rpm.contract_expires, " \
               "cast(CASE WHEN rpm.mv_type = 'Th.' THEN rpm.mv * 1000 " \
               ...
               f"and rpm.player_id = rpr.player_id and rpr.season in ('{seasons[0]}', '{seasons[1]}')"
...
class Ranking:
    ...
    def players_ranking(data_type: int, object_id: int = None):
        main_query = "select distinct rp.rankset_id, rp.rankset_name as player_name, rpr.team_name, rpr.position, rpr.tour_name, cast(rpr.player_rank as decimal (10,2)) AS player_rank," \
                     "cast(rpr.player_rank_with_stats as decimal (10,2)) AS stats_rank, cast(rpr.final_rank as decimal (10,2)) AS total_rank, rpr.tour_rank, rpr.team_rank, rpr.minutes_played, rpr.minutes_played_new, rpr.season, rpm.age, rpm.place_of_birth," \
                     "CASE WHEN rpm.mv_type = 'Th.' THEN rpm.mv * 1000 WHEN rpm.mv_type = 'm' THEN rpm.mv * 1000000 WHEN rpm.mv_type = 'bn' THEN rpm.mv * 1000000000 ELSE 0 END as market_value " \
        ...
        end_query = " and rpr.player_rank group by rp.rankset_id, rp.rankset_name, rpr.team_name, rpr.tour_name, rpr.position, rpr.player_rank," \
                    ...
                    " rpr.season, rpm.age,rpm.place_of_birth, rpm.mv, rpm.mv_type order by total_rank desc;"
        match data_type:
            case 1:
