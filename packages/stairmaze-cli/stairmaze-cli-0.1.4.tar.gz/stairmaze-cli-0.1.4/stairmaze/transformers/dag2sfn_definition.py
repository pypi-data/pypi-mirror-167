...
class Dag2SFNDefintionTransformer:
    @classmethod
    def transform(self, dag, env_name, dag_reducer=DagReducer):
        ...
        for level, task_names in enumerate(levels):
            ...
            if len(task_names) > 1:
                ...
                for task_name in task_names:
                    job_definition_name = f"stairmaze-{env_name}-{dag.name}-{task_name}"
                ...
                rendered = json.loads(
                    template.render(
                        job_definition=job_definition_name, job_queue="${job_queue}"
                    )
                )
                state = rendered
