...
def plot_image (app, plot):
    ...
    hashkey = str(cmd) + str(options) + str(text)
    ...
    if cmd == "convert" or cmd == "montage":
        #print("text: %s" %(text))
        conver_outfile = text.split()[-1].split()[-1]
...
def cmd_2_image (app, plot):
    ...
    if "convert" in args or "montage" in args or "magick" in args:
        # for html, use the original suffix, for pdf, only support png.
        plot_format = (app.builder.format in ["html"]) and \
                text[-1].split(".")[-1] or "png"
    elif "dot" in args:
        #dot
        if "-T" in args:
