...
from orpyter.core import name_to_title
from orpyter.ui import schema_utils
from orpyter.ui.streamlit_utils import CUSTOM_STREAMLIT_CSS, SessionState, get_session_state

STREAMLIT_RUNNER_SNIPPET = """
from orpyter.ui import render_streamlit_ui
from orpyter import Orpyter
from os import getcwd

import streamlit as st

st.set_page_config(page_title="Orpyter", page_icon=":arrow_forward:")

with st.spinner("Loading Orpyter. Please wait..."):
    orpyter = Orpyter("{orpyter_path}")

render_streamlit_ui(orpyter)
"""


def launch_ui(orpyter_path: str, port: int = 8501) -> None:
    ...
    with NamedTemporaryFile(
        ...
        f.write(STREAMLIT_RUNNER_SNIPPET.format(orpyter_path=orpyter_path))
        f.seek(0)

        # TODO: PYTHONPATH="$PYTHONPATH:/workspace/orpyter/src"
        import subprocess
...
def render_streamlit_ui(orpyter: Orpyter) -> None:
    """Render the streamlit UI for the given orpyter"""
    ...
    if orpyter.description:
        st.markdown(orpyter.description)

    InputUI(session_state=session_state, input_class=orpyter.input_type).render_ui()
        ...
        with st.spinner("Executing operation. Please wait..."):
            try:
                input_data_obj = parse_obj_as(
                    orpyter.input_type, session_state.input_data
                )
                session_state.output_data = orpyter(input=input_data_obj)
