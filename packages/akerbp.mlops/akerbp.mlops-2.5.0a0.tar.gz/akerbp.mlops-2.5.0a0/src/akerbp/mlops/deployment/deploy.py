    ...
    try:
        ...
        if c.platform == "cdf":
            if (env == "test" or env == "prod") and testing_only == "True":
                ...
                # Garbage collection goes here; delete all models up to v(latest_artifact_version - models_to_keep)
                if c.models_to_keep is None:
                ...
                if (
                    ...
                    logging.info(
                        f"Starting garbage collection of model {human_readable_function_name} in {env}"
                    )
                    oldest_model = (
                        mm.get_model_version_overview(model_name=c.model_name, env=env)
                        .iloc[-1, :]
                        .externalId.split("/")[-1]
                    )
                    logging.info(f"The oldest model in {env} is {oldest_model}")
