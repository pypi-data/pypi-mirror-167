            ...
            if _total > 0 and _passed > 0:
                ...
                if get_system_key(Constant.ISSUE_KEY) is not None and get_system_key(Constant.ISSUE_KEY).strip() !='':
                    ...
                    if get_system_key('TestPlanId') is not None:
                        ...
                        logger.info("测试计划："+get_system_key('TestPlanId')+" 测试周期："+get_system_key("CycleName"))
                        logger.info("测试周期用例列表"+cycleCases)
                        logger.info("自动化用例名称"+caseName)
                        logger.info("自动化用例结果" + caseStatus)
                        for i in range(0, len(caseName)):
                            logger.info("用例名称："+testcase[i]+"执行结果："+caseStatus[i])
