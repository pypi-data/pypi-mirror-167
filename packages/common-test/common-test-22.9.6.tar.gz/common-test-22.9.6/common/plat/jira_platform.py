...
class JiraPlatForm(object):
    ...
    @classmethod
    def updatecaseByrunId(self, testPlanIssueKey, cycleName, caseName, caseStatus):
        caseStatus = 4
        ...
        runid=jsonpath(JiraPlatForm.getReclyTestCase(testPlanIssueKey, cycleName), xpath)[0]
        return APIDriver.http_request(
            url=f"{Constant.JIRA_URL}/rest/synapse/1.0/testRun/updateTestRunStatus?runId={runid}&status={caseStatus}",
            method='put',
            parametric_key='json',
            _auth=HTTPBasicAuth(get_system_key(Constant.JIRA_USERNAME),
                                get_system_key(Constant.JIRA_PASSWORD))
...
if __name__ == '__main__':
    # set_system_key('jiraUserName','oushiqiang')
    # set_system_key('jiraPassword','Xiaoshu406@@')
    # name="高端旅客服务工单监控：点击旅客姓名，点击小眼睛图标，查看全部信息"
    # # JiraPlatForm.getReclyRunidByName('DS21059-2877','测试第一周期',name)

    # 取一定范围的元素

    lucky_number = [0, 1, 2, 3, 4, 5, 6,7]
    print(lucky_number[::2])
    print(lucky_number[1::2])
