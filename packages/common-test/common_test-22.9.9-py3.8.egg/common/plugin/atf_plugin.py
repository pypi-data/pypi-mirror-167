                ...
                if get_system_key(Constant.ISSUE_KEY) is not None and get_system_key(Constant.ISSUE_KEY).strip() !='':
                    ...
                    if get_system_key('TestPlanId') is not None:
                        ...
                        for i in range(0, len(caseName)):
                            print("测试计划："+get_system_key('TestPlanId')+"执行周期:" + get_system_key("CycleName")+"用例名称："+testcase[i]+"执行结果："+caseStatus[i])
                            JiraPlatForm.updatecaseByrunId(get_system_key('TestPlanId'), get_system_key("CycleName"),testcase[i], caseStatus[i])
