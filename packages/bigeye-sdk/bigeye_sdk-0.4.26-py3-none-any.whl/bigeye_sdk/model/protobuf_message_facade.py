...
class SimpleConstantThreshold(SimpleThreshold, type='CONSTANT'):
    ...
    def to_datawatch_object(self) -> List[Threshold]:
        ...
        sb.value = self.lower_bound if self.lower_bound != 0.0 else -0.00001
        lb.bound = sb
        sb = SimpleBound()
        sb.bound_type = SimpleBoundType.UPPER_BOUND_SIMPLE_BOUND_TYPE
        sb.value = self.upper_bound if self.upper_bound != 0.0 else 0.00001
...
class SimpleFreshnessThreshold(SimpleThreshold, type='FRESHNESS'):
    ...
    def to_datawatch_object(self, **kwargs) -> List[Threshold]:
        upper_bound = self.upper_bound if self.upper_bound != 0.0 else 0.00001
        ub = SimpleBound(bound_type=SimpleBoundType.UPPER_BOUND_SIMPLE_BOUND_TYPE, value=upper_bound)
...
class SimpleRelativeThreshold(SimpleThreshold, type='RELATIVE'):
    ...
    def to_datawatch_object(self, **kwargs) -> List[Threshold]:
        ...
        upper_bound = self.upper_bound if self.upper_bound != 0.0 else 0.00001
        lower_bound = self.lower_bound if self.lower_bound != 0.0 else -0.00001
        lb = SimpleBound(bound_type=SimpleBoundType.LOWER_BOUND_SIMPLE_BOUND_TYPE, value=lower_bound)
        ub = SimpleBound(bound_type=SimpleBoundType.UPPER_BOUND_SIMPLE_BOUND_TYPE, value=upper_bound)
...
class SimpleMetricDefinition(YamlModelWithValidatorContext, DatawatchFacade):
    ...
    grain_seconds: Optional[int] = 86400  # 1 day.
    ...
    def set_default_threshold(cls, threshold, values):
        ...
        if threshold:
            """For the case that a user as specifically defined a metric"""
            return threshold
    ...
    def must_be_dict_type(cls, metric_type):
        ...
        if metric_type and not isinstance(metric_type, dict):
            errlns = yaml.safe_dump({'metric_type': metric_type}, indent=True)
    ...
    def must_be_list(cls, values):
        def register_invalid_type_validation_error(field: str):
            # error_context_lines = yaml.safe_dump(values, indent=True).split('\n'),
            errlns = safe_split_dict_entry_lines(field, values[field])
            SimpleMetricDefinition.register_validation_error(
                error_lines=errlns,
                error_message=FORMATTING_ERRMSG.format(
                    s=f'`{field}` attribute expects a list but type {values.get(field).__class__.__name__} was received')
            )

        sla_ids = values.get('sla_ids')
        conditions = values.get('conditions')
        group_by = values.get('group_by')
        parameters = values.get('parameters')
        if sla_ids and not isinstance(sla_ids, List):
            register_invalid_type_validation_error('sla_ids')
            values['sla_ids'] = [sla_ids]

        if conditions and not isinstance(conditions, list):
            register_invalid_type_validation_error('conditions')
            values['conditions'] = [conditions]

        if group_by and not isinstance(group_by, list):
            register_invalid_type_validation_error('group_by')
            values['group_by'] = [group_by]

        if parameters and not isinstance(parameters, list):
            register_invalid_type_validation_error('parameters')
