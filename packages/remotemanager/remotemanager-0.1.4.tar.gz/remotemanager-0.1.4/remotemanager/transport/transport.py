...
class Transport(SendableMixin):
    ...
    def add_transfer(self,
        ...
        if mode == 'push':
            pair = (origin, self._add_address(target))
        else:
            pair = (self._add_address(origin), target)

        files = [os.path.split(f)[1] for f in ensure_list(files)]

        self._logger.info(f'adding transfer: {pair[0]} -> {pair[1]}')
        self._logger.info(f'for files {files}')

        if pair in self._transfers:
            self._transfers[pair] += files
        else:
            self._transfers[pair] = files

    def _add_address(self, dir):
        if self.address is None:
            return dir
        return f'{self.address}:{dir}'

    @staticmethod
    def _format_for_cmd(inp):
    ...
    def print_transfers(self):
        ...
        """
        i = 0
        for pair, files in self._transfers.items():
            i += 1
            print(f'transfer {i}:'
                  f'\norigin: {pair[0]}'
                  f'\ntarget: {pair[1]}')
    ...
    def transfer(self,
        ...
        Returns:
            result of command (string, or CMD instance)
        """
        ...
        for pair, files in self.transfers.items():
            ...
            primary = ensure_dir(os.path.split(pair[0])[0])
            secondary = ensure_dir(os.path.split(pair[1])[0])
