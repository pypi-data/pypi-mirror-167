...
def make_basic_fit():
    ...
    with open('files.xcm') as fi:
        xspec_commands = fi.readlines()

    with open('files-auto.xcm', 'w') as ff:
        for ss in xspec_commands[:-2]:
            ff.write(ss)
        ff.write('cpd basic-plot.gif/GIF\nplot ld del\n')
        ff.write('quit\n\n')
...
def rebin_matrix(e_min, e_max, pp, dpp, min_s_n = 50):
    ...
    #print('We rebinned from %d to %d bins at a minimum S/N of %.1f' % (len(e_min), len(new_e_mins), min_s_n))
...
def get_error_from_simul(counts, counts_err, method, n_simul=100, **kwargs):
    simul_rms = []
...
def fft_pulsed_fraction(x, dx, level=0.1, n_harm_min=2, plot=False):
    '''
    Computes the pulsed fraction using an FFT. It stops as soon as the pulse is described at beter than level
    :param x:
    :param dx:
    :param level:
    :param n_harm_min:
    :param plot:
    :return: pulsed fraction
    '''
    ...
    for n_harm in range(n_harm_min, int(np.floor(n / 2))):
        mask = np.ones(n, dtype=np.cdouble)
        mask[n_harm:-(n_harm - 1)] = 0 + 0j
        ifft = np.fft.ifft(fft * mask)
        y = np.real(ifft)

        chi2_val = np.sum(((x - y) / dx) ** 2)
        dof = 1 + 2 * (n_harm - 1)
        if chi2.cdf(chi2_val, dof) < level:
            break
    ...
    if plot:
        ...
        plt.figure()
        f = np.linspace(0, 1, n)
        plt.errorbar(f, x, yerr=dx, marker='.')
        plt.plot(f, y)
...
def plot_matrix_as_image(ee, pp, kind='E', normalize=True,outfile=None, cmap=matplotlib.cm.gist_earth,
    ...
    import matplotlib.pyplot as plt
    plt.figure()
    levels = np.linspace(np.min(pp), np.max(pp), n_levels)
    ...
    if sliders:
        ...
        data_min = pp[np.isfinite(pp)].min()
        data_max = pp[np.isfinite(pp)].max()
        smin = Slider(cmin, 'Min', data_min, data_max, valinit=1., orientation='horizontal')
        smax = Slider(cmax, 'Max', data_min, data_max, valinit=10., orientation='horizontal')
        def update(x):
...
def plot_matrix_as_lines(t, pp, dpp, kind='T', normalize=False, offset=0):
    ...
    for i in range(pt.shape[0]):
        ...
        if np.sum(dy) > 0:
            ...
            if kind == 'E':
                label = "%.0f keV" % t[i]
            plt.errorbar(phi, y+total_offset, xerr=0.5 / pt.shape[1], yerr=dy, linestyle='-', marker='.', label=label)
            total_offset += offset
