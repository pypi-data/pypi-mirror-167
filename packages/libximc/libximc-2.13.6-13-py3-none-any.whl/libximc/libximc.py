...
def _load_lib():
    ...
    if os_kind == "windows":
        ...
        if 8 * struct.calcsize("P") == 32:
            paths = (_near_script_path("win32\\libximc.dll"),
                     _near_script_path("libximc.dll"),
                     "libximc.dll")
        else:
            paths = (_near_script_path("win64\\libximc.dll"),
                     _near_script_path("libximc.dll"),
                     "libximc.dll")
    elif os_kind == "linux":
        method = CDLL
        ...
        if cpu_kind == "arm":
            cpu_path = "debian-armhf/libximc.so"
        elif cpu_kind == "i386":
            cpu_path = "debian-i386/libximc.so"
        else:
            cpu_path = "debian-amd64/libximc.so"
        print(cpu_kind)
        #sys.path.append(_near_script_path(cpu_path))
        paths = (_near_script_path("cpu_path"),
                 _near_script_path("libximc.so"),
                 "libximc.so")
    else:
