...
class ReqUtil(object):
    ...
    async def call_sync_trans(self, events, s_type="rest"):
        ...
        return await self.restful_fetch(config.Server.PCoreAddr + "/patchCore/syncTransRegister", "POST", {
            "Type": s_type, "Creator": config.Server.Name, "Events": pes
        })
    ...
    async def call_patch_delay(self, event, delay):
        ...
        from magpielib.application import config
        return await self.restful_fetch(config.Server.PCoreAddr + "/patchCore/patchRegister", "POST", {
            "Event": p_event, "Creator": config.Server.Name, "Delay": delay
        }, behavior="delay")
    ...
    async def call_patch_wait4done(self, event, callback_patch4done, callback_patch4progress=None):
        ...
        from magpielib.application import config
        status, data = await self.restful_fetch(config.Server.PCoreAddr + "/patchCore/patchRegister", "POST", {
            "Event": p_event, "Creator": config.Server.Name
        }, behavior="wait")
