...
class TableQuery:
    ...
    def __init__(self, session, db_class):
        self.query = session.query(db_class)
        self.db_class = db_class

    def search_query(self, search: list):
        """获取search对应的 cond值！
        """
        if not search:
            return self
        search_cells = self._get_search_cell(search)
        search_cond = []
    ...
    def sort_query(self, sort: list):
        """获取对应的order by 的query
        """
        if not sort:
            return self
        sort_cells = self._get_sort_cell(sort)
        sort_cond = []
    ...
    def page_count_query(self, page, count):
        if not page or not count:
            raise Exception("page count 不能为0")
        self.query = self.query.offset((page - 1) * count).limit(count)
        return self
