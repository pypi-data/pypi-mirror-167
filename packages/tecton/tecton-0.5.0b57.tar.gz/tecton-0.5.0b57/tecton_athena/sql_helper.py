...
def get_historical_features_sql(
    ...
    delta_enabled = get_session().config.enable_experimental_delta_support
    ...
    for item in feature_set_items:
        ...
        if offline_store_type != "parquet" and not delta_enabled:
            ...
            raise TectonValidationError(
                f"FeatureView {fd.name} uses {offline_store_type} as the offline store, only Parquet is supported for Athena retrieval."
            )
