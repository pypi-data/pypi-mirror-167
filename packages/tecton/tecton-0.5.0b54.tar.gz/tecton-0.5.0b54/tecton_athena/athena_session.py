...
class AthenaSessionConfig:
    ...
    spine_temp_table_name: Optional[str] = None  # Specifies the temp table name in the catalog used for spines
...
class AthenaSession:
    ...
    def _upload_pandas_to_s3(self, pandas_df: pandas.DataFrame):
        with tempfile.NamedTemporaryFile(suffix=".parquet.snappy") as f:
            ...
            s3_client.upload_file(f.name, s3_bucket, s3_object_key)
    ...
    def get_spine_temp_table_name(self):
        return self.config.spine_temp_table_name or "TEMP_SPINE_TABLE_FROM_DF"
