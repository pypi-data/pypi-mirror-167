...
def build_datasource_input_querynodes(
    ...
    for input_name, node in ds_inputs.items():
        ...
        input_querynodes[input_name] = DataSourceScanNode.ref(
...
def build_run_querytree(
    ...
    if enable_feature_metrics:
        tree = MetricsCollectorNode.ref(tree)
    if fdw.is_temporal:
        ...
        tree = PartialAggNode.ref(tree, fdw, ANCHOR_TIME, window_end_column_name, aggregation_anchor_time)
    else:
...
def build_get_features(
    ...
    if not from_source:
        ...
        return OfflineStoreScanNode.ref(feature_definition_wrapper=fdw, time_filter=feature_data_time_limits)
    else:
...
def build_get_full_agg_features(
    ...
    return FullAggNode.ref(base, fdw, spine, respect_feature_start_time=respect_feature_start_time)
