...
class OfflineStoreScanNode(QueryNode):
    ...
    def as_str(self, verbose: bool):
        ...
        if self.entities is not None:
            s += f"EntityFilter: <User-provided Data>\n"
    ...
    def with_time_filter(self, feature_time_filter: pendulum.Period):
        return OfflineStoreScanNode(
            self.feature_definition_wrapper, time_filter=feature_time_filter, entities=self.entities
        )

    def with_entity_filter(self, entities: Any):
        return OfflineStoreScanNode(self.feature_definition_wrapper, time_filter=self.time_filter, entities=entities)
