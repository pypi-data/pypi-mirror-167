...
def get_features(
    ...
    if conf.get_bool("ALPHA_QUERYTREE_ENABLED"):
        ...
        if fd.is_temporal or fd.is_feature_table:
            ...
            qt = RenameColsNode.ref(qt, {ANCHOR_TIME: None})
        else:
            ...
            qt = ConvertEpochToTimestamp.ref(qt, {ANCHOR_TIME: fd.get_feature_store_format_version})
            qt = RenameColsNode.ref(qt, {ANCHOR_TIME: fd.trailing_time_window_aggregation.time_key})

        if entities is not None:
            entities_df = DataNode.ref(functions.broadcast(entities.distinct()))
            qt = JoinNode.ref(qt, entities_df, entities.columns, how="right")

        if _start or _end:
            qt = TimeFilterNode.ref(qt, _start, _end, fd.timestamp_key)
