...
from pyg_extension.utils.log_model import LogModule, HookGradientModule, make_dot_, \
    get_target_layer, AverageMeterTotal
...
class LearningFlow:
    ...
    def __init__(self, model: Module, train_loader: DataLoader, validate_loader: DataLoader, device="cpu",
                 optimizer=None, clf: bool = False, loss_method=None, learning_rate: float = 1e-3,
                 weight_decay: float = 0.0, checkpoint=True, scheduler=None, debug: str = "", target_layers=(),
    ...
    def my_debug(self):
        ...
        if "single" in self.debug:
            self.model.train()
            for m, data in enumerate(self.train_loader):  # just record for once,
                data = data.to(self.device)
                self.model(data)
                self.weight_log.get_weight(self.model, append=False)
                break
        ...
        if "hook" in self.debug:
            # which layer to hook.
            self.target_layers = get_target_layer(self.model, self.target_layers)
            self.gradient_log = HookGradientModule(self.target_layers)
            self.model.train()
            for m, data in enumerate(self.train_loader):  # just record for once,
                data = data.to(self.device)
    ...
    def run(self, epoch=50, warm_start=False):
        ...
        if resume:
            if os.path.isfile(resume):
                ...
                self.threshold = checkpoint['best_error']
        ...
        for epochi in range(start_epoch, epoch):
            ...
            if "loop" in self.debug in self.debug:  # record each epochi
    ...
    def predict(self, predict_loader: DataLoader, return_y_true=False, add_hook=False, hook_layer=None):
        """
        ...
        add_hook:bool
        hook_layer:
            layer of model. could get layers by ``get_target_layer`` or ``get_layers_name_with_weight``.
