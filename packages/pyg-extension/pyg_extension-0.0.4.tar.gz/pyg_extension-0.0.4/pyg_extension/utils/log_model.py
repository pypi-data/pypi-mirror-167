...
def get_layers_name_with_weight(model: Module, add_name=""):
    """Get layers with weight."""
    name_weight_module = {}
    for name, layer in model._modules.items():
        name = "{}->{}".format(add_name, name)
        # if isinstance(layer,Module):
        if len(layer._modules) > 0:
            name_weight_module.update(get_layers_name_with_weight(layer, name))
        else:
...
def get_target_layer(model, target_layers):
    """which layer to hook."""
    if isinstance(target_layers, (list, tuple)) and len(target_layers) > 0:
        ...
        if target_layers == "all":
            ...
            # weight layer
            mmdict = get_layers_name_with_weight(model)
...
class LogModule:
    ...
    def get_weight(self, model: Module, add_name="", append=True):
        ...
        if append:
            ...
            self.name_weight_module = name_weight_module
