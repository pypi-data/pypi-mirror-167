...
def write(stream: BytesIO, obj: PharBase):
    assert stream.tell() == 0
    ...
    stream.write(PharSignFlag.SHA1.value.to_bytes(4, 'little'))
    stream.write(b'GBMB')


def read_manifest(stream: BytesIO, obj: PharBase):
    cursor = stream.tell()
...
def read_entry_manifest(stream: BytesIO, obj: PharBase):
    entry = PharEntry()
    ...
    entry.permissions = PharEntryPermission(flags & 0x1ff)
    entry.flags = PharEntryFlag(flags & 0xfffffe00)
    metadata_len = int.from_bytes(stream.read(4), 'little')
    if metadata_len != 0:
        metadata_raw = stream.read(metadata_len)
...
def verify_signature(stream: BytesIO, obj: PharBase):
    if PharGlobalFlag.SIGNED not in obj.flags:
        return
    ...
    if len(signature) <= 8 or not signature.endswith(b'GBMB'):
        ...
        return
    sign_flag = PharSignFlag(
        int.from_bytes(signature[-8:-4], 'little')
    )
    signature = signature[:-8]
    if PharSignFlag.MD5 in sign_flag:
        result = hashlib.md5(content).digest()
    elif PharSignFlag.SHA1 in sign_flag:
        result = hashlib.sha1(content).digest()
    elif PharSignFlag.SHA256 in sign_flag:
        result = hashlib.sha256(content).digest()
    elif PharSignFlag.OPENSSL in sign_flag:
...
def write_manifest(stream: BytesIO, obj: PharBase):
...
def write_entry_manifest(stream: BytesIO, entry: PharEntry):
