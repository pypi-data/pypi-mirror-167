            ...
            if nb_cores == 1:
                for component in files:
                    if action_type == ActionType.BUILD:
                        ...
                        log.error(f"Unknown value for action_type: '{action_type}'. It should be either '{ActionType.BUILD}' or '{ActionType.VALIDATE}'")
            else:
                log.info(f"Batch component processing with {nb_cores} threads. Action: '{action_type}'")
...
"""
    ...
    def component_is_active(self, component, modified_files) -> bool:
        ...
        component_additional_includes_contents = self.get_target_file_contents(
            component, TargetType.ADDITIONAL_INCLUDES, 
        )
        # Let's grab the contents of the additional_includes file if it exists.
        component_dependency_hints_contents = self.get_target_file_contents(
            component, TargetType.DEPENDENCY_HINTS,
        )
        ...
        for modified_file in modified_files:
            if (
                ...
                or self.is_in_target_list(
                    modified_file, TargetType.ADDITIONAL_INCLUDES,  component_additional_includes_contents
                )
                or self.is_in_target_list(
                    modified_file, TargetType.DEPENDENCY_HINTS,component_dependency_hints_contents
                )
            ):
                return True
        return False

    def get_target_file_contents(self, component, target_type) -> Union[List[str], None]:
        ...
        component_target_file_path = (
            self.get_theoretical_target_file_path(component, target_type)
        )
        # And we load it if it exists.
        if Path(component_target_file_path).exists():
            with open(
                component_target_file_path, "r"
            ) as component_target_file:
                if (target_type == TargetType.DEPENDENCY_HINTS) and (component_target_file_contents is not None):
                    component_target_file_contents += (
                        component_target_file.readlines()
                    )
                else:
                    component_target_file_contents = (
                        component_target_file.readlines()
    ...
    def is_in_target_list(self, modified_file, target_type, target_list_contents) -> bool:
        """
    ...
    def run_with_config(self):
        ...
        if self.config.signing_mode == "aml":
            ...
            built_component_files = self.process_all_components(component_files, ActionType.BUILD)
        else:
