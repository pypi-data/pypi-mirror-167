...
from lazyearth.mainfile import objearth as oe
...
class water:
        ...
        @staticmethod
        def split_data_by_bands(data, selected_keys):
        ...
        def identify_water_cluster(self,detect_water_cluster,bands):
            print("My.Data : detect_water_cluster: ",detect_water_cluster)
            print("My.Data : bands : ",bands)
            if detect_water_cluster == 'maxmndwi':
                ...
                print('My.data : self.clusters_params',self.clusters_params)
        ...
        def waterdetect(self):
            ...
            #check if the MNDWI index is necessary and if it exists
            # # print('My.DataComponent : self.required_indices',self.required_indices)
            # # print('My.DataComponent : bands',bands)
            if 'mndwi' in self.required_indices and 'mndwi' not in bands:
                mndwi,mndwi_mask = self.calc_normalized_difference(bands['Green'],bands['Mir2'],invalid_mask)
                print(mndwi,mndwi_mask)
                invalid_mask |= mndwi_mask
                bands.update({'mndwi':mndwi})
            # # print('My.DataComponent : bands :',bands)

            #check if the NDWI index exist
            if 'ndwi' in self.required_bands and 'ndwi' not in bands.keys():
                ndwi,ndwi_mask = self.calc_normalized_difference(bands['Green'],bands['Nir'],invalid_mask)
                invalid_mask |= ndwi_mask
                bands.update({'ndwi':ndwi})
            # print('My.DataComponent : bands :',bands)

            #check if the MBWI index exist
            if 'mbwi' in self.required_bands and 'mbwi' not in bands.keys():
                print('x')
                mbwi,mbwi_mask = self.calc_mbwi(bands,3,invalid_mask)
            ...
            #Transform the rasters in a matrix where each band is a column
            data_as_columns = self.bands_to_columns(bands,invalid_mask)
            # print('My.DataComponent : data_as_columns :',data_as_columns)
            ...
            # print('My.DataComponent : train_data_as_columns :',train_data_as_columns)

            #split1
            split_train_data_as_columns = self.split_data_by_bands(train_data_as_columns,self.bands_keys)
            #split2
            split_data_as_columns = self.split_data_by_bands(data_as_columns,self.bands_keys)
            # print('My.DataComponent : split_train_data_as_columns :',split_train_data_as_columns)
            # print('My.DataComponent : split_data_as_columns :',split_data_as_columns)

            #find best_k
            best_k = self.find_best_k(split_train_data_as_columns)
            # print('My.DataComponent : best_k :',best_k)

            #apply the clusterization algorithm and return labels and train dataset
            train_clusters_labels = self.apply_cluster(split_train_data_as_columns,best_k)
            # print('My.DataComponent : train_clusters_labels :',train_clusters_labels)

            #cals statistics for each cluster
            self.clusters_params = self.calc_clusters_params(train_data_as_columns,train_clusters_labels,best_k)
            print('My.DataComponent : clusters_params :',self.clusters_params)

            #detect the water cluster
            water_cluster = self.identify_water_cluster(self.detect_water_cluster,bands)
            print('My.DataComponent : water_cluster :',water_cluster)
            ...
            for band,value in zip(self.clip_band,self.clip_sup_value):
                ...
                if value is not None:
                    if self.glint_mode and self.glint_processor is not None:
                        ...
                        print('1')
                        comp_array = value
                    clusters_labels[(clusters_labels == water_cluster['clusterid']) & (bands[band][~invalid_mask]>comp_array)] = -1
            #after obtainting the final labels,clip bands with inferior limit
            ...
            #create an cluster array based on the cluster result (water will be value1)
            cluster_matrix = self.create_matrice_cluster(ind_data,bands,clusters_labels,water_cluster,best_k)
