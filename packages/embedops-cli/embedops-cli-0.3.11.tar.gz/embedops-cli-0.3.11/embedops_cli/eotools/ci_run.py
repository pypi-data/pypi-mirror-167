...
class CIRun:
    ...
    def create_main(self):
        ...
        return self.create(analysis_type, commit_id, branch_name, job_name)

    @staticmethod
    def create(analysis_type, commit_id, branch_name, job_name=None):

        """Use the branch name, commit sha, and analysis type to create a new ciRun instance"""

        api_client = get_client()
        create_properties = CIRunCreateProps(
            branch=branch_name, commit_id=commit_id, type=analysis_type, name=job_name
        )
