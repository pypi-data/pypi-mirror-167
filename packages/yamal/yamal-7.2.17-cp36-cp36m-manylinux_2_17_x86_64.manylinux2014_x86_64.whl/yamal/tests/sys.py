...
class TestLoadModule(unittest.TestCase):
    ...
    def test_reactor_deploy(self):
        cfgpath = get_tempfile_name("config")
        cfg = {
            "component_one" : {
                "module":"testmodule",
                "type":"testcomponent",
    ...
    def test_yamal_run(self):
        ...
        with open(cfgpath,"w") as f:
            f.write(json.dumps(cfg))
        subprocess.run(f"yamal-run.py -c {cfgpath} -s main -m testmodule -o testcomponent", env=f"YAMALCOMPPATH={yamal.__path__}", check=True)
        os.remove(cfgpath)
    ...
    def test_yamal_run_only_cfg(self):
        cfgpath = get_tempfile_name("config")
        cfg = {
            "component_one" : {
                "module":"testmodule",
        ...
        subprocess.run(f"yamal-run.py -c {cfgpath}", env=f"YAMALCOMPPATH={yamal.__path__}", check=True)
        os.remove(cfgpath)
