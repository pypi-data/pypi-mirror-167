...
def start(tkn):
  resp = requests.get("https://goobler.ericplayzyt.repl.co/api/get_name_using_auth", {"auth": f"{tkn}"}).json()
  print(f"Starting {resp['user_name']}...")
  resp2 = requests.post("https://goobler.ericplayzyt.repl.co/api/login/bot_account", {"auth": f"{tkn}"}).json()
