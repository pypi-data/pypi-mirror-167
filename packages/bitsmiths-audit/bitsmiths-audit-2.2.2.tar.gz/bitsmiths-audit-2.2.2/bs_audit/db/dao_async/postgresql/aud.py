...
class dAud:
    ...
    async def try_select_one_by_key(self, _key: tAudKey) -> bool:
        ...
        """
        async with dAudSelectOne(self._dbcon) as _qry:
    ...
    async def select_one(self) -> "dAud":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dAud, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dAud, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tAudKey) -> "dAud":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dAud, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def insert(self, irec: tAud = None):
        ...
        async with dAudInsert(self._dbcon) as _qry:
