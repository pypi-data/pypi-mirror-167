...
class dCfg:
    ...
    async def try_select_one_by_key(self, _key: tCfgKey) -> bool:
        ...
        """
        async with dCfgSelectOne(self._dbcon) as _qry:
    ...
    async def select_one(self) -> "dCfg":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dCfg, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dCfg, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tCfgKey) -> "dCfg":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dCfg, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def insert(self, irec: tCfg = None):
        ...
        async with dCfgInsert(self._dbcon) as _qry:
