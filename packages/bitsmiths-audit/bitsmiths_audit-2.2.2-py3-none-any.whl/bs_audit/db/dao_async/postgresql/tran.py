...
class dTran:
    ...
    async def try_select_one_by_key(self, _key: tTranKey) -> bool:
        ...
        """
        async with dTranSelectOne(self._dbcon) as _qry:
    ...
    async def select_one(self) -> "dTran":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dTran, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dTran, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tTranKey) -> "dTran":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dTran, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def insert(self, irec: tTran = None):
        ...
        async with dTranInsert(self._dbcon) as _qry:
