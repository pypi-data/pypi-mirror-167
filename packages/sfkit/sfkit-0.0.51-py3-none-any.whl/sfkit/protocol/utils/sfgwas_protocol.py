...
def start_sfgwas(role: str) -> None:
    ...
    bash_command = f"PID={role} go run sfgwas.go | tee /dev/tty > stdout_party{role}.txt"
    command = f"export PYTHONUNBUFFERED=TRUE && export GOROOT=~/.local/lib/go && cd sfgwas && bash {bash_command}"
    run_command(command)
