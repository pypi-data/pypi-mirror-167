...
from pylabrobot.liquid_handling.resources.abstract import Resource, Tip
  ...
  def discard_tips(
    ...
    **backend_kwargs
    ...
    for channel in tips:
      venus_resource = self._get_venus_resource(resource)
      if channel is not None:
        column = string_to_position(channel)[1]
        if last_column and column < last_column:
          raise ValueError("With this backend, tips must be picked up in ascending column order.")
        pos_tuples.append((venus_resource, column))
      else:
  ...
  def aspirate(
    ...
    **backend_kwargs
    ...
    for channel in channels:
      ...
      if channel is not None:
        # channel.resource
        # TODO: get channel from resource
        column = string_to_position(channel.position)[1]
        if last_column and column < last_column:
          raise ValueError("With this backend, tips must be picked up in ascending column order.")
        pos_tuples.append((venus_resource, column))
