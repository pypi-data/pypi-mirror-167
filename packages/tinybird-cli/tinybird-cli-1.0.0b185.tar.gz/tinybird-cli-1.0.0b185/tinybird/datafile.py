...
async def new_pipe(
    ...
    r = await requests_post(f"{host}/v0/pipes?{urlencode(params)}", headers=post_headers, data=json.dumps(body))
    data = json.loads(r.content)

    if r.status_code != 200:
        raise Exception(FeedbackManager.error_pushing_pipe(pipe=p['name'], error=data.get('error', 'Unknown error')))
    ...
    if datasource and populate:
        ...
        if not is_materialized and not skip_tokens and not as_standard and not data.get('endpoint'):
            ...
            r = await requests_put(f"{host}/v0/pipes/{p['name']}/endpoint?{urlencode(cli_params)}", headers=headers, data=endpoint_node.get('id'))
            if r.status_code != 200:
                raise Exception(FeedbackManager.error_creating_endpoint(node=endpoint_node.get('name'), pipe=p['name'], error=r.json()['error']))
...
def format_datasource(filename):
    ...
    format_comment(file_parts, filename)
...
def format_comment(file_parts, filename):
    comment = f'# file automatically formatted with `tb format {filename}`'
    file_parts.append(comment)
    file_parts.append(DATAFILE_NEW_LINE)
    file_parts.append(DATAFILE_NEW_LINE)
    return file_parts
