...
class RGBDCamera:
    def __init__(self, pybullet, robot, **kargs):
        ...
        rgb_frame_id = rospy.get_param('~rgbd_camera/rgb_frame_id', "kinect_rgb_optical_frame")
        depth_frame_id = rospy.get_param('~rgbd_camera/depth_frame_id', "kinect_depth_optical_frame")
        if not rgb_frame_id:
            rospy.logerr('Required parameter rgbd_camera/rgb_frame_id not set, will exit now...')
            rospy.signal_shutdown('Required parameter rgbd_camera/rgb_frame_id not set')
            return
        if not depth_frame_id:
            rospy.logerr('Required parameter rgbd_camera/depth_frame_id not set, will exit now...')
            rospy.signal_shutdown('Required parameter rgbd_camera/depth_frame_id not set')
            return
        # get pybullet camera link id from its name
        link_names_to_ids_dic = kargs['link_ids']
        if not rgb_frame_id in link_names_to_ids_dic:
            rospy.logerr('Camera reference rgb frame "{}" not found in URDF model'.format(rgb_frame_id))
            ...
            return
        if not depth_frame_id in link_names_to_ids_dic:
            rospy.logerr('Camera reference depth frame "{}" not found in URDF model'.format(depth_frame_id))
            rospy.logwarn('Available frames are: {}'.format(link_names_to_ids_dic))
            rospy.signal_shutdown('required param rgbd_camera/frame_id not set properly')
            return
        self.pb_camera_link_id = link_names_to_ids_dic[rgb_frame_id]
        self.rgb_image_msg.header.frame_id = rgb_frame_id
        self.pb_camera_link_id = link_names_to_ids_dic[depth_frame_id]
        self.depth_image_msg.header.frame_id = depth_frame_id
        # create publisher
        # rgb_image
        self.pub_rgb_image = rospy.Publisher('kinect/rgb/image_raw', Image, queue_size=10)
        self.rgb_image_msg.encoding = rospy.get_param('~rgbd_camera/resolution/rgb_encoding', 'rgb8')
        self.rgb_image_msg.is_bigendian = 0
        self.rgb_image_msg.step = 1920
        # depth_image
        self.pub_depth_image = rospy.Publisher('kinect/depth/image_raw', Image, queue_size=10)
        self.depth_image_msg.encoding = rospy.get_param('~rgbd_camera/resolution/depth_encoding', '32FC1')
        self.depth_image_msg.is_bigendian = 0
        self.depth_image_msg.step = 2560
        # projection matrix
        ...
        with open(depth_file, "r") as depth_file_handle:
            depth_data = yaml.load(depth_file_handle)
        ...
        with open(rgb_file, "r") as rgb_file_handle:
            rgb_data = yaml.load(rgb_file_handle)
    ...
    def extract_depth_frame(self, camera_image):
        ...
        # return frame
        return depth_img.astype(np.uint32)
