...
def make_column_list(
    column_1: list[Block], column_2: list[Block]
) -> ColumnListBlock:
...
def make_callout(
    ...
    callout = CalloutAttributes(
        rich_text=[rich_txt], icon=Emoji(emoji=emoji), color=color
    )
...
def make_column_cell(
    ...
    column_cell = ColumnCellBlock(
        text=txt_content, annotations=annotations, href=href
    )
    return column_cell.dict()


def make_table_row_block(
    column_cells: list[list[ColumnCellBlock]],
) -> TableRowBlock:
    """Make table rows
...
def make_database(page_id: str) -> DatabaseBlock:
    ...
    database = DatabaseBlock(
        parent=parent, title=database_title, properties=properties
    )
