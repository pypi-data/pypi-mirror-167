        ...
        async with aiosqlite.connect(self._database_name) as db:
            async with db.cursor() as cursor:
                getUser = await cursor.\
                    execute(f"SELECT {self._primary_key} FROM {self._table_name} WHERE {self._primary_key} = ?", (my_id,))
                ...
                if isUserExists:
                    for key, val in dictionary.items():
                        if str(val).startswith("[") or str(val).startswith("{") or str(val).startswith("("):
                            ...
                            pass
                        await cursor.execute(f"UPDATE {self._table_name} SET {key} = ? WHERE {self._primary_key} = ?", (val, my_id,))
                else:
