...
from solanasdk.blockhash import Blockhash
...
from solanasdk.rpc.types import RPCResponse, TxOpts
...
from solanasdk.utils.helpers import ui_amount_to_amount
...
class Token(_TokenCore):  # pylint: disable=too-many-public-methods
    ...
    def spl_transfer(
        ...
        if self.decimals:
            ...
            decimals = self.get_mint_info().decimals

        amount = ui_amount_to_amount(uiamount, decimals)

        sender_ata = spl_token.get_associated_token_address(source, self.pubkey)
        dest_ata = spl_token.get_associated_token_address(dest, self.pubkey)
        create_ata = self._conn.get_account_info(dest_ata)["result"]["value"] is None

        txn, signers, opts = self._transfer_with_ata(
            source,
            sender_ata,
            dest,
            dest_ata,
        ...
        txn.fee_payer = self.payer.public_key
        signers.append(self.payer)
