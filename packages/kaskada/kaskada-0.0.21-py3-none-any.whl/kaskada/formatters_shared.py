...
from domonic.html import pre, table
...
def get_materialization_html_and_schema_df(obj):
    ...
    if hasattr(obj, "destination") and obj.HasField("destination"):
        ...
        if hasattr(obj.destination, "redis_a_i") and obj.destination.HasField("redis_a_i"):
            redis_a_i = table(_class="kda_table")
            appendHtmlObjTableRowIfAttrExists(redis_a_i, obj.destination.redis_a_i, "host")
            appendHtmlObjTableRowIfAttrExists(redis_a_i, obj.destination.redis_a_i, "port")
            appendHtmlObjTableRowIfAttrExists(redis_a_i, obj.destination.redis_a_i, "db")
...
def get_table_html_and_schema_df(obj):
    ...
    if hasattr(obj, "table_source") and obj.HasField("table_source"):
        if hasattr(obj.table_source, "iceberg") and obj.table_source.HasField("iceberg"):
            iceberg = table(_class="kda_table")
            appendHtmlObjTableRowIfAttrExists(iceberg, obj.table_source.iceberg, "table_name")
            appendHtmlObjTableRowIfAttrExists(iceberg, obj.table_source.iceberg, "namespace")
            if hasattr(obj.table_source.iceberg, "config") and obj.table_source.iceberg.HasField("config"):
                config = table(_class="kda_table")
                if hasattr(obj.table_source.iceberg.config, "glue") and obj.table_source.iceberg.config.HasField(
                    "glue"
                ):
                    ...
                    if hasattr(
                        ...
                        appendHtmlObjTableRowIfAttrExists(
                            warehouse, obj.table_source.iceberg.config.glue.warehouse, "bucket"
                        )
                        appendHtmlObjTableRowIfAttrExists(
                            warehouse, obj.table_source.iceberg.config.glue.warehouse, "region"
                        )
                        appendHtmlObjTableRowIfAttrExists(
                            warehouse, obj.table_source.iceberg.config.glue.warehouse, "prefix"
                        )
