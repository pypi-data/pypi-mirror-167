...
#                     return pd.DataFrame(columns=results)
    ...
    async def async_query(self, query) -> pd.DataFrame:
    ...
    def field_permission_check(self, object) -> dict[str, pd.DataFrame]:
        fls_results = transform_sf_result_set_rec(self.sf.query_all(f"""
                                            SELECT Field,ParentId,PermissionsEdit,PermissionsRead,SobjectType FROM FieldPermissions
                                            WHERE SobjectType in ('{object}')
                                            """)['records'])

        permissionSetProfiles = transform_sf_result_set_rec(self.sf.query_all("""
                                                    SELECT Id,Description,IsOwnedByProfile,Name,ProfileId, Profile.Name, Profile.Description,Type FROM PermissionSet
                                                    """)['records'])
    ...
    def get_network_admin_sf_user_info(self):
        sfOrgId = self.instance.split("--")[0]
        
        if self.veeva_common is None:
            self.load_veeva_common()
        
        try:
            response = requests.get(f"{self.veeva_common['Veeva_Server_vod__c'][0]}/{self.veeva_common['Veeva_Version_vod__c'][0]}?VER={self.veeva_common['Veeva_Version_vod__c'][0]}&SSID={self.session_id}&url=https://{self.instance}/services/Soap/u/24.0/{sfOrgId}&ses={self.session_id}&oType=networkAdmin&event=getSFCredentials")
            return response.json()
        except:
            return {"error": "Unable to get network admin credentials"}
    ...
    def get_network_admin_network_user_info(self):
        ...
        try:
            response = requests.get(f"{self.veeva_common['Veeva_Server_vod__c'][0]}/{self.veeva_common['Veeva_Version_vod__c'][0]}?VER={self.veeva_common['Veeva_Version_vod__c'][0]}&SSID={self.session_id}&url=https://{self.instance}/services/Soap/u/24.0/{sfOrgId}&ses={self.session_id}&oType=networkAdmin&event=getNetworkCredentials")
            return response.json()
        except:
            return {"error": "Unable to get network admin credentials"}
