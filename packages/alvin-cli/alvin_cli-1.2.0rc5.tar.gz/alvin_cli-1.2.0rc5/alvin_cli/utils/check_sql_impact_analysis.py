...
from alvin_cli.utils.common_arguments import PLATFORM_ID_FROM_CONFIG
...
def __call_impact_analysis_api(
    ...
    try:
        ...
        # if env variable doesn't have it, fetch platform_id from the config
        platform_id = (
            settings.alvin_platform_id
            if settings.alvin_platform_id
            else PLATFORM_ID_FROM_CONFIG
        )
        ...
        impact_analysis_request = ImpactAnalysisRequest(
            query=impact_analysis_query_request,
            platform=impact_analysis_platform_request,
        )
        ...
        print(f"Platform ID passed is : {platform_id}, enter a valid value")
        exit(1)
