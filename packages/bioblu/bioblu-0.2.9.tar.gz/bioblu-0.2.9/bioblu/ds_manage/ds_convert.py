...
def create_labelme_json_from_yolo(fpath_yolo_txt: str, fpath_img: str, materials_dict: dict,
    ...
    if len(annotation.shapes) > 0:
        ...
        print(f"Created json at: {json_target_path}")
    else:
        print(f"No json file created for {fname_base_txt} because no labels.")
...
def create_yolo_annotations(labelme_path, save_materials_dict=False):
    """
    Non-recursive!\n
    ...
    """
    materials: list = ds_annotations.get_materials_from_labelme_jsons(labelme_path, recursive=False)
    materials_dict = ds_annotations.create_materials_dict(materials)
    print('Creating yolo annotation files...')
    pairs = get_img_and_json_file_pairs(labelme_path)
...
#
#     # if fpath_dst_mats_dict is None:
#
...
def create_yolo_dataset(fdir_src: str, fdir_dst: str = None,
    ...
    if ds_annotations.all_imgs_have_yolo_annotation(fdir_src):
        ...
        create_yolo_annotations(fdir_src)
...
def recreate_labelme_set_from_yolo(yolo_ds_root, target_dir, exist_ok=False):
    ...
    if os.path.exists(mats_dict_fpath):
        materials_dict = ds_annotations.load_json(mats_dict_fpath)
    else:
    ...
    for i, (fpath_img, fpath_txt) in enumerate(img_txt_pairs):
        ...
        create_labelme_json_from_yolo(fpath_txt, fpath_img,
                                      materials_dict=materials_dict, save_dir=target_dir, overwrite=exist_ok)
...
if __name__ == "__main__":
    ...
    src = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_13/"
    t0 = time.perf_counter()
    create_yolo_annotations(src)
    t1 = time.perf_counter()
    print(f"Time elapsed during yolo label creation: {(t1 - t0) / 60} minutes.")
    # create_yolo_dataset(src, exist_ok=True)
