...
def create_fallback_yolo_materials_dict(yolo_root_dir) -> dict:
    ...
    print(f"Fallback materials dict:\n{json.dumps(materials_dict, indent=2)}")
...
def delete_jsons_without_matching_img(fdir):
    pass  # ToDo: Implement.
...
def get_files_with_OOB_yolo_boxes(fdir) -> dict:

    materials_dict = create_fallback_yolo_materials_dict(fdir)

    invalid_bboxes = {}
    txt_files = get_paths_to_txt_files(fdir)
    img_files = [get_corresponding_file(fpath, fdir, YOLO_IMG_FORMATS) for fpath in txt_files]
    for txt_file, img_file in zip(txt_files, img_files):
        annotations = load_yolo_annotations_as_BBoxes(txt_file, img_file, materials_dict)
        for bbox in annotations:
            if not bbox_conversions.yolo_bbox_is_within_bounds(bbox.bbox):
                print(f"Invalid box {bbox} for file {txt_file}")
                invalid_bboxes[txt_file] = bbox
...
if __name__ == '__main__':
    ...
    # n_annot_train = count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_01/labels/train")
    # n_annot_val = count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_01/labels/valid")
    # n_annot_test = count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_01/labels/test")
    # for dset, n_annot in {"TRAIN": n_annot_train, "VALID": n_annot_val, "TEST": n_annot_test}.items():
    #     print(f"{dset}:\t{n_annot}")
    # line = "0 0.956771 0.974537 0.08125 0.0509259"
    # line_dict = load_yolo_line_as_dict(line)
    # print(line_dict)

    # fdir = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_13/"
    # # fdir = "/media/findux/DATA/Documents/Malta_II/datasets/red_sea_shapes/"
    # ds_summary = summarize_labelme_dataset(fdir)
    # print(json.dumps(ds_summary, indent=2))
    # [print(_) for _ in find_labelme_files_with_category(fdir, "Unlabeled litter")]

    # json_fpath = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_13_yolo/materials_dict.json"
    # materials_dict = load_json(json_fpath)
    # print(set_mats_dict_keys_to_int(materials_dict))

    # get_files_with_OOB_yolo_boxes("/media/findux/DATA/Documents/Malta_II/datasets/dataset_13/")
    # ds13_summary = summarize_labelme_dataset("/media/findux/DATA/Documents/Malta_II/datasets/dataset_13/")
    # print(json.dumps(ds13_summary, indent=2))

    fdir = "/media/findux/DATA/Documents/Malta_II/datasets/kaggle_garbage_classification/garbage classification/Garbage classification/metal/"
    fdir = "/media/findux/DATA/Documents/Malta_II/datasets/kaggle_garbage_classification/garbage classification/Garbage classification/plastic/"
    fdir = "/media/findux/DATA/Documents/Malta_II/datasets/kaggle_garbage_classification/garbage classification/Garbage classification/glass/"
    add_file_prefix(fdir, "kaggle_")