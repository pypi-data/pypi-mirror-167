...
class BasicModelTrainer(ABC):
    ...
    @abstractmethod
    def get_model_feedback(self, task) -> ModelFeedback:
        pass
    ...
    def train(
        ...
        model_registry_name = txp.common.utils.model_registry_utils.get_gcs_dataset_prefix(dataset_name, dataset_versions)
        ...
        if error_feedback is not None:
            txp.common.utils.model_registry_utils.set_error_ml_model(firestore_db, tenant_id, machine_id, task_id, model_registry_name,
                                                                     error_feedback, self.models_registry_collection_name)
            firestore_db.close()
            raise exception
        ...
        os.remove(file_path)
        feedback = self.get_model_feedback(task)
        txp.common.utils.model_registry_utils.acknowledge_ml_model(firestore_db, tenant_id, machine_id, task_id, model_registry_name,
    ...
    def publish_model(self, tenant_id, machine_id, task_id, dataset_name, dataset_versions):
        ...
        deprecation_res = txp.common.utils.model_registry_utils.deprecate_active_ml_model(firestore_db, tenant_id, machine_id, task_id,
                                                                                          self.models_registry_collection_name)
        if deprecation_res:
        ...
        model_registry_name = txp.common.utils.model_registry_utils.get_gcs_dataset_prefix(dataset_name, dataset_versions)
        publish_res = txp.common.utils.model_registry_utils.publish_ml_model(firestore_db, tenant_id, machine_id, task_id, model_registry_name,
                                                                             self.models_registry_collection_name)
        if not publish_res:
        ...
        firestore_db = firestore.Client(credentials=self.credentials, project=self.credentials.project_id)
        txp.common.utils.model_registry_utils.update_model_in_task(firestore_db, tenant_id, machine_id, task_id, gcs_file_path)
