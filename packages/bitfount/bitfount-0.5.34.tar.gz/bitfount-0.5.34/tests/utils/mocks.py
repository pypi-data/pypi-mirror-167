...
from bitfount.utils.concurrency_utils import asyncify
    ...
    async def get_message(self, mailbox_id: str) -> _BitfountMessage:
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"Retrieved message ({message.message_type}) from {mailbox_id}"
                )
                return message
            except queue.Empty:
...
class LocalMessageService(_MessageService):
    ...
    async def poll_for_messages(
        ...
        while not stop_event.is_set():
            ...
            stop_fut = asyncify(stop_event.wait)
            get_message_task = asyncio.create_task(
                self._shared_queues.get_message(mailbox_id)
            )

            awaitables: List[Awaitable] = [get_message_task, stop_fut]
