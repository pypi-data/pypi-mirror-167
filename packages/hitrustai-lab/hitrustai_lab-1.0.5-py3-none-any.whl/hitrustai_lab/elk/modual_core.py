...
class UpdateELKAPI:
    ...
    def get_update_data(self):
        ...
        if info_start_point is None:
            info_data = self.orm.query_filter(self.dict_arg["orm_predict_info"], self.n_rows)
            result_data = self.orm.query_filter(self.dict_arg["orm_predict_result"], self.n_rows)
        else:
            info_data = self.orm.query_filter(self.dict_arg["orm_predict_info"], self.n_rows, (dict_arg["orm_predict_info"].pk_id > info_start_point))
            result_data = self.orm.query_filter(self.dict_arg["orm_predict_result"], self.n_rows, (dict_arg["orm_predict_result"].pk_id > result_start_point))
        ...
        if info_data.shape[0] == 0 and result_data.shape[0] == 0:
            ...
            predict_data = self.orm.query_filter(self.dict_arg["orm_predict_result"], self.n_rows, (dict_arg["orm_predict_result"].fd_info_id.in_(info_data['pk_id'].tolist())))
    ...
    def preprocess(self, info_data, result_data):
        merge_data = pd.merge(info_data, result_data, left_on=['pk_id'], right_on=['fd_info_id'], how='inner')
        merge_data = merge_data.reset_index(drop=True)

        # column rename
        merge_data.rename(columns={"pk_id_x": "pk_id", "create_time_y": "create_time"}, inplace=True)
        merge_data.drop(["pk_id_y", "create_time_x"], axis=1, inplace=True)
