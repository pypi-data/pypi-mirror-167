...
class ManagedFsMixin:
    ...
    def custom_query(cls, session: Union[Session, sessionmaker]) -> Query:
        ...
        q = q.add_columns(case((or_(cls.policies == None, cast(cls.policies, String) == 'null'), ''),
                          ...
                          .label("policies_name"),
                          case((cls.instance_tags == None, ''), else_=func.replace(cast(cls.instance_tags.op('->')('Name'), String), "\"", ""))
                               .label('instance_name'),
                          func.array(subq.scalar_subquery().where(cls.fs_id == clsb.fs_id)).label('instance_tags_keys')
                          )
        return q
