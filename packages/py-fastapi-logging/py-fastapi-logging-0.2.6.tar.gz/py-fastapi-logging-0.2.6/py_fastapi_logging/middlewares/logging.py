...
from py_fastapi_logging.middlewares.data_filter import DataFilter
...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if not is_excluded_request:
            ...
            self._data_filter.filter_request_payload(request_log_payload, request.headers)
        ...
        if response.status_code >= status.HTTP_400_BAD_REQUEST or not is_excluded_request:
            ...
            self._data_filter.filter_response_payload(response_log_payload)
