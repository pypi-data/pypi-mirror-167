...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    _filtered_fields: frozenset[str]
    ...
    def __init__(
        ...
        *,
        ...
        self._filtered_fields = frozenset(filtered_fields or ())
    ...
    def _filter_private_data(self, payload: _Payload) -> _Payload:
        ...
        match payload:
            case {"method": _, "path": _, "host": _, "params": params, "body": str(body)} as value:
                ...
                payload["params"] = self._filter_params(cast(_Params, params))
                payload["body"] = self._filter_body(body)
            case {"status": _, "response_time": _, "response_body": str(body)} as value:
                payload = cast(ResponsePayload, value)
                payload["response_body"] = self._filter_body(body)

        return payload

    def _filter_params(self, params: _Params) -> _Params:
        if not params:
            return params

        new_params: dict[str, Any] = params.copy()
        for key in params:
            if key in self._filtered_fields:
                new_params[key] = self._marker_filtered

        return new_params

    def _filter_body(self, body: str) -> str:
        try:
            request_json: Any = json.loads(body)
            response_json: Any = self._filter_json_body(request_json)
            return json.dumps(response_json, ensure_ascii=False, separators=(",", ":"))
        except Exception:
            return body

    def _filter_json_body(self, body: Any) -> Any:
        if isinstance(body, MutableMapping):
            return {
                key: self._filter_json_body(value) if key not in self._filtered_fields else self._marker_filtered
                for key, value in body.items()
            }
        elif isinstance(body, MutableSequence):
            return list(map(self._filter_json_body, body))
        else:
            return body
