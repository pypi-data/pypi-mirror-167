    ...
    def update(self, data: Dict[str, Any] = {}):
        ...
        if (repl_data["owner"]):
            ...
            if (repl_data["database"]):
                self.database = db
                self.database.db_url = repl_data["database"]["jwt"]
            # if (self.owner.username != self.c.user.username):
            #     for p in ['folderId', 'multiplayerInvites', 'pinnedToProfile', 'powerUpCosts', 'delete', 'change', 'files', 'database']:
            #         delattr(self, p)
...
class Comment:
    ...
    def reply(self, body: str, **options: Dict[str, Any]):
        ...
        res = post(self.c.sid, "sendReplCommentReply", vars = {
            "input": {
                "body": (f"@{self.user.username} {body}" if (getattr(options, "mention", False) and options.mention) else body),
