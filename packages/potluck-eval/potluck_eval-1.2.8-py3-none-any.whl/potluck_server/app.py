...
def route_deliver(course, semester):
    ...
    status, ecredit, _ = exercise_credit(einfo, outcomes)

    # Record the outcomes list for each author
    for author in authors:
    ...
    ).format(len([x for x in outcomes if x[0]]), len(outcomes), status)
    if status != "complete":
...
def route_exercise(
    ...
    outcomes = storage.fetch_best_outcomes(course, semester, username)
...
def exercise_credit(exercise_info, outcomes):
    ...
    individual exercise dictionary).
...
def set_concept_statuses(concepts, task_info, outcomes):
    ...
    exercise outcomes provided. Also updates the task info with time +
    submission statuses for each exercise.
    ...
    In the task info, each exercise group will gain a "phase" slot with
    a string indicating which time phase it's in (see the "exercises"
    entry above). Also, each individual exercise which has been submitted
    will gain a "status" slot with a submission status string (also see
    above). Finally, the exercise groups will gain a "status" slot
    aggregating the statuses of its components, depending on its phase.
    This group status is determined as follows:
    ...
    The concepts list must already be augmented via `augment_concepts`.
    """
    # Get current time:
    ...
    for group in task_info.get("exercises", []):
        ...
        for einfo in elist:
            ...
            # Track credit (full/partial) for this exercise
            ecredit = None

            # If it hasn't been submitted, note that
            if outcomes_here is None:
                submission_status = "unsubmitted"
            else:
                # Get submission status & exercise credit
                submission_status, ecredit, gcredit = exercise_credit(
                    einfo,
                    outcomes_here
                )
                # Accumulate group credit
                group_credit += gcredit
