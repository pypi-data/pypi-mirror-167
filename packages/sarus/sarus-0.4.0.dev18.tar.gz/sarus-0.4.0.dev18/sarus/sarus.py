...
class Dataset:
    ...
    @staticmethod
    def matches_names(name, metadata):
        """
        returns True is the provided name targets the name in MetaData
        It allows for not checking the first namespace
        """
        split_arg_name = name.split(".")
        split_metadata_name = metadata["name"].split(".")
        if len(split_arg_name) == len(split_metadata_name):
            return name == metadata["name"]
        elif (
            len(split_arg_name) == len(split_metadata_name) - 1
        ):  # first namespace was omitted
            return name == ".".join(split_metadata_name[1:])
        else:
            return False
    ...
    def table(self, name: str) -> Table:
        ...
        Args:
            name (str): Name of a form `namespace_1.namespace_2....namespace_n.table_name`.
        ...
        type_metadata = [
            ...
            if Dataset.matches_names(name, meta)
        ]

        if len(type_metadata) != 1:
            raise ValueError("Invalid or ambiguous table name given")
        namespaces = name.split(".")
        paths = self._dataspec.schema().tables()

        if len(paths) == 1:
            ds_filter = self._dataspec.schema().data_type().get(paths[0])
            filtered_ds = sarus_data_spec.transform.filter(filter=ds_filter)(
                self._dataspec
            )
            selected_ds = sarus_data_spec.transform.get_item(path=paths[0])(
                filtered_ds
            )

            return Table(selected_ds, type_metadata[0])

        path = _find_path_by_namespaces(namespaces, paths)
        ds_filter = self._dataspec.schema().data_type().get(path)
        filtered_ds = sarus_data_spec.transform.filter(filter=ds_filter)(
            self._dataspec
        )
        selected_ds = sarus_data_spec.transform.get_item(path=path)(
            filtered_ds
        )
        return Table(selected_ds, type_metadata[0])
    ...
    def tables(self):
        return _get_tables(self.type_metadata, self._dataspec)
...
class Client:
    ...
    def table(self, name: str) -> Table:
        """Select a table from a Sarus Dataset.

        Args:
            name (str): This should be of a form `dataset_slugname.namespace1.namespace2...namespace_n.table_name`.
                Namespaces are optional if they are not ambiguous.

        Returns:
            Table: The selected Sarus Table.
        """
        slug = name.split(".")[0]
        namespace = name[len(slug) + 1 :]
        return self.dataset(slugname=slug).table(name=namespace)
...
class Table:
    ...
    def __init__(self, filtered_dataspec, metadata):
        if metadata["name"][-4:] != ".csv" or "/" not in metadata["name"]:
            self.name: str = metadata["name"]
        else:
            self.name = metadata["name"].split(".")[-2].split("/")[-1]
        self._dataspec = filtered_dataspec
        self.type_metadata = metadata
    ...
    @property
    def features(self):
        return self.type_metadata["features"]
...
def _get_tables(type_metadatas, dataspec) -> List[Table]:
    """For given parameters of a Sarus Dataset return a list of Sarus Tables."""
    table_paths = [path for path in dataspec.schema().tables()]
    if len(table_paths) == 1:
        _filter = dataspec.schema().data_type().get(table_paths[0])
        filtered_dataspec = sarus_data_spec.transform.filter(filter=_filter)(
            dataspec
        )
        return [Table(filtered_dataspec, type_metadatas[0])]
    matched_paths = []
    for metadata in type_metadatas:
        path = _find_path_by_namespaces(
            metadata["name"].split("."), table_paths
        )
        matched_paths.append((metadata, path))
    matched_filters = [
        (matched_path[0], dataspec.schema().data_type().get(matched_path[1]))
        for matched_path in matched_paths
    ]
    matched_dataspecs = [
        (
            matched_filter[0],
            sarus_data_spec.transform.filter(filter=matched_filter[1])(
                dataspec
            ),
        )
    ...
    return [
        Table(filtered_dataspec, type_metadata)
        for type_metadata, filtered_dataspec in matched_dataspecs
    ]


def _find_path_by_namespaces(
    namespaces: List[str], paths: List[st.Path]
) -> st.Path:
    """Given a table namespace address and a list of Paths find a Path that corresponds to that table."""
    fitting_paths = []
    for path in paths:
        common_namespaces = [
            i
            for i, j in zip(
                namespaces,
                path.to_strings_list()[0][
                    len(path.to_strings_list()[0]) - len(namespaces) :
                ],
            )
            if i == j
        ]
        if len(common_namespaces) == len(namespaces):
            fitting_paths.append(path)
    if len(fitting_paths) > 1 or len(fitting_paths) == 0:
        raise ValueError("Invalid table name/namespace given.")
    else:
        return fitting_paths[0]
