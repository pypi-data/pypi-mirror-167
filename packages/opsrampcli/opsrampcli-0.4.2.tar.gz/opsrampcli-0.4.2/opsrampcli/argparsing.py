...
def do_arg_parsing():
    ...
    env_parser.add_argument('--env', required=True, help='Name of environment to use, as defined in your environments.yml file')
    env_parser.add_argument('--secure', default=True, help='Whether or not to verify SSL cert')
    env_parser.add_argument('--envfile', default='environments.yml', help='Location of environments YAML file')
    ...
    parser_postalerts = subparsers.add_parser('postalerts', parents=[env_parser], formatter_class=argparse.ArgumentDefaultsHelpFormatter, help="Post alerts to a Webhook integration (using vtoken) or directly to the API (using OAuth creds)")
    parser_postalerts.add_argument('--infile', required=True, help='File containing an array of json alert payloads')
    parser_postalerts.add_argument('--range', default='all', help='An integer or range identifying which alert in the file to send')
    parser_postalerts.add_argument('--auth', default='vtoken', help='Authentication method: vtoken or oauth')
