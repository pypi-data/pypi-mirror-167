    ...
    if command == "stop":
        ...
        if len(audio_devices):
            for ad in audio_devices:
                click.echo(f"🛑 {red('stopping')} ➡️ all tracks on  {yellow('audio_device')}: {blue(ad)}", err=True)
            ...
            if not len(tracks):
                tracks = ["*"]
            for track in tracks:
                click.echo(f"🛑 {red('stopping')} ➡️ {yellow('track')}: { blue(track)}", err=True)
...
def run_track_func(item):
    ...
    if command == "stop":
        colorfx = red
    click.echo(f"▶️ {colorfx(f'{command}ing')} ➡️ {yellow('track')}: {blue(track).ljust(26)} {yellow('audio_device')}: {blue(ad or 'default').ljust(20)} {yellow('parent_pid')}: {blue(pid)}", err=True)
