    ...
    if command == "stop":
        ...
        if len(audio_devices):
            for ad in audio_devices:
                click.echo(f"Stopping {seq} tracks on audio device {ad}", err=True)
            ...
            if not len(tracks):
                tracks = ["*"]
            for track in tracks:
                click.echo(f"Stopping {seq} track: {track}", err=True)
...
def run_track_func(item):
    ...
    click.echo(f"Starting track {track} on audio_device: {ad or 'default'} with parent_pid: {pid}", err=True)
