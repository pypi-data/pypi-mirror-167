...
class App(MiniumObject):
    ...
    def mock_choose_image(self, name: str, image_b64data: str):
        ...
        if name in self._mocked_images:  # 命中缓存
            self._evaluate_js(
                "evalMockChooseImage",
                [{
                    "imageName": name,
                    "size": self._mocked_images[name]
                }]
            )
            return True
        ret = self._evaluate_js(
            ...
            [{
                "imageName": name,
                "imageData": image_b64data
            }]
        )
        self._mocked_images[name] = ret["size"]
        return True
    ...
    def mock_choose_image_with_name(self, file_name):
        ...
        self._mocked_images[file_name] = ret["size"]
        return True
