...
#     data = get_py_file_meta_data(r"E:\Workspace\Python\fat-wheel\setup.py")
#     for d in data:
#         print(f"{d.__dict__.get('arg')} : {d.__dict__.get('value')}")

# Code = namedtuple("Code", ("code", "lineno", "endlineno"))
#
#
# class PyParser:
#
#     @staticmethod
#     def parse_import(data: ast.Import):
#         imports = []
#         for imp in data.names:
#             imports.append(Code(f"import {imp.name}", 0, data.end_lineno))
#         return imports
#
#     @staticmethod
#     def parse_import_from(data: ast.ImportFrom):
#         modulename = data.module
#         from_imports = []
#         for imp in data.names:
#             from_imports.append(Code(f"from {modulename} import {imp.name}", 0, data.end_lineno))
#         return from_imports
#
#     @staticmethod
#     def parse_list(data: ast.List):
#         pass