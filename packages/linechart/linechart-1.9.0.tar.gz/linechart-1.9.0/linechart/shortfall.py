...
class ShortfallTearsheet(BaseTearsheet):
    ...
    def from_csvs(
        ...
        return t.create_full_tearsheet(
            x_perf, y_perf, largest_n=largest_n, shift_x_positions=shift_x_positions
        )

    def create_full_tearsheet(
        self, x_performance, y_performance, largest_n=None, shift_x_positions=0
    ):
        """
        ...
        if largest_shortfalls is not None:
            ...
            self._plot_field(
                x_performance.returns.where(largest_shortfalls_x_mask.isnull(), 0).sum(
                    axis=1
                ),
                y_performance.returns.where(largest_shortfalls_y_mask.isnull(), 0).sum(
                    axis=1
                ),
                fig_title="Returns",
                fig_title_suffix=" excluding Top {} Largest Shortfalls".format(
                    largest_n
                ),
                cumplots=True,
            )
        ...
        # Commissions
        if (
            x_agg_performance.commissions is not None
            and y_agg_performance.commissions is not None
        ):
            self._plot_field(
        ...
        # Total holdings
        if (
            x_agg_performance.total_holdings is not None
            and y_agg_performance.total_holdings is not None
        ):
            self._plot_field(
        ...
        # Net exposures
        if (
            x_agg_performance.net_exposures is not None
            and y_agg_performance.net_exposures is not None
        ):
            self._plot_field(
        ...
        # Absolute exposures
        if (
            x_agg_performance.abs_exposures is not None
            and y_agg_performance.abs_exposures is not None
        ):
            self._plot_field(
        ...
        # Turnover
        if (
            x_agg_performance.turnover is not None
            and y_agg_performance.turnover is not None
        ):
            self._plot_field(
    ...
    def create_largest_shortfalls_tearsheet(
        self, x_performance, y_performance, largest_n=5
    ):
        """
        ...
        shared_columns = x_performance.returns.columns.intersection(
            y_performance.returns.columns
        )
        ...
        if returns.empty:
            warnings.warn(
                "skipping largest shortfalls table because no shortfall found"
            )
            return None

        largest_shortfalls = (
            returns.sort_values("abs_shortfall", ascending=False)
            .drop("abs_shortfall", axis=1)
            .head(largest_n)
        )
        ...
        largest_shortfalls.insert(
            ...
            largest_shortfalls.Date.dt.strftime("%Y-%m-%d").str.cat(
                largest_shortfalls.Sid, sep=" "
            ),
        )
        ...
        with sns.axes_style("white"):
            ...
            table = axis.table(
                ...
                cellText=[largest_shortfalls_dict["columns"]]
                + largest_shortfalls_dict["data"],
                loc="center",
            )
    ...
    def _plot_field(
        ...
        if cumplots:
            ...
            with_baseline(cum_results).plot(
                ax=ax1, title="Cumulative " + fig_title + fig_title_suffix
            )
        ...
        if format_as_pct_decimal_places is not None:
            self._y_format_as_percentage(
                axis, max_decimal_places=format_as_pct_decimal_places
            )
    ...
    def create_summary_tearsheet(self, x_performance, y_performance):
        ...
        stats.append(
            [
                ...
                [
                    round(
                        (
                            x_performance.returns.index.max()
        ...
        stats.append(
            [
                ...
                [
                    "{0}%".format(
                        round(ep.cum_returns_final(x_performance.returns) * 100, 1)
                    ),
                    "{0}%".format(
                        round(ep.cum_returns_final(y_performance.returns) * 100, 1)
                    ),
                ],
            ]
        )
        if (
            x_performance.commissions is not None
            and y_performance.commissions is not None
        ):
            stats.append(
                [
                    ...
                    [
                        "{0}%".format(
                            round(
                                ep.cum_returns_final(x_performance.commissions) * 100, 1
                            )
                        ),
                        "{0}%".format(
                            round(
                                ep.cum_returns_final(y_performance.commissions) * 100, 1
                            )
                        ),
                    ],
                ]
        ...
        stats.append(
            [
                ...
                [
                    "{0}%".format(
                        round(ep.annual_volatility(x_performance.returns) * 100, 1)
                    ),
                    "{0}%".format(
                        round(ep.annual_volatility(y_performance.returns) * 100, 1)
                    ),
                ],
            ]
        )
        ...
        if (
            x_performance.abs_exposures is not None
            and y_performance.abs_exposures is not None
        ):
            stats.append(
                [
        ...
        if (
            x_performance.net_exposures is not None
            and y_performance.net_exposures is not None
        ):
            stats.append(
                [
        ...
        if (
            x_performance.total_holdings is not None
            and y_performance.total_holdings is not None
        ):
            stats.append(
                [
                    ...
                    [
                        "{0}%".format(
                            round(
                                (
                                    x_performance.cagr
                                    / x_performance.abs_exposures.mean()
                                )
                                * 100,
                                1,
                            )
                        ...
                        "{0}%".format(
                            round(
                                (
                                    y_performance.cagr
                                    / y_performance.abs_exposures.mean()
                                )
                                * 100,
                                1,
                            )
