...
class BaseTearsheet(object):
    ...
    def _y_format_at_least_two_decimal_places(self, axis):
        ...
        def format_at_least_two_decimal_places(x, pos):
            if round(x, 2) == round(x, 3):
                ...
                while True:
                    if math.isclose(
                        round(x, decimal_places), round(x, decimal_places + 1)
                    ):
    ...
    def _create_returns_plots(
        self, performance, subplot, extra_label, figsize=None, legend_title=None
    ):
        """
        ...
        with sns.color_palette(color_palette):
            ...
            plot = performance.drawdowns.plot(
                ax=axis, title="Drawdowns {0}".format(extra_label)
            )
            axis.set_xlabel("")
            ...
            if performance.cum_pnl is not None:
                ...
                if (
                    ...
                    try:
                        pnl_breakdown = pd.concat(
                            (cum_pnl, cum_gross_pnl, cum_commissions), axis=1, sort=True
                        )
                    except TypeError:
                        # sort was introduced in pandas 0.23
                        pnl_breakdown = pd.concat(
                            (cum_pnl, cum_gross_pnl, cum_commissions), axis=1
                        )
                    plot = pnl_breakdown.plot(
                        ax=axis, title="Cumulative PNL {0}".format(extra_label)
                    )
                    axis.set_xlabel("")
                    if isinstance(returns_breakdown, pd.DataFrame):
                        self._clear_legend(plot, legend_title=legend_title)
                else:
                    plot = performance.cum_pnl.plot(
                        ax=axis, title="Cumulative PNL {0}".format(extra_label)
                    )
                    axis.set_xlabel("")
            ...
            if (
                len(performance.rolling_sharpe.index)
                > performance.rolling_sharpe_window
            ):
                ...
                plot = performance.rolling_sharpe.plot(
                    ax=axis,
                    title="Rolling Sharpe ({0}-day) {1}".format(
            ...
            if performance.benchmark_returns is not None:
                ...
                try:
                    ...
                    # sort was introduced in pandas 0.23
                    vs_benchmark = pd.concat(
                        (performance.cum_returns, benchmark_cum_returns), axis=1
                    )
