...
from .utils import with_baseline, get_sharpe, get_cagr, get_cum_returns, get_drawdowns
...
class Tearsheet(BaseTearsheet):
    ...
    def create_full_tearsheet(self, performance):
        ...
        if any(
            [
                ...
                for exposures in (performance.net_exposures, performance.abs_exposures)
            ]
        ):
    ...
    def create_summary_tearsheet(self, performance, agg_performance=None):
        ...
        if agg_performance.commission_amounts is not None:
            stats.append(
                ["Commissions", round(agg_performance.commission_amounts.sum(), 2)]
            )

        stats.append(
            ["Start Date", agg_performance.returns.index.min().date().isoformat()]
        )
        stats.append(
            ["End Date", agg_performance.returns.index.max().date().isoformat()]
        )
        ...
        stats.append(
            [
                ...
                round(
                    (
                        agg_performance.returns.index.max()
        ...
        stats.append(
            ["Sortino Ratio", "%.2f" % ep.sortino_ratio(agg_performance.returns)]
        )
        stats.append(
            ["Calmar Ratio", "%.2f" % ep.calmar_ratio(agg_performance.returns)]
        )
        stats.append(["Skew", "%.2f" % scipy.stats.skew(agg_performance.returns)])
        stats.append(
            ["Kurtosis", "%.2f" % scipy.stats.kurtosis(agg_performance.returns)]
        )
    ...
    def create_returns_tearsheet(self, performance, agg_performance=None):
        ...
        if show_details:
            self._create_returns_plots(
                performance, subplot=212, extra_label="(Details)"
            )
    ...
    def _create_detailed_returns_bar_charts(self, performance):
        ...
        if performance.pnl is not None:
            ...
            if performance.commission_amounts is not None:
                ...
                try:
                    pnl = pd.concat(
                        (pnl, gross_pnl, commission_amounts), axis=1, sort=True
                    )
                except TypeError:
    ...
    def _create_positions_plots(self, performance, subplot, extra_label, figsize=None):
        ...
        with sns.color_palette(color_palette):
            ...
            if performance.net_exposures is not None:
                ...
                axis = fig.add_subplot(subplot)
                self._y_format_as_percentage(axis)
                plot = performance.net_exposures.round(2).plot(
                    ax=axis, title="Net Exposure {0}".format(extra_label)
                )
                axis.set_ylabel("Percentage of capital")
                axis.set_xlabel("")
            ...
            if performance.total_holdings is not None:
                ...
                axis = fig.add_subplot(subplot)
                plot = performance.total_holdings.plot(
                    ax=axis, title="Daily Holdings {0}".format(extra_label)
                )
            ...
            if performance.turnover is not None:
                ...
                plot = turnover.plot(
                    ax=axis, title="Daily Turnover {0}".format(extra_label)
                )
                axis.set_ylabel("Percentage of capital")
                axis.set_xlabel("")
    ...
    def _create_detailed_positions_bar_charts(self, performance):
        ...
        with sns.color_palette(color_palette):
            ...
            if performance.abs_exposures is not None:
                ...
                next_pos += 1
                self._y_format_as_percentage(axis)
                avg_abs_exposures.plot(
                    ax=axis, kind="bar", title="Avg Absolute Exposure (Details)"
                )
                axis.set_ylabel("Percentage of capital")
            ...
            if performance.net_exposures is not None:
                ...
                next_pos += 1
                self._y_format_as_percentage(axis)
                avg_net_exposures.plot(
                    ax=axis, kind="bar", title="Avg Net Exposure (Details)"
                )
                axis.set_ylabel("Percentage of capital")
            ...
            if performance.total_holdings is not None:
                ...
                axis = fig.add_subplot(rows, 2, next_pos)
                next_pos += 1
                avg_total_holdings.plot(
                    ax=axis, kind="bar", title="Avg Daily Holdings (Details)"
                )
                axis.set_ylabel("Number of holdings")
            ...
            if performance.turnover is not None:
                ...
                next_pos += 1
                self._y_format_as_percentage(axis)
                avg_daily_turnover.plot(
                    ax=axis, kind="bar", title="Avg Daily Turnover (Details)"
                )
                axis.set_ylabel("Percentage of capital")
    ...
    def create_returns_by_year_tearsheet(self, performance, agg_performance=None):
        ...
        if show_details:
            self._create_returns_by_year_plots(
                performance, rows=2, row=2, fig=fig, extra_label="(Details)"
            )
    ...
    def _create_returns_by_year_plots(self, performance, rows, row, fig, extra_label):
        ...
        sharpes_by_year = grouped_returns.apply(
            get_sharpe, riskfree=performance.riskfree
        )
        ...
        with sns.color_palette(color_palette):
            ...
            axis.set_ylabel("CAGR")
            self._y_format_as_percentage(axis)
            plot = cagrs_by_year.plot(
                ax=axis, kind="bar", title="CAGR by Year {0}".format(extra_label)
            )
            axis.set_xlabel("")
            ...
            axis.set_ylabel("Sharpe ratio")
            self._y_format_at_least_two_decimal_places(axis)
            plot = sharpes_by_year.plot(
                ax=axis, kind="bar", title="Sharpe by Year {0}".format(extra_label)
            )
            axis.set_xlabel("")
    ...
    def create_montecarlo_tearsheet(
        self, performance, cycles=5, aggregate_before_shuffle=True
    ):
        """
        ...
        for i in range(cycles):
            if aggregate_before_shuffle:
                sim_returns = pd.Series(
                    np.random.permutation(returns), index=returns.index
                )
            else:
