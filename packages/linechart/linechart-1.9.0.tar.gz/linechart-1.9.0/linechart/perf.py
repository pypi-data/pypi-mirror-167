...
class DailyPerformance(object):
    ...
    def _from_moonshot_or_pnl(
        ...
        if "CommissionAmount" in fields:
            kwargs["commission_amounts"] = results.loc["CommissionAmount"].astype(
                np.float64
            )
    ...
    def benchmark_returns(self):
        ...
        if len(have_benchmarks.index) > 1:
            ...
            warnings.warn(
                "Multiple benchmarks found, only using first ({0})".format(col)
            )
    ...
    def benchmark_cum_returns(self):
        ...
        if self._benchmark_cum_returns is None and self._benchmark_returns is not None:
            self._benchmark_cum_returns = get_cum_returns(
                self._benchmark_returns, compound=True
            )
