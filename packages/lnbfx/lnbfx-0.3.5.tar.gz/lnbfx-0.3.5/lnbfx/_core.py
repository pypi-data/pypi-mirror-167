...
class BfxRun:
    def __init__(self, *, pipeline: dict, run_id: str = None):
        ...
        self._run_dir: Optional[Union[Path, str]] = None
    ...
    def run_dir(self):
        ...
        if self._run_dir is None:
            ...
            return Path(self._run_dir)  # type: ignore
    ...
    def link_dobject(self, dobject_id: str, dobject_filepath: Union[str, Path]):
        ...
        if self.run_dir is not None and (
            self.run_dir.as_posix() in Path(dobject_filepath).as_posix()
        ):
        ...
        dobject_dirpath = Path(dobject_filepath).parent.resolve().as_posix()
