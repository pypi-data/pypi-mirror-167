...
def has_internet_connection(url: str) -> bool:
    if urllib3.get_host(url)[1] == "0.0.0.0":
        return True
    try:
        host = socket.gethostbyname(urllib3.get_host(url)[1])
        if urllib3.get_host(url)[0] == "https":
...
def fetch_file(location: str, description: str, output_file: str, quiet: bool = False, sha512sum: str = None) -> bool:
    ...
    if is_url(location):
        if has_internet_connection(location):
            ...
            try:
                ...
                online_size_readable = get_readable_size(online_size * 8)
                ...
                local_size += 1024
                if not quiet:
                    print(f" {Icon.SUCCESS} {__msg} ({get_readable_size(local_size*8)}): 100%")
