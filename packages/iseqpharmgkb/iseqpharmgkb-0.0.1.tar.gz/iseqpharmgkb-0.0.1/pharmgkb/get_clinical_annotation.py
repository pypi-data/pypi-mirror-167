...
def check_for_other_genotypes(variant: str, genotype: str, gene: str, database: sqlite3.Connection, logs: dict):
...
def get_clinical_annotation(queries: dict, database: sqlite3.Connection) -> dict:
    ...
    for variant, genotype in queries.items():
        ...
        if df.empty:
            check_for_other_genotypes(variant, genotype, gene, database, logs)
        else:
