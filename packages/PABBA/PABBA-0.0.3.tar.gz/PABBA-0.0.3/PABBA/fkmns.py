...
def kmeanspp_speedup1(data, k, random_state=2022):
    '''
    ...
    for c_id in range(1, k):
        shortest_distance = calculate_shortest_distance_refine1(data, centers[:c_id])
        centers[c_id] = data[np.argsort(shortest_distance)[data.shape[0]-1], :]
        
    return centers


# initialization algorithm
def kmeanspp_speedup2(data, k, random_state=2022):
    '''
    initialized the centroids for K-means++
    inputs:
        data - numpy array of data points having shape (200, 2)
    ...
    for c_id in range(1, k):
        shortest_distance = calculate_shortest_distance_refine2(data, centers[:c_id])
        centers[c_id] = data[np.argsort(shortest_distance)[data.shape[0]-1], :]
...
class kmeanspp:
    ...
    def fit_predict(self, X):
        init_centers = kmeanspp_speedup1(X, self.n_clusters, self.random_state)
        return self.fit(X).labels_
        
    def fit(self, X):
        self.centers = kmeanspp_speedup1(X, self.n_clusters, self.random_state)
        self.record_iters = 0
        prev_centers = None
        while np.not_equal(self.centers, prev_centers).any() and self.record_iters < self.max_iter:
            self.labels_ = calculate_shortest_distance_label(X, self.centers)
