...
def kmeanspp_speedup1(data, k):
    ...
    for c_id in range(1, k):
        ...
        centers[c_id] = data[np.argsort(shortest_distance)[int(0.9*len(shortest_distance))], :]
        
    return centers


# initialization algorithm
def kmeanspp_speedup2(data, k):
    '''
    initialized the centroids for K-means++
    inputs:
        data - numpy array of data points having shape (200, 2)
...
class kmeans:
    def __init__(self, k=1, max_iter=1):
        self.n_clusters = k
        self.max_iter = max_iter
        self.labels_ = None
        self.record_iters = None # the iterations
        
    def fit_tranform(self, X, init_centers):
