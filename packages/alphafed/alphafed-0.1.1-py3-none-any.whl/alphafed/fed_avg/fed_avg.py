...
class FedAvgScheduler(Scheduler, metaclass=ABCMeta):
    ...
    def _check_and_run_test(self):
        ...
        if (
            ...
            self.push_log(f'The latest test results are: {test_result}')
...
def register_metrics(name: str, keys: Iterable[str]):
    ...
    def decorate(func):
        ...
        def wrapper(_self, *args, **kwargs):
            if not isinstance(_self, FedAvgScheduler) or func.__name__ != 'test':
                raise RuntimeError(
                    'Only applicable on "test" function of a "FedAvgScheduler" object.'
                )
...
class FedSGDScheduler(FedAvgScheduler):
    ...
    def _setup_context(self, id: str, task_id: str, is_initiator: bool = False):
        ...
        self.push_log(f'train examples = {len(train_loader.dataset)}')
        if train_loader.batch_size != len(train_loader.dataset):
