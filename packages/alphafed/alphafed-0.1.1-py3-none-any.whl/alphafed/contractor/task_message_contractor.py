...
from .common import ContractEvent, ContractException, Contractor
...
class TaskMessageContractor(Contractor):
    ...
    _URL = 'http://alphamed-fed-service:8080/fed-service/api/v1/message'
    ...
    def _new_contract(self,
        ...
        assert (
            message_title and message_title in (self._TASK_INSIDE, self._TASK_FINISH)
    ...
    def notice_task_completion(self,
                               aggregator: str,
                               result: bool,
                               report_file: str,
                               model_file: str):
        """Notice task manager a task is complete.
        ...
        assert result is not None and isinstance(result, bool), 'result must be True or False'
        assert (
            report_file and isinstance(report_file, str)
        ...
        if not TxId:
            raise ContractException('failed to notice task completion')
