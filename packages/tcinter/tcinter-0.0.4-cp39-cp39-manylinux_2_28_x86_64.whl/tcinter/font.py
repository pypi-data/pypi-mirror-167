...
class Font:
    ...
    exists -- does a named font by this name already exist?
       ...
       Raises _tcinter.TclError if the assertion is false.
    ...
    def __init__(self, root=None, font=None, name=None, exists=False,
                 **options):
        if root is None:
            root = tcinter._get_default_root('use font')
        ...
        if exists:
            ...
            if self.name not in tk.splitlist(tk.call("font", "names")):
                raise tcinter._tcinter.TclError(
...
def families(root=None, displayof=None):
    ...
    if root is None:
        root = tcinter._get_default_root('use font.families()')
    args = ()
    if displayof:
        args = ('-displayof', displayof)
...
def names(root=None):
    ...
    if root is None:
        root = tcinter._get_default_root('use font.names()')
...
if __name__ == "__main__":
    ...
    w = tcinter.Label(root, text="Hello, world", font=f)
    w.pack()

    w = tcinter.Button(root, text="Quit!", command=root.destroy)
    w.pack()
