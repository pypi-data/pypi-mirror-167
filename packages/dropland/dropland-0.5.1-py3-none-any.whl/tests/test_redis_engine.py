...
async def test_session_connections():
    ...
    session = await redis_storage.session()
    conn_data = redis_storage.get_connection()
    conn_data1 = session.get_connection('dropland1')
    assert redis_storage.manager().get_connection('dropland1') == conn_data == conn_data1
