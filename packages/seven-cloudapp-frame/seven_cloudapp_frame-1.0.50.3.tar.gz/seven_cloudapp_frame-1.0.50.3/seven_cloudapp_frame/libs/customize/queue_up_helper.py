...
@LastEditTime: 2022-09-15 19:37:20
                        ...
                        if hash_value:
                            if first_is_process == True and i == 0:
                                if now_timestamp > hash_value["end_timestamp"]:
                                    is_delete = True
                                    self.logger_error.error("【踢掉办理用户1】" + SevenHelper.json_dumps({"zset_name": zset_name, "user_id": data, "now_timestamp": now_timestamp, "pop_timestamp": hash_value["end_timestamp"]}))
                            elif first_is_process == True and i > 0:
                                if now_timestamp > first_end_timestamp + (i * int(config.get_value("queue_confirm_time", 8))):
                                    is_delete = True
                                    self.logger_error.error("【踢掉办理用户2】" + SevenHelper.json_dumps({"zset_name": zset_name, "user_id": data, "now_timestamp": now_timestamp, "pop_timestamp": first_end_timestamp + (i * int(config.get_value("queue_confirm_time", 8)))}))
                            else:
                                ...
                                if now_timestamp > pop_timestamp + ((i + 1) * int(config.get_value("queue_confirm_time", 8))):
                                    is_delete = True
                                    self.logger_error.error("【踢掉办理用户3】" + SevenHelper.json_dumps({"zset_name": zset_name, "user_id": data, "now_timestamp": now_timestamp, "pop_timestamp": pop_timestamp + ((i + 1) * int(config.get_value("queue_confirm_time", 8)))}))
