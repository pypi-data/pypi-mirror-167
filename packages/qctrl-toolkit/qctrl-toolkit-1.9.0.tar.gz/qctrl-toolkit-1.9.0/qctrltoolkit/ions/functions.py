...
def _molmer_sorensen_quantities(
    graph,
    lamb_dicke_parameters,
    relative_detunings,
    drives,
    ...
    """
    Add the Mølmer–Sørensen phases, displacements, and infidelities to the graph
    and return the infidelities node.
    """
...
def ms_simulate(
    ...
    sample_times = np.linspace(duration / sample_count, duration, sample_count)
    ...
    simulation_result = qctrl.functions.calculate_graph(
        graph=graph, output_node_names=_MS_NODE_NAMES
    )
...
def ms_optimize(
    ...
    infidelities = _molmer_sorensen_quantities(
        graph,
        lamb_dicke_parameters=lamb_dicke_parameters,
        relative_detunings=relative_detunings,
        drives=ion_drives,
