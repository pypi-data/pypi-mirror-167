...
class FeatureTransformer:
    ...
    def _extract_feature_per_shipment_single(self, params, df):
        ...
        num_features = [
            col for col in params["type_dict"] if params["type_dict"][col] == "numerical"
        ]
        # Statistics to aggregate on categorical features
        cat_operations = ["nunique"]
        cat_features = [
            col for col in params["type_dict"] if (params["type_dict"][col] == "categorical") 
            and (col != params["id_field"])
            and (col in df)
        ]
        ...
        params["processed_cat_features"] = [col+"_first" for col in cat_features] + [col+"_last" for col in cat_features]
