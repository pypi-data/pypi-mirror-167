...
def ml_setup(params, num_input, num_output, df):
    ...
    # Allocate here with wanted scikit-learn classifiers
        ...
        if params["ft_encoding"] is not None:
            logging.warning("Note that catboost performs best with no/minimal feature preprocessing. Ohe/pca to categorical features should be avoided.")
        if params["prediction_type"] == "classification":
...
def cat_classification_model_setup(params):
    ...
    # cat_features = [feature for feature, dtype in params["type_dict"].items() if dtype in ["categorical", "id"] and feature!=params["id_field"]]
    model_params = params["model_params_cat"]["model_kwargs"]
    model = CatBoostClassifier(cat_features=params["processed_cat_features"], **model_params)
...
def cat_regressor_model_setup(params):
    ...
    # cat_features = [feature for feature, dtype in params["type_dict"].items() if dtype in ["categorical", "id"] and feature!=params["id_field"]]
    model_params = params["model_params_cat"]["model_kwargs"]
    model = CatBoostRegressor(cat_features=params["processed_cat_features"], **model_params)
