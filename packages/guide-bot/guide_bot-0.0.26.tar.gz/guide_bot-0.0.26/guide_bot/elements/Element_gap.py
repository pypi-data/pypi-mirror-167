...
class Gap(guide_elements.GuideElement):
    ...
    def add_to_instr(self):
        ...
        gap = self.current_instrument.add_component(self.end_component_name, "Arm")
        gap.set_AT([0, 0, self.get_length_name()], RELATIVE=self.reference_component_name)

    def get_geometry(self):
        return GeometryGap(start_point=self.start_point_parameter,
...
class GeometryGap(BaseElementGeometry):
    def __init__(self, start_point, next_start_point, start_width, start_height, end_width, end_height):
        ...
        self.visible = False  # Disable drawing of this element
