...
from pandas import Series, DataFrame, DatetimeIndex, date_range, Timedelta, read_csv
from pandas.testing import assert_frame_equal

import os
import gzip

from linefolio.round_trips import (
    extract_round_trips,
...
class RoundTripTestCase(TestCase):
    ...
    def test_groupby_consecutive(self, transactions, expected):
        ...
        assert_frame_equal(
            grouped_txn.sort_index(axis="columns"), expected.sort_index(axis="columns")
        )
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                DataFrame(
                    data=[
                        [dates[0], dates[2], Timedelta(days=2), -10.0, -0.2, True, "A"]
                    ],
                    columns=[
                        "open_dt",
                        "close_dt",
    ...
    def test_extract_round_trips(self, transactions, expected, portfolio_value=None):
        ...
        assert_frame_equal(
            round_trips.sort_index(axis="columns"), expected.sort_index(axis="columns")
        )
