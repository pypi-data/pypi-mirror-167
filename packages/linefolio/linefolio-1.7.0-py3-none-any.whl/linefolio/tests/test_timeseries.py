...
class TestDrawdown(TestCase):
    ...
    def test_gen_drawdown_table_relative(
        ...
        self.assertEqual(
            np.round(drawdowns.loc[1, "Net drawdown in %"]), second_net_drawdown
        )
    ...
    def test_drawdown_overlaps(self):
        ...
        pairs = list(
            zip(spy_drawdowns["Recovery date"], spy_drawdowns["Peak date"].shift(-1))
        )[:-1]
...
class TestStats(TestCase):
    ...
    simple_week_rets = pd.Series(
        [0.1] * 3 + [0] * 497, pd.date_range("2000-1-31", periods=500, freq="W")
    )

    simple_month_rets = pd.Series(
        [0.1] * 3 + [0] * 497, pd.date_range("2000-1-31", periods=500, freq="M")
    )

    simple_benchmark = pd.Series(
        [0.03] * 4 + [0] * 496, pd.date_range("2000-1-1", periods=500, freq="D")
    )
