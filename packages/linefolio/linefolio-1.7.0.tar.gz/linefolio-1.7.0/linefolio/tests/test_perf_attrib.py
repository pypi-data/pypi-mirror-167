...
def generate_toy_risk_model_output(start_date="2017-01-01", periods=10, num_styles=2):
    ...
    factor_returns = (
        pd.DataFrame(
            columns=styles, index=dts, data=np.random.randn(periods, len(styles))
        )
        / 100
    )
...
class PerfAttribTestCase(unittest.TestCase):
    def test_perf_attrib_simple(self):
        ...
        positions = pd.DataFrame(
            index=dts, data={"stock1": [20, 20], "stock2": [50, 50], "cash": [0, 0]}
        )
        ...
        positions = pd.DataFrame(
            index=dts, data={"stock1": [20, 20], "stock2": [-20, -20], "cash": [20, 20]}
        )
    ...
    def test_perf_attrib_regression(self):
        ...
        positions = pd.read_csv(
            "linefolio/tests/test_data/positions.csv", index_col=0, parse_dates=True
        )
        ...
        residuals = pd.read_csv(
            "linefolio/tests/test_data/residuals.csv", index_col=0, parse_dates=True
        )
        ...
        self.assertEqual(
            ep.cum_returns_final(combined_returns), perf_attrib_summary["Total Returns"]
        )
        ...
        pd.util.testing.assert_series_equal(
            avg_factor_exposure, exposures_summary["Average Risk Factor Exposure"]
        )
        ...
        pd.util.testing.assert_series_equal(
            cumulative_returns_by_factor, exposures_summary["Cumulative Return"]
        )
    ...
    def test_missing_stocks_and_dates(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            perf_attrib(
                returns, positions, factor_returns, factor_loadings_missing_stocks
            )
