...
class _DFrame:
    _config   = ['_pt_scale_columns', '_pt_scale_omit_interval', '_pt_scalertype', '_pt_columny', '_pt_columnx', 
                 ...
                 '_pt_dummies', '_pt_evaluator',
                 '_pt_transforms', '_pt_train_transforms' ]
    ...
    _config_indices = [
                 ...
                 '_pt_train_valid_indices', '_pt_test_indices', '_pt_folds' ]
    ...
    _cached_fold = [ '_cached_folds', '_cached_index' ]
    ...
    def _indices_before_testsplit(self):
        ...
        try:
            test_indices = set(self._pt_test_indices)
    ...
    def evaluator(self, *metrics):
        try:
            if len(metrics) == 0 or self._pt_evaluator.metrics == metrics:
    ...
    def train(self):
        ...
        r = self._dset_indices(self._train_indices, self._transforms())
        if self._train_transformation_parameters(r):
            r = self._dset_indices(self._train_indices, self._transforms())
    ...
    def raw_train(self):
        try:
            if self._cached_raw_train is not None:
                return self._cached_raw_train
        except: pass
        self._cached_raw_train = self._dset_indices(self._train_indices, self._transforms())
        return self._cached_raw_train
    ...
    def valid(self):
        ...
        r = self._dset_indices(self._valid_indices, self._transforms(train=False))
    ...
    def test(self):
        ...
        if self._pt_sequence_window is None:
            r = DSet.df_to_testset(self.iloc[self._test_indices], self, self._test_indices, self._transforms(train=False))
        else:
            low, high = min(self._test_indices), max(self._test_indices) + self._sequence_window + self._shift_y - 1
            r = DSet.df_to_testset(self.iloc[low:high], self, list(range(low, high)), self._transforms(train=False))
    ...
    def cross_validate_sklearn(self, model, *target, reset_evaluator=True, annot={}, **kwargs):
        """
        ...
        Arguments:
            ...
            reset_evaluator: bool (True)
                when False, an evaluator is reused for these experiments, allowing to stack multiple
                experiment runs in one evaluator for comparison. Note that this only makes sense if
                you also provide annotations (annot) for the runs.
            annot: {}
                the annotations that are stored with the metrics. cross_validate will add a
                'fold' metric to indicat the fold.
    ...
    def study(self, *target, reset_evaluator=True, **kwargs):
        """
        On a DFrame that is configured for n-fold cross validation (df.folds(n)), this function iterates
        over the trials, fitting an SKLearn model and storing the targets for the train, valid and test subsets.
...
class DFrame(pd.DataFrame, _DFrame):
    ...
    def _change(self, key, value):
        ...
        if key in self._config_indices_set:
            ...
            self._cached_changed = True
        elif key in self._config_fold_set:
            self._cached_changed = True
            self._cached_changed_fold = True
    ...
    def _unchanged(self):
        ...
        if self._cached_changed:
            try:
                if not self.index.equals(self._cached_index):
            ...
            if self._cached_changed_fold:
                for p in self._cached_indices:
                    setattr(self, p, None)
            for p in self._cached:
                setattr(self, p, None)
            self._cached_changed = False
            self._cached_changed_fold = False
            return False
        return True
    ...
    def dropna(self, **kwargs):
        if self._pt_test_indices is not None:
            warnings.warn("Using dropna on a dataset with an existing test set " +
