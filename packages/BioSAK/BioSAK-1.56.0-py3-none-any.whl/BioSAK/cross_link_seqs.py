...
def cross_link_seqs(args):
    ...
    blast_result_txt_filtered = '%s_blastn_filtered.txt' % prefix
    output_plot               = '%s_iden%s_min%sbp.%s'   % (prefix, iden_cutoff, aln_len_cutoff, plot_format)
    ...
    ctg_1_name_str = '%s    Identity (%s-%s%s)  Alignment length (%s-%sbp)' % (ctg_1_record.id, iden_min, iden_max, '%', aln_len_min, aln_len_max)
    contig_1_gene_content_track = diagram.new_track(1, name=ctg_1_name_str, greytrack=True, greytrack_labels=1,
                                                    greytrack_font='Helvetica', greytrack_fontsize=20,
    ...
    ctg_2_name_str = '%s  Identity (%s-%s%s)  Alignment length (%s-%sbp)' % (ctg_2_record.id, iden_min, iden_max, '%', aln_len_min, aln_len_max)
    contig_2_gene_content_track = diagram.new_track(1, name=ctg_2_record.id, greytrack=True, greytrack_labels=1,
                                                    greytrack_font='Helvetica', greytrack_fontsize=20,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', required=True,                 help='identity cutoff, 0-100')
    parser.add_argument('-l', required=True,                 help='alignment length cutoff, in bp')
