...
class Version:
    ...
    def __init__(self, version_as_string):
        ...
        if first_character == '/' or first_character == '~':
            if len(version_as_string) and first_character == '~':
                version_as_string = os.path.expanduser(version_as_string)

            if len(version_as_string) == 0 or version_as_string[0] != '/':
                raise SyntaxError('Invalid path \'' + version_as_string + '\' for local version.')

            self.original_version = version_as_string
        else:
            if first_character == 'v' and len(version_as_string) > 1:
                first_character = version_as_string[1]

            if first_character == '>' or first_character == '<' or (first_character >= '0' and first_character <= '9'):
                self.original_version = version_as_string
    ...
    def isBranch(self):
        return self.asSemVer is None and self.original_version is not None and self.original_version[0] != '/'
