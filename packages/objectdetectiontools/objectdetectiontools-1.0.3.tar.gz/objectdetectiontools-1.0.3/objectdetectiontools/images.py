...
def preprocess_slice(annotations: list[dict], size: int, margin: float, input_path: str, output_path: str) -> list[dict]:
    '''
    Uses `annotations` to slice the images referenced by the filename field into parts that are roughly the square of `size`, having `margin` as a tolorance to defining the slice lines.
        `annotations`: A list of dicts defining the objects in the images. Can be created by the read_annotations_* function(s).
    ...
    for filename in filenames:
        ...
        for h in horizontal_lines:
            ...
            for v in vertical_lines:
                number = number +1
                new_filename = filename + "___slice-" + str(chr(ord('`')+row)) + str(number) + fileext
...
def collect_slices(path: str, image_path: str) -> tuple[list[Image], int, int, list[int], list[int]]:
    '''
    ...
    files = glob(os.path.join(path, image_name + "___slice-*" + ext))
    for f in files:
        if f[f.find("___slice-")+len("___slice-"):][0] == "a":
            columns = columns +1
        else:
            break
...
def stitch(slices: list[Image], width: int, height: int) -> Image:
    '''
    Merges `slices` (a list of PIL.Images) back into 1 large image of dimension equal to `width` and `height`.
        `slices`: A list of PIL.Images that makes up the slices.
...
def preprocess_break(image_path: str, size: int) -> tuple[list[Image], int, int]:
    '''
    Slices the image at `image_path` into a number of equaly large slices of roughly the size specified by `size`.
