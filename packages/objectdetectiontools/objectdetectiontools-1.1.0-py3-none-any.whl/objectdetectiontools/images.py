...
def slice(annotations: list[dict], size: int, margin: float, input_path: str, output_path: str) -> list[dict]:
...
def collect(path: str, image_path: str) -> tuple[list[PIL], int, int, list[int], list[int]]:
    ...
    files = glob.glob(os.path.join(path, image_name + "___slice-*" + ext))
    for f in files:
...
def stitch(slices: list[PIL], width: int, height: int) -> PIL:
    """Merges the list of PIL.Images `slices` back into 1 large image of dimension equal to `width` and `height`.
...
def chop(image_path: str, size: int) -> tuple[list[PIL], int, int]:
