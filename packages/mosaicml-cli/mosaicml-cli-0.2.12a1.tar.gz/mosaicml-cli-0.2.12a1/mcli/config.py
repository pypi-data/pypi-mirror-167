...
MOSAICML_API_ENDPOINT: str = 'http://localhost:3001/graphql'
...
class MCLIConfig(SerializableDataclass):
    ...
    @staticmethod
    def _remove_deprecated_platform_details(platforms: List[Dict[str, str]]) -> List[Dict[str, str]]:
        # pylint: disable-next=import-outside-toplevel
        from mcli.serverside.platforms.platform import GenericK8sPlatform

        valid_platforms: List[Dict[str, str]] = []
        for platform in platforms:
            if 'kubernetes_context' in platform and platform[
                    'kubernetes_context'] not in GenericK8sPlatform.get_k8s_context_map():
                continue
    ...
    def load_config(cls, safe: bool = False) -> MCLIConfig:
        ...
        try:
            with open(MCLI_CONFIG_PATH, 'r', encoding='utf8') as f:
                ...
                data['platforms'] = cls._remove_deprecated_platform_details(data.get('platforms', []))
