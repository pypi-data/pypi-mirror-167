...
def get_runs(
    ...
    Arguments:
        ...
        platforms (``Optional[List[str] | List[``:class:`~mcli.models.mcli_platform.MCLIPlatform` ``]]``):
            List of platforms to filter runs. This can be a list of str or
            :class:`~mcli.models.mcli_platform.MCLIPlatform`
            objects. Only runs submitted to these platforms will be returned.
        gpu_types (``Optional[List[str] | List[``:class:`~mcli.serverside.platforms.gpu_type.GPUType` ``]]``):
            List of gpu types to filter runs. This can be a list of str or
            :class:`~mcli.serverside.platforms.gpu_type.GPUType`
    ...
    if platforms is not None:
        ...
        # If runs were specified, pull platforms from them to speed up kube calls
        known_platforms = {r.config.platform for r in runs if isinstance(r, Run)}
        platforms = list(known_platforms) or None
