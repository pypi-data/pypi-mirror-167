...
from pih.tools import DataTools, ResultTools, FullNameTool, PasswordTools
...
class PIH:
    ...
    class RESULT:
        ...
        class TIME_TRACKING:
            ...
            @staticmethod
            def in_period(day_start: int = 1, day_end: int = None, month: int = None, tab_number: str = None) -> Result[List[TimeTrackingResultByDivision]]:
            ...
            @staticmethod
            def create(start_date: datetime = None, end_date: datetime = None, tab_number: str = None) -> Result[List[TimeTrackingResultByDivision]]:
                ...
                time_tracking_entity_list: List[TimeTrackingEntity] = DataTools.to_result(
                ...
                for time_tracking_entity in time_tracking_entity_list:
                        ...
                        for date in result_data[division_name][tab_number]:
                            ...
                            if time_tracking_start_entity is not None:
                                if time_tracking_end_entity is not None:
                                    duration = str(timedelta(seconds=(datetime.fromisoformat(
            ...
            while True:
                ...
                if format:
                    ...
                    if check and telehone_fixed != telehone:
                        ...
                        PR.value("Телефон отформатирован:", telehone)
    ...
    class CHECK:
            ...
            def by_group(group: CONST.AD.Groups) -> bool:
                ...
                if user_list is not None:
                    for user in user_list:
                        if user.samAccountName == PIH.OS.get_login():
                            return True
                return False
        ...
        class TIME_TRACKING:
            ...
            @staticmethod
            def available() -> bool:
                return PIH.CHECK.RIGTH.by_group(CONST.AD.Groups.TimeTrackingAdmin)
    ...
    class MESSAGE_COMMAND:
        ...
        @staticmethod
        def login() -> bool:
            return PIH.message_command(LogCommands.LOG_IN, (PIH.OS.get_login(), PIH.OS.get_host_name()))
        ...
        def hr_notify_about_new_employee(login: User) -> bool:
            ...
            return PIH.message_command(LogCommands.HR_NOTIFY_ABOUT_NEW_EMPLOYEE, (FullNameTool.to_given_name_string(FullNameTool.from_string(hr_user.name)), 
                                                                                  user.name, user.mail))

        @staticmethod
        def it_notify_about_new_user(login: str, password: str) -> bool:
            it_user_list: list[User] = PIH.RESULT.USER.by_job_position(CONST.AD.JobPositions.IT).data
            me_user_login: str = PIH.OS.get_login()
            it_user_list = list(
            ...
            return PIH.message_command(LogCommands.IT_NOTIFY_ABOUT_NEW_USER, (user.name, user.description, user.samAccountName, password, user.telephoneNumber, user.mail, mark.TabNumber)) and \
                PIH.message_command(LogCommands.IT_TASK_AFTER_CREATE_NEW_USER, (FullNameTool.to_given_name_string(FullNameTool.from_string(it_user.name)), user.name, user.mail, password))
    ...
    class ACTION:
        ...
        class TIME_TRACKING:
            ...
            def report(path: str, start_date: datetime, end_date: datetime, tab_number: str = None) -> bool:
                ...
                lan_adress: str = f"//{PIH.OS.get_host_name()}/"
                if path[1] == ":":
                    path = f"{lan_adress}{path[0]}${path[2:]}"
...
class ActionStack(List):
    ...
    def show_action_values(self) -> None:
        def label(index: int, item: ActionValue):
            return item.caption
