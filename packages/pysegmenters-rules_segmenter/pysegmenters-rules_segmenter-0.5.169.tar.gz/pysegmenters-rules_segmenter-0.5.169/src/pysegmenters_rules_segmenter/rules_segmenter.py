...
SPLIT_DEFAULT_MAPPING = {'xx': json.dumps(SPLIT_DEFAULT)}
...
class RulesSegmenterParameters(SegmenterParameters, ProcessorParameters, ABC):
    split_rules: Dict[str, str] = Field(SPLIT_DEFAULT_MAPPING,
...
```""" + SPLIT_EXAMPLE_STR+"```", extra="key:language,val:json")
    join_rules: Dict[str, str] = Field(None,
    ...
    Then you need to define a rule like:<br/>
```""" + JOIN_EXAMPLE_STR+"```", extra="key:language,val:json")
...
class RulesSegmenter(SegmenterBase, ProcessorBase, ABC):
    ...
    #languages:en,fr,de,nl,es,pt,it,zh,ar,hi,ur,fa,ru
    """
    ...
    def segment(self, documents: List[Document], parameters: SegmenterParameters) \
        ...
        split_mapping = {k: json.loads(v) for k, v in
                         params.split_rules.items()} if params.split_rules else {}
        if len(split_mapping) == 0:
            logger.warning(
                f"No splitting rules defined: ignoring",
                exc_info=True,
            )
            return documents
        join_mapping = {k: json.loads(v) for k, v in
                        params.join_rules.items()} if params.join_rules else {}
        default_lang = 'xx' if 'xx' in split_mapping else next(iter(split_mapping))
        for document in documents:
            # Retrieve nlp pipe
            lang = document_language(document, default_lang)
            json_split = json_rules_to_string(split_mapping[lang] if lang in split_mapping else split_mapping[default_lang])
            json_join = json_rules_to_string(join_mapping.get(lang, []))
