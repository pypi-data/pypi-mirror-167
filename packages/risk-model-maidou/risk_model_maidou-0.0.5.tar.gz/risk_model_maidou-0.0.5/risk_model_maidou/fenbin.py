...
def calculator(data_df, x, new_list, na_df):
    ...
    non_na_indicator = pd.DataFrame({'Bin': bin_list, 'KS': ks_list, 'WOE': woe_list, 'IV': IV_series,
...
def woe_quzheng(data,messa,out_in_list,y,rate):
    ...
    def quzheng(x):
        ...
        if xx>=10:
            n=len(str(int(xx)))-1
            return round(xx/10**n,1)*10**n*int(x/xx)
        if xx>=1:
            n=len(str(int(xx)))-1
            return round(xx/10**n,2)*10**n*int(x/xx)
        if xx<1:
            n=-int(np.log(xx)/np.log(10))+2
             
            return float(int(round(xx*10**n,0))*(10**(-n)))*int(x/xx)

    def cal(new_list,non_na_df,na_df):
        non_na_df=non_na_df.copy()
        na_df=na_df.copy()
        temp_df_list=[]
        bin_list=[]
        bin_list1=[]
        ...
        indicator = pd.DataFrame({'Bin': bin_list,  'WOE': woe_list, 'IV': IV_series,
                                         'total_count': total_list,'bad_count': bad_list, 'bad_rate': bad_rate_list})
        indicator['var']=x
        indicator['per']=indicator['total_count']/(total_good+total_bad)
        return indicator,bin_list1
    ...
    def check_s(m):
        if sorted(m)==m or sorted(m,reverse=True)==m:
            return 1
        else:
            return 0
...
#    y='y'
    ...
    for x in list(messa['var'].unique()):
        ...
        try:
            ...
            indicator,bin_list1=cal(new_list,non_na_df,na_df)
            indicator1=indicator[~indicator['Bin'].isin(out_in_list)]
                                 
            mnin=min(abs(indicator['bad_rate'].shift(1)/indicator['bad_rate']-1).fillna(1))                   
            mmin=min(list(indicator['per']))
                                 
                                 
            if check_s(list(indicator1['bad_rate']))==1 and mnin>=0.1 and mmin>rate:
                nl.append(indicator)
            else:
                ddi=dict()
                for i in range(len(bin_list1))[:-1]:
                    ddi[str(bin_list1[i])]=str(bin_list1[i+1])
                
                    
                ind=[str(k) for k in bin_list1[:-1]]
                nnew=[]
                for k in range(len(ind)):
                    new_list1=[i.replace(ind[k],ddi[ind[k]]) for i in new_list]  
                    new_list1.remove('('+str(ddi[ind[k]])+','+str(ddi[ind[k]])+']')
                    nnew.append(new_list1)
                miv=0
                nn=None
                for i in nnew:
                    indicator,i1=cal(i,non_na_df,na_df)
                    indicator1=indicator[~indicator['Bin'].isin(out_in_list)]
                    mnin=min(abs(indicator['bad_rate'].shift(1)/indicator['bad_rate']-1).fillna(1))                    
                    mmin=min(list(indicator['per']))
                ...
                if miv==0:
                    print('error-'+str(x))
                    pass
                indicator,bin_list1=cal(nn,non_na_df,na_df)
...
def woe_quzheng1(data,messa,out_in_list,y):
    out_in_list=out_in_list+['nan','None']
    data=data.copy()
    messa=messa.copy()
    def quzheng(x):
...
#amc=woe_quzheng1(data0,messa,[],y)

def cal_woe(data,messa,out_in_list,y):
    def check(m):
