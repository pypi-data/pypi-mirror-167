...
class SessionManager:
    ...
    def init_handle_pool(self, mode, nums):
        if mode == THREAD_MODE:
            self.handle_pool = self.init_thread(nums)
        elif mode == PROCESS_MODE:
            self.handle_pool = self.init_process(nums)
        else:
            self.handle_pool = self.init_thread(nums)
        self.initialized = True

    def init_threads(self, nums=0):
        # 线程模式
        return ThreadPool(nums)
...
class WorkerThread(threading.Thread):  # 继承父类threading.Thread
    ...
    def run(self):
        while True:
            if not self.in_q.empty() and self.req_data_status != DataEnd:
                req_data = self.in_q.get()
                self.writeFunc(req_data)
            if not self.out_q.empty() and self.resp_data_status != DataEnd:
                resp_data = self.out_q.get()
                self.readFunc(self.out_q)

        print("开始线程：" + self.name)
        self.action
        print("退出线程：" + self.name)

    def writeFunc(self, reqData):
        raise NotImplementedError("you must implement this func in your thread class")
