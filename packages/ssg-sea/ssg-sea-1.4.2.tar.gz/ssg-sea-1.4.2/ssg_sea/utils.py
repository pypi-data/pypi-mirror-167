...
def getSkillWeight(skillExtractionList, id_mapping_dict, skillForm):
    """
    ...
    for askill, afreq in odict.items():
        ...
        aSkillId = id_mapping_dict.get(askill.lower().strip())
    ...
    for i in range(len(skillList)):
        output_with_weight = (skillIdList[i], skillList[i], skillWeightList[i], skillForm)
