...
class ItemWithOwnerAndAcl(Item):
    ...
    def _pull_owner_and_acl(self, session: Session, owner: IdentityPreviewV1, errors='raise', status=None):
        items_api = ItemsApi(session.client)

        self['Owner'] = {'ID': owner.id}
        self['Owner']['Redacted'] = owner.is_redacted
        if not owner.is_redacted:
            owner_details = User.pull(owner.id, session=session, errors=errors, status=status)
            if owner_details is not None:
                self['Owner'] = owner_details.definition

        @request_safely(action_description=f'get access control list for Item {self["ID"]}',
            ...
            for ace_output in acl_output.entries:  # type: AceOutputV1
                ...
                if not ace_output.identity.is_redacted:
                    if ace_output.identity.type == 'User':
                        identity = User.pull(ace_output.identity.id, session=session, errors=errors, status=status)
                    else:
                        identity = UserGroup.pull(ace_output.identity.id, session=session, errors=errors, status=status)
    ...
    def _push_owner_and_location(self, session: Session, item_output, owner_id, folder_id, errors, status):
        items_api = ItemsApi(session.client)
    ...
    def _push_acl(self, session: Session, pushed_id, datasource_maps, item_map, access_control, errors, status):
        replace = False
        strict = False
        if access_control:
...
class User(Identity):
    @staticmethod
    def pull(item_id, *, allowed_types=None, status=None, session: Optional[Session] = None, errors='raise'):
        session = Session.validate(session)
        users_api = UsersApi(session.client)
        user_output = safely(lambda: users_api.get_user(id=item_id),
                             action_description=f'get User {item_id}',
                             errors=errors, status=status)  # type: UserOutputV1
...
class UserGroup(Identity):
    ...
    def pull(item_id, *, allowed_types=None, status=None, session: Optional[Session] = None, errors='raise'):
        ...
        usergroup_output = safely(lambda: usergroups_api.get_user_group(user_group_id=item_id),
                                  ...
                                  errors=errors, status=status)  # type: UserGroupOutputV1
