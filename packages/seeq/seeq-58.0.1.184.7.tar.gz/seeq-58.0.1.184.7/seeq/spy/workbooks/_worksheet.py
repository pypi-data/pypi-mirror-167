...
class AnalysisWorksheet(Worksheet):
    ...
    def refresh_from(self, new_item, item_map, errors: str, status: Status):
        ...
        self.document.refresh_from(new_item.document, item_map, errors, status)

        for workstep_id, workstep in self.worksteps.items():
            workstep['ID'] = item_map[workstep_id.upper()]
...
def get_analysis_worksheet_from_url(url: str, include_archived: bool = False,
                                    errors: str = 'raise', status: Status = None, quiet: bool = False):
    """
    ...
    errors : {'raise', 'catalog'}, default 'raise'
        If 'raise', any errors encountered will cause an exception. If 'catalog', non-breaking
        errors will be logged as a warning.
    status : Status, default None
        The Status object to report progress to.
    ...
    workbook_search = spy.workbooks.search({'ID': workbook_id}, errors='catalog', status=status, quiet=quiet)
    ...
    workbooks = spy.workbooks.pull(workbook_search, include_referenced_workbooks=False, include_inventory=False,
                                   specific_worksheet_ids=[worksheet_id], errors=errors, status=status, quiet=quiet)
    if not workbooks:
