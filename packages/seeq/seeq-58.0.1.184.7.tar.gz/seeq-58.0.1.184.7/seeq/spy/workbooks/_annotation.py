...
class Annotation:
    ...
    def refresh_from(self, new_item, item_map, errors: str, status: Status):
...
class Report(Annotation):
    ...
    def pull(self, session: Session, include_images=True, errors='raise', status=None):
        ...
        for content in contents.content_items:
            ...
            if content.date_range is not None:
                new_date_range = DateRange.pull(content.date_range, report=self, session=session, errors=errors,
                                                status=status)
            ...
            if content.asset_selection is not None:
                new_asset_selection = AssetSelection.pull(content.asset_selection, report=self,
                                                          session=session, errors=errors, status=status)
        ...
        for date_range_id in annotation_output.date_range_ids:
            if date_range_id not in self.date_ranges.keys():
                new_date_range = DateRange.pull(date_range_id, report=self, session=session, errors=errors,
                                                status=status)
                self.date_ranges[new_date_range.id] = new_date_range
    ...
    @staticmethod
    def _refresh_child_dict(existing_item_dict: dict, new_item_dict: dict, item_map: dict, errors: str, status: Status):
        ...
        for existing_item in existing_items:
            ...
            # Refresh item itself
            existing_item.refresh_from(new_item, item_map, errors, status)

    def refresh_from(self, new_item, item_map, errors: str, status: Status):
        super().refresh_from(new_item, item_map, errors, status)

        Report._refresh_child_dict(self.date_ranges, new_item.date_ranges, item_map, errors, status)
        Report._refresh_child_dict(self.asset_selections, new_item.asset_selections, item_map, errors, status)
        Report._refresh_child_dict(self.content, new_item.content, item_map, errors, status)
