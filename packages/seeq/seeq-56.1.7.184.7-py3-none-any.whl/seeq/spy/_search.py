...
def search(query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True,
           ...
           quiet=False, errors='raise', status=None, session: Optional[Session] = None):
    """
    ...
    try:
        return _search(session, query, all_properties=all_properties, workbook=workbook, recursive=recursive,
                       ...
                       estimate_sample_period=estimate_sample_period, order_by=order_by,
                       quiet=quiet, errors=errors, status=status, input_args=input_args)
...
def _search(session: Session, query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True,
            ignore_unindexed_properties=True, include_archived=False, estimate_sample_period=None, order_by=None,
            quiet=False, errors='raise', status=None, input_args=None):
    _common.validate_errors_arg(errors)
    status = Status.validate(status, quiet)
    if order_by:
    ...
    if isinstance(query, pd.DataFrame):
        ...
        worksheet = spy.utils.get_analysis_worksheet_from_url(query, include_archived,
                                                              errors=errors, status=status, quiet=status.quiet)
    ...
    def _estimate_sample_period(_signal_id, _signal_name):
        ...
        formula_run_output = safely(
            ...
            errors=errors, additional_errors=[400])
