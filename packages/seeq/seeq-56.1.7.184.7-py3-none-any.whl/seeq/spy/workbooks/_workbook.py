...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def refresh_from(self, new_item, item_map, errors: str, status: Status):
        ...
        for worksheet in self.worksheets:
            ...
            if len(new_worksheet_list) == 1:
                worksheet.refresh_from(new_worksheet_list[0], item_map, errors, status)
        ...
        for inventory_item_id, inventory_item in self.item_inventory.copy().items():
            ...
            if new_inventory_item_id not in new_inventory:
                ...
                inventory_item.refresh_from(new_inventory_item, item_map, errors, status)
                del self.item_inventory[inventory_item_id]
    ...
    def find_by_name(session: Session, workbook_name, workbook_type, folder_id, errors, status) \
        ...
        @request_safely(action_description=f'find {workbook_type} "{workbook_name}" in folder {folder_id}',
                        additional_errors=[400], errors=errors, status=status)
