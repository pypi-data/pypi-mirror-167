...
def uninstall(items: Union[pd.DataFrame, pd.Series], *, errors: str = 'raise', quiet: bool = False,
    ...
    for row_index, row in items.iterrows():
        @request_safely(action_description=f"uninstalling addon with id {row['ID']}", errors=errors, status=status)
