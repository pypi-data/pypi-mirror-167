...
class Content(Item):
    ...
    def push(self, session: Session, item_map: dict, existing_contents: dict, errors='raise', status=None):
        ...
        if not existing_content:
            ...
            content_output = safely(
                ...
                errors=errors, status=status)  # type: ContentOutputV1
        else:
            content_output = safely(
    ...
    @staticmethod
    def pull(item_id, *, allowed_types=None, report=None, session: Session = None, errors='raise', status=None):
        session = Session.validate(session)
        content_api = ContentApi(session.client)
...
class DateRange(Item):
    ...
    def push(self, session: Session, item_map: dict, existing_date_ranges: dict, errors='raise', status=None):
...
class AssetSelection(Item):
    ...
    def push(self, session: Session, item_map: dict, existing_asset_selections: dict, errors='raise', status=None):
