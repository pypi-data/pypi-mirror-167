...
def search(query: Union[pd.DataFrame, pd.Series, dict, list, str], *, errors: str = 'raise', quiet: bool = False,
           status: Status = None, session: Optional[Session] = None):
    """
    ...
    try:
        return _search(session, query, errors=errors, quiet=quiet, status=status, input_args=input_args)

    except KeyboardInterrupt:
        status.update('Search canceled', Status.CANCELED)


def _search(session: Session, query, *, errors, quiet=False, status=None, input_args=None):
    status = Status.validate(status, quiet)
