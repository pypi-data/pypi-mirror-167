...
class StoredOrCalculatedItem(Item):
    ...
    def _populate_asset_and_path(self, session: Session, item_id, errors, status):
        ...
        asset_tree_output = safely(
            ...
            errors=errors, status=status)  # type: AssetTreeOutputV1
    ...
    def _pull(self, session: Session, item_id, errors, status):
        super()._pull(session, item_id, errors, status)

        if 'Path' not in self.definition:
            self._populate_asset_and_path(session, item_id, errors, status)
