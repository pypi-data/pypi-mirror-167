...
def test_push_to_workbook():
    ...
    # Now that we have a workbook, test that _get_primary_worksheet functions correctly
    workbook_by_id = mock.Mock()
    workbook_by_id.id = workbook.id
    primary_worksheet_by_id = _push._get_primary_worksheet(spy.session, workbook_by_id, folder_id=None,
                                                           errors='raise', status=Status())  # type: WorksheetOutputV1
    assert primary_worksheet_by_id.name == 'My Worksheet!'

    workbook_by_name = mock.Mock()
    workbook_by_name.name = workbook.name
    workbook_by_name.definition = {'Workbook Type': 'Analysis'}
    folder_id = workbook['Ancestors'][-1]
    primary_worksheet_by_name = _push._get_primary_worksheet(spy.session, workbook_by_name, folder_id=folder_id,
                                                             errors='raise', status=Status())  # type: WorksheetOutputV1
    assert primary_worksheet_by_name.name == 'My Worksheet!'
