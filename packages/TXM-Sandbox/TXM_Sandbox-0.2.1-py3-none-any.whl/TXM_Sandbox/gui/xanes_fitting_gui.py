...
WL_SAVE_ITEM_OPTIONS = ['wl_fit_coef',
                        'wl_fit_err',
                        'wl_pos_dir',
                        'wl_pos_fit',    
                        'centroid_of_eng',
...
class xanes_fitting_gui():
    def __init__(self, parent_h, form_sz=[650, 740]):
        ...
        if self.parent_h.gui_name == 'xanes3D':
            ...
            self.xanes_mask_external_command_name = os.path.join(self.global_h.script_dir, 'xanes3D_mask_external_command.py')
        elif self.parent_h.gui_name == 'xanes2D':
            self.fn = self.parent_h.xanes_save_trial_reg_filename
            self.xanes_mask_external_command_name = os.path.join(self.global_h.script_dir, 'xanes2D_mask_external_command.py')
    ...
    def build_gui(self):
        ...
        layout = {'background-color': 'white', 'color': 'cyan', 'left': '41%', 'width': 'auto'}
        ...
        self.hs['FitItem tab'].layout = {'width': 'auto', 'height': f'{0.61*(self.form_sz[0]-128)}px'}
        ...
        fit_pars_GridspecLayout[3, 80:100] = widgets.Button(description='calc masks', disabled=True,
        ...
        fit_pars_GridspecLayout[3, 110:130] = widgets.Button(description='EM it', disabled=True,
                                                             description_tooltip='Calculate and preview the edge_jump and edge_offset filters',
                                                             layout={'width': '95%', 'height': '70%'})
        ...
        fit_pars_GridspecLayout[3, 140:160] = widgets.Button(description='OM it', disabled=True,
                                                             description_tooltip='Calculate and preview the edge_jump and edge_offset filters',
                                                             layout={'width': '95%', 'height': '70%'})
        ...
        fit_pars_GridspecLayout[3, 170:190] = widgets.Button(description='EM&OM it', disabled=True,
                                                             description_tooltip='Calculate and preview the edge_jump and edge_offset filters',
                                                             layout={'width': '95%', 'height': '70%'})
        ...
        fit_pars_GridspecLayout[6, 170:190] = widgets.Button(layout={'width':"95%", "height": "70%"},
    ...
    def set_save_items(self):
        if self.parent_h.xanes_fit_type == 'wl':
            tem1 = set(deepcopy(sorted(WL_SAVE_ITEM_OPTIONS)))
            tem2 = set(deepcopy(sorted(WL_SAVE_DEFAULT)))
            if not self.fit_wl_pos:
                [tem1.remove(ii) for ii in ['wl_pos_fit', 'wl_fit_coef', 'wl_fit_err'] 
                if ii in self.hs['FitSavSetting mulsel'].options]
                [tem2.remove(ii) for ii in ['wl_pos_fit', 'wl_fit_coef', 'wl_fit_err'] 
            ...
            tem1 = set(deepcopy(sorted(FULL_SAVE_ITEM_OPTIONS)))
            tem2 = set(deepcopy(sorted(FULL_SAVE_DEFAULT)))
            if not self.fit_wl_pos:
                [tem1.remove(ii) for ii in ['wl_pos_fit', 'wl_fit_coef', 'wl_fit_err'] 
                if ii in self.hs['FitSavSetting mulsel'].options]
                [tem2.remove(ii) for ii in ['wl_pos_fit', 'wl_fit_coef', 'wl_fit_err'] 
            ...
            if not self.fit_edge50_pos:
                [tem1.remove(ii) for ii in ['edge50_pos_fit', 'edge_pos_fit', 'edge_fit_coef', 'edge_fit_err'] 
                if ii in self.hs['FitSavSetting mulsel'].options]
                [tem2.remove(ii) for ii in ['edge50_pos_fit', 'edge_pos_fit', 'edge_fit_coef', 'edge_fit_err'] 
                if ii in self.hs['FitSavSettingSel mulsel'].options]
            if not self.find_wl_pos_dir:
                [tem1.remove(ii) for ii in ['wl_pos_dir'] 
                if ii in self.hs['FitSavSetting mulsel'].options]
    ...
    def update_fit_params(self):
        ...
        if self.parent_h.gui_name == 'xanes3D':
            ...
            json.dump(self.parent_h.xanes_config, open(self.parent_h.xanes_save_trial_reg_config_filename, 'w'),
                      cls=NumpyArrayEncoder, indent=4, separators=(',', ': '))
        elif self.parent_h.gui_name == 'xanes2D':
            self.parent_h.update_xanes2D_config()
            json.dump(self.parent_h.xanes_config, open(self.parent_h.xanes_file_save_trial_reg_config_filename, 'w'),
                      cls=NumpyArrayEncoder, indent=4, separators=(',', ': '))
    ...
    def fit_run_btn_clk(self, a):
        ...
        if self.parent_h.gui_name == 'xanes3D':
            ...
            code[ln] = f"                    xana.cal_wgt_eng(xanes3D_analysis_wl_fit_eng_s, xanes3D_analysis_wl_fit_eng_e)"; ln += 1
            code[ln] = f"                for jj in {self.analysis_saving_items}:"; ln += 1
            ...
            code[ln] = f"                    xana.find_wl(xanes3D_analysis_wl_fit_eng_s, xanes3D_analysis_wl_fit_eng_e, optimizer='direct',ufac={wl_ufac})"; ln += 1
            code[ln] = f"                if ('centroid_of_eng' or 'centroid_of_eng_relative_to_wl' or 'weighted_attenuation' or 'weighted_eng') in {self.analysis_saving_items}:"; ln += 1
            code[ln] = f"                    xana.cal_wgt_eng(xanes3D_analysis_pre_edge_e + xanes3D_analysis_edge_eng, xanes3D_analysis_wl_fit_eng_e)"; ln += 1
            ...
            code[ln] = f"        if ('centroid_of_eng' or 'centroid_of_eng_relative_to_wl' or 'weighted_attenuation' or 'weighted_eng') in {self.analysis_saving_items}:"; ln += 1
            code[ln] = f"            xana.cal_wgt_eng(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e)"; ln += 1
            code[ln] = f"        for jj in {self.analysis_saving_items}:"; ln += 1
            code[ln] = f"            if jj == 'wl_pos_fit':"; ln += 1
            ...
            code[ln] = f"            xana.find_wl(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e, optimizer='direct',ufac={wl_ufac})"; ln += 1
            code[ln] = f"        if ('centroid_of_eng' or 'centroid_of_eng_relative_to_wl' or 'weighted_attenuation' or 'weighted_eng') in {self.analysis_saving_items}:"; ln += 1
            code[ln] = f"            xana.cal_wgt_eng(xanes2D_analysis_pre_edge_e + xanes2D_analysis_edge_eng, xanes2D_analysis_wl_fit_eng_e)"; ln += 1
            code[ln] = f"        if 'lcf_fit' in {self.analysis_saving_items}:"; ln += 1
            code[ln] = f"            xana.interp_ref_spec()"; ln += 1
            code[ln] = f"            xana.lcf()"; ln += 1
            code[ln] = f"";ln += 1
