...
class PySDBSegmenterParameters(SegmenterParameters, ProcessorParameters, ABC):
    language: str = Field("xx",
                          description="Name of the 2-letter language to use if the language of the document is not set", extra="language")
...
class PySDBSegmenter(SegmenterBase, ProcessorBase, ABC):
    ...
    def segment(self, documents: List[Document], parameters: SegmenterParameters) \
        ...
        for document in documents:
            lang = document_language(doc, params.language)
            # Retrieve nlp pipe
