...
#   2022-07-08  WXG  verify 增加ppt，自定义错误提示 prompt。版本 1.1.4
# =============================================================================
...
ERR_VERIFY_NEED_PARAM = 500     # 缺少参数
ERR_VERIFY_VAL_EXCEED = 501     # 数值越限
ERR_VERIFY_ERROR = 900          # 校验出错
ERR_VERIFY_WHO = 901            # 未知的校验类型


__version__ = "1.1.6"
__author__ = "WXG"
__all__ = ['verify', 'validation', 'ERR_VERIFY_NEED_PARAM', 'ERR_VERIFY_VAL_EXCEED', 'ERR_VERIFY_ERROR']
...
def _validate_single_key(key, s, dictionary, who, extra_tip):
    ...
    # 读取特殊 校验 对象
    special = {'prompt': {'has': False, 'val': ''},
               'default': {'has': False, 'val': ''},
               'point': {'has': False, 'val': ''},
               'add_hms': {'has': True},
               }
    for validator in validations:
        if isinstance(validator, Prompt):   # 自定义提示
            special['prompt'].update({'has': True, 'val': validator.prompt})
    ...
    if key not in dictionary:
        return {'code': E_PUB_NEED, 'msg': E_PUB_NEED_MSG % name}
    ...
    if who in ['s', 'str']:
        ...
        if not passed:
            return {"code": E_PUB_NUM, "msg": E_PUB_NUM_MSG % name}
        dec = JdTools.decimal(val, pt=pt + 1)
        if dictionary[key] != dec:
            return {"code": E_PUB_NUM_PT2, "msg": f'{name} 最多 {pt} 位小数！'}
        # 校验 decimal 数值 不能超过 7 位数字
        if dec > 9999999:
            return {'code': ERR_VERIFY_VAL_EXCEED, 'msg': f'{name} 数值越限，最多7位（除小数外）'}
    elif who in ['d', 'date']:
        add_hms = special['add_hms']['has']
        try:
            dictionary[key] = JdTools.str_to_datetime(val, add_hms=add_hms)
        except (ValueError, TypeError):
            return {'code': ERR_DATE_FMT, 'msg': f'{name} 格式错误【{val}】，期望 年-月-日！'}
    elif who in ['t', 'time']:      # 时间 时分秒
        try:
            dictionary[key] = JdTools.str_to_time(val)
        except (ValueError, TypeError):
            return {'code': 201, 'msg': f'{name} 格式错误【{val}】，期望 HH:MM:SS！'}
    elif who in ['i', 'int']:
        passed, dictionary[key] = JdTools.int(val)
        if not passed:
            if special['prompt']['has']:
                return {"code": E_PUB_CUSTOMIZED_ERR, "msg": special['prompt']['val']}
            else:
                return {"code": E_PUB_NUM_INT, "msg": E_PUB_NUM_INT_MSG % name}
    ...
    for validator in validations:
        ...
        if not passed:
            if special['prompt']['has']:
                ...
                return {'code': 900, 'msg': f'[{name}] {validator.err_message}'}
