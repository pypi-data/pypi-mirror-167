...
class JdTools(object):
    ...
    @staticmethod
    def get_filename(name, excel2010=False):
        """ 返回 唯一 文件名，name 后面增加 年月日 时分秒 毫秒后缀   excel2010 true-4位后缀文件扩展名"""
        tm = datetime.datetime.today()
        time_str = datetime.datetime.strftime(tm, '_%Y%m%d_%H%M%S_%f')
        return name + time_str + '.xls' + ('x' if excel2010 else '')
    ...
    def str_to_datetime(s_dt, add_hms=False):
        ...
        :return:
        """
        if isinstance(s_dt, datetime.datetime) or isinstance(s_dt, datetime.date):
            return s_dt
    ...
    def decimal(number, result=False, pt=2):
        ...
        if number == '' or number is None:      # 容错处理： None, ''(空串), 返回 0
            ...
            number = decimal.Decimal(JdTools.get_float(number, pt))
        elif isinstance(number, int):
            number = decimal.Decimal(number)
        elif isinstance(number, str):
            try:
                number = decimal.Decimal(number).quantize(decimal.Decimal('0.' + pt * '0'))
            except (decimal.InvalidOperation, decimal.DecimalException):
                success = False
        elif isinstance(number, decimal.Decimal):
            number = number.quantize(decimal.Decimal('0.' + pt * '0'))
        else:
            success = False
