...
class Cfg(object):
    ...
    def __init__(self, path_filename):
        ...
        self.__http_port = self.__get_v(STARTUP_SECTION_RUN, 'http_port')
        if self.__http_port == 0:
            self.__http_port = 80
    ...
    def get_val(self, val, key=STARTUP_SECTION_RUN):  # 获取配置项，通用
        return self.__get_v(key, val)

    def str_val(self, val, key=STARTUP_SECTION_RUN):  # 获取配置项，通用
