...
def _get_torrent_target_filepath(torrent, target_path_template, torrent_filename):
    ...
    try:
        target_path = target_path_template.format(
            ...
            name=torrent.name,
            torrent_date=torrent.creation_date.strftime('%Y-%m-%d'),
            torrent_datetime=torrent.creation_date.strftime('%Y-%m-%dT%H:%M:%S'),
            torrent_time=torrent.creation_date.strftime('%H:%M:%S'),
            tracker=get_tracker(),
        )
