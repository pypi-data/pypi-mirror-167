...
def trace_2D(function, min_variable, max_variable, values_number, args_before_variable=[], args_after_variable=[], **kwargs) :
    ...
    plt.plot(variable_list, results_list)#displaying

def trace_3D(function, min_x, max_x, values_x, min_y, max_y, values_y, args_before_variables=[], args_between_variables=[], args_after_variables=[], x_before_y = True, colormap=cm.RdYlGn, label_x ="", label_y="", label_z="", **kwargs) :
    ...
    label_x, label_y et label_z are texts to put on the axes
    You can add after keywords arguments for the function "function"
    """
    ...
    ax.plot_surface(array_x, array_y, array_z, cmap=colormap, linewidth=5)#linewidth : distance entre les lignes
