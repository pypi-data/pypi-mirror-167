...
class GantryQueryFrame(object):
    ...
    def _head(
        ...
        data = self._populate_raw_request(
            query_filters=query_filters, ascending=True, n=n, tags=tags
        )
    ...
    def _tail(
        ...
        if not (n > 0):
            raise ValueError("n must be a positive int. Got {}".format(n))
        data = self._populate_raw_request(
            query_filters=query_filters, ascending=False, n=n, tags=tags
        )
        return self._prepare_pandas_dataframe(data, column)
    ...
    def _fetch(
        ...
        """
        data = self._populate_raw_request(
            query_filters=query_filters, ascending=True, n=None, tags=tags
        )
        return self._prepare_pandas_dataframe(data, column)
