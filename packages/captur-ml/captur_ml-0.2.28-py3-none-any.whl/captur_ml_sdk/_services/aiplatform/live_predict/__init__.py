...
def get_image_classification_prediction_from_deployed_automl(
    ...
    try:
        ...
        raise GoogleCloudVertexAIEndpointDoesNotExistError(code=e.code, endpoint_id=endpoint_id)

    except google_exceptions.FailedPrecondition as e:
        if "doesn't have traffic_split" in e.message:
            raise GoogleCloudVertexAIEndpointNoModelDeployedError(code=e.code, endpoint_id=endpoint_id)
        elif re.search(r"exceeds \d*.* limit", e.message) is not None:
            match = re.search(r"exceeds (\d*.*) limit", e.message)
            size_limit = match.group(1)
            raise GoogleCloudVertexAIEndpointImageTooLargeError(code=e.code, size_limit=size_limit)
