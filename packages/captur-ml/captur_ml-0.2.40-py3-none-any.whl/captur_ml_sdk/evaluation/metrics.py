...
class Metrics(metaclass=abc.ABCMeta):
    ...
    def calculate(self) -> Dict:
        if self.is_external:
            return self.external_metrics()

        else:
            metric_list = self.internal_metrics().keys()
            if self.metrics is not None:
                metric_list = self.metrics

            metrics_dict = {}
            for metric in metric_list:
                metrics_dict[metric] = self.internal_metrics()[metric]()
            return metrics_dict

    @staticmethod
    def dictify_per_class_metrics(classes, result):
        return {
            class_name: score for class_name, score in zip(classes, result)
        }

    @staticmethod
    def csvify_confusion_matrix(classes: List[str], confusion_matrix: List[List[int]]):
        csv = []
...
class ClassificationMetrics(Metrics):
    def __init__(self, predictions: torch.Tensor, labels: torch.Tensor, classes: List[str], metrics: List[str] = None, top_k: int = None, is_external: bool = False) -> None:
    ...
    def f1_score_per_class(self) -> Dict[str, float]:
        ...
        return self.dictify_per_class_metrics(result, self.classes)
...
class AuditMetrics(Metrics):
    def __init__(self, audit_labels: List[List[str]], predictions: List[str], is_external: bool = False) -> None:
    ...
    def audit_precision_per_class(self):
        all_classes = list(set(self.predictions))
        precision_dict = {}
...
class MappedClassificationMetrics(Metrics):
    def __init__(self, labels: List[str], predictions: List[str], classes: List[str], metrics: List[str] = None, is_external: bool = False) -> None:
        super().__init__(labels, predictions, metrics, is_external)
        self.classes = classes
    ...
    def f1_score_per_class(self):
        ...
        return self.dictify_metrics(self.classes, result)
