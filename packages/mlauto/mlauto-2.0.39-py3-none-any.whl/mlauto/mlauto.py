...
def block(block_name, connect_to, project_id):

  data = {'project_id': project_id, 'block_name': block_name, 'connect_to': connect_to, 'username': os.environ['username'], 'key': os.environ['key']}
  response = requests.post('http://'+IP+'/api/create_new_project', data=data)
  ...
  if response.text == '0':
    ...
    if response_dict['exists'] == 0:
      print("Block created! Now continue by creating the connecting block')
    else:
      print("Using a block that already exists.")
      os.environ["project_id"] = str(response_dict['project_id'])


      
  
def node(node_name, connect_to, node_description, start_node = False, end_node = False):
  data = {'node_name': node_name, 'connect_to': connect_to, 'node_description': node_description,
